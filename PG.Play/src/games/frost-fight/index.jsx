...
import BottomRail from './ui/BottomRail.jsx';
import { LevelIntro, LevelClearChip, WinCard, GameOverCard, ThemePreloadOverlay } from './ui/Overlay.jsx';
import { markRoomCleared, markRoomReached, markRunCleared } from './utils/progress.js';
import { useIsMobile } from '../../input/useVirtualControls.jsx';
import { spawnFx, tickFx, drawFx, spawnFloater, spawnRing } from './fx.js';
...
import playerSpriteUrl     from './sprites/player.png?url';
import player2SpriteUrl    from './sprites/player-2.png?url';
...
import skinVanillaUrl      from './sprites/icecream-vanilla.png?url';
import skinSundaeUrl       from './sprites/icecream-sundae.png?url';
import skinTripleUrl       from './sprites/icecream-triple.png?url';
import skinSandwichUrl     from './sprites/icecream-sandwich.png?url';
const PLAYER_SKINS = {
...
import strawberrySpriteUrl from './sprites/strawberry.png?url';
import blueberrySpriteUrl  from './sprites/blueberry.png?url';
import fruitSpriteUrl      from './sprites/fruit.png?url';
import peachSpriteUrl      from './sprites/peach.png?url';
import iceSpriteUrl        from './sprites/ice.png?url';
import exitSpriteUrl       from './sprites/exit.png?url';
import strawberryWindupUrl from './sprites/strawberry-windup.png?url';
import blueberryWindupUrl  from './sprites/blueberry-windup.png?url';
import orangeSpriteUrl     from './sprites/orange.png?url';
...
import orangeWindupUrl     from './sprites/orange-windup.png?url';
import cherrySpriteUrl     from './sprites/cherry.png?url';
import cherryWindupUrl     from './sprites/cherry-windup.png?url';
...
import bananaBotUrl        from './sprites/banana-bot.png?url';
import grapeBotUrl         from './sprites/grape-bot.png?url';
import grapeWindupUrl      from './sprites/grape-windup.png?url';
import plumBotUrl          from './sprites/plum-bot.png?url';
import eggplantBotUrl      from './sprites/eggplant-bot.png?url';
import melonBotUrl         from './sprites/melon-bot.png?url';
import cherrybombBotUrl    from './sprites/cherrybomb-bot.png?url';
...
import kiwiWindupUrl       from './sprites/kiwi-windup.png?url';
// Themed fruit pickups (sheet 1). All count as 1 toward room clear; the
// kind change is purely visual variety per era.
import appleFruitUrl       from './sprites/apple-fruit.png?url';
import lemonFruitUrl       from './sprites/lemon-fruit.png?url';
import kiwiFruitUrl        from './sprites/kiwi-fruit.png?url';
import cherryFruitUrl      from './sprites/cherry-fruit.png?url';
import coverUrl            from './sprites/cover.webp?url';
...
import { loadAtlas, getFrame, tickAnim, setAnimAction, ATLAS_CHARS }
  from './atlas.js';
import { preloadTheme } from './preload.js';
...
const LEVELS = [
  ...
  {
    name: 'Bramble',
    tip: 'Harvest 8 — bramble corridors. Two grapes lay ice trails; pick a lane.',
    // Bramble shape: two long horizontal hedges divide the room into
    // three lanes. Apples line the outer lanes; the middle lane is the
    // grape highway with a peach scout. Pick a lane and commit — the
    // grape leaves an ice trail in whichever lane you don't pick.
    grid: [
      '######################',
      '#p..A.....A.....A....#',
      '#....................#',
...
export default function FrostFightGame({ mode = 'solo', difficulty = DEFAULT_DIFFICULTY, theme = DEFAULT_THEME, startLevel = 0, skin = 'default' }) {
  ...
  // Theme preload — front-load every atlas frame, wall texture, floor
  // texture, and base sprite the chosen theme uses BEFORE mounting the
  // gameplay loop. The browser cache is already very good at serving
  // these once they've been requested; this just ensures they're warm
  // by the time the canvas first paints, so there's no mid-room hitch
  // when a kind first appears or a wall texture first decodes. The
  // progress callback drives a loading bar overlay below.
  ...
  useEffect(() => {
    ...
    setPreloading(true);
    setPreloadPct(0);
    preloadTheme(theme, (loaded, total) => {
      if (cancelled) return;
      setPreloadPct(total > 0 ? loaded / total : 1);
    }).then(() => {
      if (cancelled) return;
      setPreloading(false);
    });
