...
## intuition
Here is an analogy first. Imagine photographing a relay race where you cannot stop the runners and cannot fit everyone in one frame. If you photograph each runner at a slightly different instant, a baton might appear in two photos (double-counted) or in none (lost). Chandy-Lamport's fix is to send a whistle down each lane: the moment a runner hears the whistle they freeze their own pose in memory, and they also start noting every baton that arrives on their other lanes until a whistle comes down those lanes too. Stitch the frozen poses and the noted batons together and you get a picture that is internally consistent — every baton counted exactly once — even though no single camera flash ever captured it.

Concretely: pick one process to initiate. It records its own state, then sends a special `MARKER` message along every outgoing channel before it sends any further application message. When any other process p receives a `MARKER` along channel c for the first time, it records its state, marks the state of c as empty, then sends `MARKER` along all its outgoing channels. For every other incoming channel c', p starts recording the messages that arrive on c' until a marker arrives there; those recorded messages become the channel state for c'. When markers have flowed along every channel, the union of process states plus channel states is the consistent snapshot. FIFO channels are the crucial assumption: the marker acts as a clean delimiter between "before snapshot" and "after snapshot" messages on each channel. In a 3-process system with bidirectional links that is 6 directed channels and exactly 6 markers — one per channel — and the "money in flight" you recover equals every in-transit message no process had yet consumed.

## visualization
Three processes A, B, C; A initiates. Every directed channel carries one MARKER:

```
step  event                                          recorded
 1    A: record S_A; send MARKER -> B, C             {S_A}
 2    C->B sends app msg m  (before C sees a marker)  in flight on C->B
 3    B: first MARKER (from A); record S_B;           {S_B}
        chan A->B = empty; send MARKER -> A, C
 4    B: begin recording channel C->B ...
 5    m arrives at B after A-marker, before C-marker  -> C->B state = [m]
 6    C: first MARKER (from A); record S_C;           {S_C}
        chan A->C = empty; send MARKER -> A, B
 7    all 6 channels have carried a marker            -> DONE
snapshot = { S_A, S_B, S_C, A->B=[], C->B=[m], A->C=[], ... }
```
...
The reason to reach for the marker protocol over any "pause and ask" scheme is that it is *non-blocking*: application messages keep flowing while the snapshot is collected, so throughput is untouched and the only overhead is O(E) marker messages plus per-channel buffering. What you get in return is a subtle but exactly-sufficient guarantee. The snapshot never captures an effect without its cause, which is precisely the property recovery, deadlock detection, and stable-property evaluation require. It is emphatically *not* a wall-clock instant: no real moment need have looked like the snapshot, but the snapshot is *reachable* from some real state by replaying the recorded channel messages, and reachability is all a recovery algorithm needs. The failure modes to engineer around: a slow marker on one channel forces the receiving process to keep buffering on every *other* in-channel until it arrives, so memory spikes — production systems cap the buffer or fall back to aligned/synchronous checkpointing (Flink does both). Multiple concurrent initiators must tag markers and recordings with a unique snapshot id, or two snapshots interleave and corrupt each other. And process failures mid-collection are outside the original paper, so real systems layer barrier alignment plus acknowledgement on top — Flink's checkpoint barriers along the dataflow DAG are Chandy-Lamport with exactly that added.
