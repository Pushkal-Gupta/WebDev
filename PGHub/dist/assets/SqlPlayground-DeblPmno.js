...
import{_ as V}from"./index-KPKUfE1T.js";import{r as n,j as e}from"./vendor-query-FJdQ8OJm.js";import{F as Ge}from"./vendor-monaco-BrjDLSos.js";import{P as Me}from"./PlaygroundSwitcher-BSKwVlei.js";import{r as qe,a as Fe}from"./monacoTheme-yMFPRmqj.js";import{b as we,u as Pe,L as f}from"./vendor-react-firagBrd.js";import{x as de,a0 as q,a1 as Te,v as ue,L as Re,a2 as me,Q as xe,u as pe,a3 as je,q as Se,g as Xe,i as He,a4 as Ve,c as Je,X as Ke}from"./vendor-icons-Bc055RmK.js";import"./vendor-supabase-ClVc2H6D.js";const Ae=""+new URL("sql-wasm-UFUCzYNW.wasm",import.meta.url).href,ze=`
...
WHERE 
ORDER BY 
;`,expected:{columns:["Year","cum_value"],values:[[2020,41250],[2021,82810],[2022,124790],[2023,167020]]}}]},"employees-dept":{id:"employees-dept",kind:"playground",title:"Employees & Departments",blurb:"Classic toy schema: 10 employees across 4 departments. The simplest place to practice SELECT, WHERE, JOIN, GROUP BY.",tables:["employees","departments"],seedSql:aa},chinook:{id:"chinook",kind:"playground",title:"Chinook (Music Store)",blurb:"Artists, albums, tracks, customers, invoices. The well-known SQLite sample database for trying JOINs across a real-shaped schema.",tables:["artists","albums","genres","tracks","customers","invoices","invoice_items"],seedSql:Qe},sakila:{id:"sakila",kind:"playground",title:"Sakila (Movie Rental)",blurb:"A DVD rental store: actors, films, rentals, customers, payments. MySQL’s canonical sample database, ported to SQLite.",tables:["actors","categories","films","film_actors","customers","rentals","payments"],seedSql:$e},world:{id:"world",kind:"playground",title:"World (Countries & Cities)",blurb:"Countries, cities, and spoken languages — 15 countries with GDP and population, 28 major cities, and language usage rows for HAVING and window-function drills.",tables:["countries","cities","languages"],seedSql:Ze},ecommerce:{id:"ecommerce",kind:"playground",title:"E-commerce",blurb:"Customers, products, orders, line items, reviews. Realistic order shape for aggregation, top-N per group, and review-rating queries.",tables:["customers","products","orders","line_items","reviews"],seedSql:ea},library:{id:"library",kind:"playground",title:"Library (Books & Loans)",blurb:"Authors, books, members, and loans. NULL return dates make this the natural place to practice outstanding-loan filters and LEFT JOINs.",tables:["authors","books","members","loans"],seedSql:sa},university:{id:"university",kind:"playground",title:"University (Enrollments)",blurb:"Students, instructors, courses, and enrollments with grades. GPA averages, course rosters, and per-department aggregation across terms.",tables:["students","instructors","courses","enrollments"],seedSql:ta},social:{id:"social",kind:"playground",title:"Social Network",blurb:"Users, follows, posts, and likes. A self-referencing follow graph for mutual-follow, most-liked, and follower-count queries.",tables:["users","follows","posts","likes"],seedSql:na},flights:{id:"flights",kind:"playground",title:"Flights & Airports",blurb:"Airports, airlines, flights, and bookings. Origin/destination both point at airports — ideal for self-join routing and fare aggregation.",tables:["airports","airlines","flights","bookings"],seedSql:ia},hospital:{id:"hospital",kind:"playground",title:"Hospital (Visits)",blurb:"Doctors, patients, visits, and prescriptions. Per-specialty visit counts, cost totals, and prescription drilldowns through a visit join.",tables:["doctors","patients","visits","prescriptions"],seedSql:ra},bank:{id:"bank",kind:"playground",title:"Bank (Accounts)",blurb:"Branches, customers, accounts, and signed transactions. Running balances, deposit vs withdrawal splits, and per-branch rollups.",tables:["branches","customers","accounts","transactions"],seedSql:la}};function oa(){return Object.values(Le).filter(S=>S.kind==="playground")}function Ea(S,A){if(!S||S.length===0)return{ok:!1,reason:"No rows returned (your query produced an empty result)."};const c=S[S.length-1];if(!Array.isArray(c==null?void 0:c.columns)||!Array.isArray(c==null?void 0:c.values))return{ok:!1,reason:"Could not interpret the result."};const i=c.columns.map(t=>String(t)),g=A.columns.map(t=>String(t));if(i.length!==g.length||i.some((t,E)=>t!==g[E]))return{ok:!1,reason:`Column mismatch.
  expected: ${g.join(", ")}
...
`,ca={"employees-dept":[{label:"All employees",sql:"SELECT * FROM employees ORDER BY salary DESC;"},{label:"Department headcount",sql:"SELECT d.name, COUNT(*) AS n FROM employees e JOIN departments d ON d.id = e.department_id GROUP BY d.name;"},{label:"Window: rank by salary",sql:"SELECT name, salary, RANK() OVER (ORDER BY salary DESC) AS rank FROM employees;"},{label:"CTE: above-avg earners",sql:`WITH avg_s AS (SELECT AVG(salary) AS a FROM employees)
...
GROUP BY b.id ORDER BY customers DESC;`},{label:"Schema",sql:"SELECT name, sql FROM sqlite_master WHERE type='table';"}]},he=["var(--accent)","var(--hue-violet)","var(--hue-sky)","var(--hue-pink)","var(--hue-mint)"],da="pgcode_sql_last_playground_db",K="pgcode_sql_custom_seed",ge=`-- Your tables are loaded. List them, then query away.
SELECT name FROM sqlite_master WHERE type='table';
`;function Oe(S){const A=[],c=/CREATE\s+(?:TEMP(?:ORARY)?\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?["`[]?([A-Za-z_][\w$]*)["`\]]?/gi;let i;for(;(i=c.exec(S))!==null;)A.includes(i[1])||A.push(i[1]);return A}function ha({theme:S}){var ce;const A=we(),{courseSlug:c}=Pe(),i=c?Le[c]:null,g=c==="custom",R=(i==null?void 0:i.kind)==="course",d=(i==null?void 0:i.kind)==="playground",o=R?i:null,t=d?i:null,E=n.useMemo(()=>oa(),[]),[T,z]=n.useState(()=>{if(typeof window>"u")return"";try{return localStorage.getItem(K)||""}catch{return""}}),p=g&&!!T;n.useEffect(()=>{if(d&&c)try{localStorage.setItem(da,c)}catch{}},[d,c]);const[Y,W]=n.useState(0),l=(ce=o==null?void 0:o.questions)==null?void 0:ce[Y],[I,v]=n.useState(null),k=p?T:(i==null?void 0:i.seedSql)||"",F=o?`pgcode_sql_course_${o.id}_${(l==null?void 0:l.id)||"q"}`:t?`pgcode_sql_pg_${t.id}`:p?"pgcode_sql_pg_custom":"pgcode_sql_picker",Ie=R?(l==null?void 0:l.starter)||"-- write your query here":t?Ne[t.id]||J:p?ge:J,ye=typeof window<"u"&&localStorage.getItem(F)||Ie,[C,b]=n.useState(ye),Q=l==null?void 0:l.starter;n.useEffect(()=>{if(!R||!l)return;const a=`pgcode_sql_course_${o.id}_${l.id}`;b(localStorage.getItem(a)||Q||"-- write your query here"),v(null)},[R,o==null?void 0:o.id,l,Q]);const B=t==null?void 0:t.id;n.useEffect(()=>{if(!d||!B)return;const a=`pgcode_sql_pg_${B}`,s=Ne[B]||J;b(localStorage.getItem(a)||s),y([]),O(null),_(null)},[d,B]),n.useEffect(()=>{if(!p)return;b(localStorage.getItem("pgcode_sql_pg_custom")||ge),y([]),O(null),_(null)},[p]);const[D,$]=n.useState(!1),[w,Z]=n.useState(!1),[ee,O]=n.useState(null),[ae,y]=n.useState([]),[se,_]=n.useState(null),N=n.useRef(null),[te,ne]=n.useState(""),[P,Ce]=n.useState(""),[ie,h]=n.useState(null),[x,re]=n.useState(!1),j=n.useCallback(async a=>{const s=(a||"").trim();if(!s){h("Nothing to load — paste some SQL or fetch a URL first.");return}h(null);try{const r=(await V(async()=>{const{default:H}=await import("./sql-wasm-browser-zRtwOtG3.js").then(Ue=>Ue.s);return{default:H}},__vite__mapDeps([0,1]),import.meta.url)).default,u=await r({locateFile:()=>Ae}),m=new u.Database;m.run(s);const L=Oe(s).length;if(m.close(),L===0){h("No CREATE TABLE statements found. Include at least one table definition.");return}try{localStorage.setItem(K,s)}catch{}z(s)}catch(r){h(`That SQL did not load:
