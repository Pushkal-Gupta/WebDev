...
1. ~~**Search not working**~~ — **FIXED + APPLIED LIVE 2026-06-21.** Root cause: `pgcode_problem_page` RPC kept number-bucket paging active during a search. `migrate-58-search-number-bucket-fix.sql` applied to prod via Management API; verified: search "island" → Number of Islands, Island Perimeter, Flood Map Island Count, ... (5 matches).
...
**UPDATE 2026-06-21 — backfill SCALED + harness fixed + APPLIED:** Search migrate-58 applied to prod via Management API (search verified working: "island" → 5 matches). Solutions backfill upgraded: `scripts/backfill-solutions.mjs` now merges 3 authored batch files (`scripts/sol-batches/batch-{easy,medium,hard}.mjs` = 41 new signature-matched solutions for zero-solution targets) on top of its 30 inline canonicals → **62 unique targets**. Two grader gaps fixed in `scripts/sol-batches/grade-helpers.mjs` (`compareOutputSmart`): (1) string-return quote-asymmetry (py prints bare, js/java/cpp JSON-quote — strip matching outer quotes both sides), (2) order-insensitive list outputs (subsets/combinations/combination-sum/k-closest/find-all-duplicates/permutations — canonicalize/sort both sides). First live run wrote ~14 problems' verified solutions before the upgrade; v2 run (LIVE, background `/tmp/backfill-v2.log`) skips already-written langs and grades the rest with smart-compare. Skipped-and-documented: tree-as-`List[int]` params the driver can't parse (null tokens crash), and `Any`-return count-vs-board mismatches — those need a test/param-type fix, not a solution. To scale toward 242: author more batch files under `scripts/sol-batches/` (same `export default {slug:{python,javascript,java,cpp}}` shape), they auto-merge.

**UPDATE 2026-06-22 — SCALED to 450 staged solutions.** Re-audit (catalog grew to 4,364 as other processes add problems): **zero-solution 174→81**, python coverage **98.1%**, but JS/Java/C++ ~84.9% → the real gap is now **590 problems that HAVE verified Python but lack JS/Java/C++**. Strategy pivoted from authoring-from-scratch to **translating the verified Python** (far more scalable). Fan-out: 80 authored (zero-solution) + **354 translated** (xlate-1..12, slices [0,360) of the 590) = **434 staged solutions** across 17 `scripts/sol-batches/(batch|xlate)-*.mjs` files (auto-merged; glob widened to `(batch|xlate)-*.mjs`). Backfill v6 (LIVE, `/tmp/backfill-v6.log`, 450 targets) grading+writing in background — Judge0-grades each lang before write, skips already-present. **THE BOTTLENECK IS NOW Judge0 public-CE throughput**, not authoring — grading 450×~3langs×up-to-50-cases is many thousands of rate-limited submissions (hours). A dedicated Judge0 key / self-hosted instance is the single biggest unlock. Remaining to stage: slices [360,590) of the missing-language set (~230 more, same translation pattern). Confirmed written so far via earlier runs: ~78 problems (v2+v3). Re-audit after v6 to get the true delta.

**UPDATE 2026-06-22 (final stage) — ENTIRE backlog staged.** Completed all translation slices [0,590) via xlate-1..20 (20 files): **582 translations + 80 authored = 662 staged solutions** across 25 `scripts/sol-batches/(batch|xlate)-*.mjs` files — covers all 81 zero-solution + all 590 missing-language targets (only ~12 genuinely-ungradeable skipped: `List[float]` returns, `null`-token-in-`List[int]` tree params, bare nested `List`, `Any`-display-string returns — those need a driver/param-type fix, not a solution). Comprehensive backfill **v7 (LIVE, `/tmp/backfill-v7.log`, 675 targets)** running uninterrupted in background: skips already-present langs, Judge0-grades each new lang, writes only passing. **Authoring is DONE — the only remaining work is Judge0 grading throughput** (v7 will grind for hours; ~662 × up-to-3-langs × up-to-50-cases = many thousands of rate-limited submissions). A dedicated Judge0 key / self-hosted instance is THE unlock to finish fast. Re-run `audit-solutions.mjs` after v7 completes for the true final delta. STOP killing/relaunching the runner — let v7 finish; each relaunch re-walks all 675 skip-present checks.
...
## SOLUTIONS — AUTHORING COMPLETE (2026-06-24): 903 staged across 36 batch files
Every gradeable problem now has a staged, signature-matched solution. Coverage went all-four-real 84.6%→87.5% (and climbing as the watchdog writes). Staged: 30 inline + ~290 authored (zero-solution: batch-easy/medium/hard + batch-z1..z5) + ~580 translated (xlate-1..26 from verified Python). Watchdog (`backfill-watchdog.sh`, sample-graded ~3-4x faster) grades + writes them over its passes — Judge0 throughput is the only remaining gate.

**The GENUINE CEILING (problems that CANNOT reach 4-lang coverage without a DATA/DRIVER fix — NOT authoring):**
- **`Any` return type**: `generateTemplate` emits the literal `Any` as the Java/C++ return type → uncompilable, so those get python+js only (n-queens, bulk-switcher, champagne-tower, buildTree pairs, etc.). Fix = give these problems a concrete `return_type` in the DB.
- **Display-string expected outputs** (`"5, nums = [1,1,2,2,3,_]"`): LeetCode in-place-mutation display text the driver can't produce (remove-duplicates-from-sorted-array-ii, populating-next-right-pointers). Fix = normalize the stored `expected` to JSON.
- **Tree-as-`List[int]`-with-`null`-tokens** (~13 zero-solution + ~10 translatable): params typed `List[int]`/`List[Optional[int]]` whose inputs carry `null`/`None` → the int parsers crash. Fix = retype the param to `Optional[TreeNode]` (the driver HAS tree helpers) and ensure the Python solution takes a root. ~7 of these have tree-shaped Python already.
- **Tree-serialization / list-of-TreeNode `Any` returns** (unique-binary-search-trees-ii): no harness. Fix = bespoke serializer or retype.
These ~35-40 need a focused DB-params/driver pass — deferred (shared live grader + a parallel process is active; risky to touch now). Everything else is staged.

## CONCEPT-VIZ COVERAGE — 100% (486/486) as of 2026-06-23
Every concept now resolves to an interactive visualization (the platform north-star). Drove the gap closed via parallel agent waves building standalone `src/components/learn/viz/*Viz.jsx` (theme tokens, lucide, deterministic/mulberry32, no inner scrollbars, viewBox-scaled, vertical model diagrams), wired centrally in `interactiveViz.js` (no dup keys, build green). Triple-reviewed: fixed 1 slug mismatch (`three-pointers`) + glyph cleanups. Detection caveat learned: coverage MUST be checked by each concept's frontmatter `slug:`, not the md filename (they differ for some, inflating false "missing"). Fixed `sparse-table-rmq-deep` which a parallel pass had mapped to the BASIC `SparseTableRmqViz` → repointed to the purpose-built `SparseTableDeepViz`. NOTE: a few orphaned duplicate viz files exist (two passes built for the same slug; parallel pass won registration) — harmless dead code, left in place.

## SOLUTIONS BACKFILL — resilient watchdog (2026-06-23)
`scripts/backfill-watchdog.sh` (background) re-runs `backfill-solutions.mjs` until a full pass writes 0 new (converged) or 12-pass cap — catches transient Judge0 rate-limit/network failures across passes. Logs: `.logs/backfill.log` + `.logs/watchdog.log`. 662 staged solutions; Judge0 public-CE throughput is the only bottleneck (a machine sleep kills detached processes — `/tmp` was cleared once, killing earlier runs; logs now under `.logs/` not `/tmp`). Re-launch the watchdog on next session if a sleep stopped it; re-run `audit-solutions.mjs` for the true delta.
