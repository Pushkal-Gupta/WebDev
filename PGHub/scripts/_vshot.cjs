...
const SHOTS = [['fx-assessments','/#/assessments'], ['fx-vizgallery','/#/visualize'], ['fx-concepts','/#/learn']];
(async () => {
  ...
  for (const [name, route] of SHOTS) {
    ...
    try { await p.goto(BASE + route, { waitUntil: 'networkidle2', timeout: 30000 }); } catch (e) { console.log('WARN '+name+': '+e.message); }
