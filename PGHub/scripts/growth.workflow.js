...
Read ${dir}/slice.json — an array of problems { id, name, difficulty, constraints, method_name, params, return_type, python }. \`python\` is the correct canonical solution; \`constraints\` states the valid input ranges/shape.

For EACH problem, produce ~24 NEW, DIVERSE, STRICTLY-VALID input tuples. Each tuple is an array of per-parameter strings, one per entry in \`params\`, in this exact string format:
- int -> "7" ; float -> "1.5" ; bool -> "true"/"false" ; str -> the raw string e.g. "leetcode"
- List[int] -> "[1,2,3]" ; List[str] -> "[\\"a\\",\\"b\\"]" ; List[List[int]] -> "[[1,2],[3,4]]"
Match the order and types of \`params\` exactly (look at the python signature to confirm).
