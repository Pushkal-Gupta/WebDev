#!/usr/bin/env node
// Grade authored Python solutions for stub-targets slice [890,950) against the
...
//
// Authored Python lives in scripts/sol-batches/stub-slice-890-950.mjs keyed by id.
//
//   node scripts/run-stub-slice-890.mjs --dry            # grade, no write
//   node scripts/run-stub-slice-890.mjs                  # grade + write on full pass
//   node scripts/run-stub-slice-890.mjs --only id1,id2   # subset
...
import AUTHORED from './sol-batches/stub-slice-890-950.mjs';
...
const PAUSE_MS = Number(val('pause') || 50);
const PY_ID = 71;
...
async function gradePython(problem, code) {
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    if (!r.ok) return { pass: false, passed, total: cases.length, skipped, corrupted, reason: `case ${i}: ${r.error?.slice(0, 180)}`, failInputs: tc.inputs };
    if (!compareOutputSmart(r.stdout, tc.expected, { orderInsensitive: ORDER_INSENSITIVE.has(problem.id) }))
      return { pass: false, passed, total: cases.length, skipped, corrupted, reason: `case ${i} WA: got ${JSON.stringify(r.stdout).slice(0, 80)} want ${JSON.stringify(tc.expected).slice(0, 80)}`, failInputs: tc.inputs };
    passed++; await sleep(PAUSE_MS);
  }
  return { pass: true, passed, total: cases.length, skipped, corrupted, reason: null };
}
...
async function main() {
  ...
  let slice = all.slice(890, 950).filter(s => !corrupt.has(s));
  if (ONLY) slice = slice.filter(s => ONLY.includes(s));
  console.log(`run-stub-slice-890 ${DRY ? '(DRY)' : '(LIVE)'} | Judge0 ${JUDGE0_URL} | ALL cases | targets ${slice.length}\n`);
  const results = [];
  for (const id of slice) {
    process.stdout.write(`> ${id.padEnd(62)} `);
  ...
  for (const [st, ids] of Object.entries(by)) console.log(`${st.padEnd(20)} ${ids.length}  ${ids.join(', ')}`);
  fs.writeFileSync('/tmp/stub_slice_890_results.json', JSON.stringify(results, null, 2));
  console.log('\nresults -> /tmp/stub_slice_890_results.json');
}
