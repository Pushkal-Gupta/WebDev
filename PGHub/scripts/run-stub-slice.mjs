#!/usr/bin/env node
// Grade authored Python solutions for stub-targets slice [100,150) against the
...
//
// Authored Python lives in scripts/sol-batches/stub-slice-100-150.mjs keyed by id.
//
...
import AUTHORED from './sol-batches/stub-slice-100-150.mjs';
...
async function processOne(id) {
  const { data: problem, error } = await sb.from('PGcode_problems')
    .select('id, name, method_name, params, return_type, test_cases, solutions').eq('id', id).maybeSingle();
...
async function main() {
  ...
  let slice = all.slice(100, 150).filter(s => !corrupt.has(s));
