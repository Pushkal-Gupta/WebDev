...
// Topic -> ForgeThumb motif. Keyword-matched on the section title so each card
...
const TOPIC_MOTIF_RULES = [
  ...
  [/prefix sum|range query|segment|fenwick/i, 'matrix'],
  [/dynamic programming|\bdp\b|memo/i, 'matrix'],
  [/string matching|\bkmp\b|rabin|substring/i, 'heat'],
  [/hash/i, 'heat'],
  ...
  [/greedy/i, 'bars'],
  [/array|string/i, 'bars'],
  [/fundamental|basics|intro/i, 'cards'],
];

function motifForTopic(title) {
  for (const [re, motif] of TOPIC_MOTIF_RULES) if (re.test(title)) return motif;
  return 'cards';
}
          ...
          {visibleSections.map(({ info, idx }) => (
            ...
            >
              <div className="tut-card-thumb" aria-hidden="true">
                <ForgeThumb seed={info.title} kind={motifForTopic(info.title)} />
              </div>
