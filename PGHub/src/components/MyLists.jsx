...
export default function MyLists({ session }) {
  ...
  const submitRename = async (name) => {
    ...
    if (!id || !name.trim() || pendingId === id) return;
    setPendingId(id);
    try {
  ...
  return (
    <div className="ml-container">
      ...
      <div className={`ml-create-row${justCreated ? ' ml-create-row-done' : ''}`}>
        <input
          ref={createInputRef}
          className="ml-input"
          value={newListName}
        ...
        />
        {justCreated && (
          <span className="ml-create-done" role="status">
      ...
      {lists.length === 0 ? (
        <div className="ml-empty ml-empty-fill">
          <ListPlus size={28} className="ml-empty-icon" />
          <h2 className="ml-empty-title">No lists yet</h2>
          <p className="ml-empty-sub">Name a list above and hit Create. Add problems to it from any problem page.</p>
        </div>
      ) : (
      ...
      {renameTarget && (
        <RenameModal
          ...
          pending={pendingId === renameTarget.id}
...
function RenameModal({ initialName, pending, onClose, onSave }) {
  const [name, setName] = useState(initialName);
  const inputRef = useRef(null);
  useEscapeClose(onClose);
  useEffect(() => { inputRef.current?.focus(); inputRef.current?.select(); }, []);

  const canSave = name.trim() && name.trim() !== initialName.trim() && !pending;
  const save = () => { if (name.trim() && !pending) onSave(name); };

  return (
    <div className="ml-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="ml-rename-title" onMouseDown={onClose}>
      <div className="ml-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="ml-modal-head">
          <div className="ml-modal-titlerow">
            <Pencil size={15} className="ml-modal-icon" />
            <h2 className="ml-modal-title" id="ml-rename-title">Rename list</h2>
          </div>
          <button className="ml-modal-x" onClick={onClose} aria-label="Close"><X size={15} /></button>
        </div>
        <label className="ml-modal-label" htmlFor="ml-rename-input">List name</label>
        <input
          id="ml-rename-input"
          ref={inputRef}
        ...
        <div className="ml-modal-actions">
          ...
          <button className="ml-btn ml-btn-primary" onClick={save} disabled={!canSave}>
            <Check size={13} /> Save
          </button>
        </div>
      </div>
...
function DeleteModal({ name, pending, onClose, onConfirm }) {
  useEscapeClose(onClose);
  return (
    <div className="ml-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="ml-delete-title" onMouseDown={onClose}>
      <div className="ml-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="ml-modal-head">
          <div className="ml-modal-titlerow">
            <Trash2 size={15} className="ml-modal-icon ml-modal-icon-danger" />
            <h2 className="ml-modal-title" id="ml-delete-title">Delete list</h2>
          </div>
          <button className="ml-modal-x" onClick={onClose} aria-label="Close"><X size={15} /></button>
        </div>
        <p className="ml-modal-body">
          Delete <strong>{name}</strong>? Problems in it are not affected.
        </p>
        <div className="ml-modal-actions">
          <button className="ml-btn" onClick={onClose}>Cancel</button>
          <button className="ml-btn ml-btn-danger" onClick={onConfirm} disabled={pending}>
