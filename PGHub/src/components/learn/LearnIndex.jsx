...
// Map a module's slug/name to a ForgeThumb motif so each sub-module card carries
// a fitting picture. First matching pattern wins; the hashed seed keeps any
// unmatched module on a distinct motif rather than a shared default.
const MODULE_THUMB_RULES = [
  [/analysis|complexity|big.?o|principle|asymptot/i, 'bars'],
  [/pattern|design|technique|template|paradigm/i,    'network'],
  [/number|algebra|modular|prime|arithmet/i,         'matrix'],
  [/geometr|sampling|random|probab|sweep|hull/i,     'field'],
  [/graph|network|traversal|shortest|flow|spanning/i, 'network'],
  [/tree|bst|trie|heap|balanced|segment/i,           'tree'],
  [/sort|order|select|partition/i,                   'bars'],
  [/string|match|text|suffix|pattern.?match/i,       'bits'],
  [/bit|xor|mask/i,                                   'bits'],
  [/dp|dynamic|memo|optimi/i,                         'matrix'],
  [/hash|map|set|bucket/i,                            'heat'],
  [/array|window|pointer|range|prefix/i,             'bars'],
  [/recursion|backtrack|divide/i,                     'tree'],
  [/system|distributed|cache|consensus|encoding/i,   'rings'],
  [/stack|queue|deque/i,                              'bars'],
  [/linked.?list/i,                                   'network'],
];

function moduleThumbKind(name = '', slug = '') {
  const hay = `${name} ${slug}`;
  for (const [re, kind] of MODULE_THUMB_RULES) if (re.test(hay)) return kind;
  return 'auto';
}
            ...
            {activeChildren.map(child => {
              ...
              return (
                <Link key={child.slug} to={`/learn/${child.slug}`} className="learn-submodule-card">
                  <div className="learn-submodule-card-thumb" aria-hidden="true">
