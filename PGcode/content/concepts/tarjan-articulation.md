...
## whyItMatters
Cut vertices and bridges describe the fragility of a network. Network engineers use them to find single points of failure in fiber backbones; transportation planners use them to identify roads that, if closed, sever a city; biologists use them on protein-interaction graphs to spot essential proteins. Tarjan's algorithm is also the gateway to biconnected components, strongly connected components (the directed cousin), and 2-edge-connectivity — themes that recur in graph rounds at FAANG and quant interviews.

## intuition
Run DFS and stamp each node with the order it was visited (`disc[u]`). For every node, compute `low[u]` = the smallest `disc` value reachable from `u`'s subtree using tree edges going down and at most one back edge going up. An edge `(u, v)` is a bridge exactly when `low[v] > disc[u]` — meaning v's subtree has no back edge bypassing u, so cutting that edge isolates v. A non-root vertex u is articulation when it has any child v with `low[v] >= disc[u]`. The root is articulation iff it has two or more DFS children.
...
## optimal
One DFS. Maintain a timer; on first visit set `disc[u] = low[u] = timer++`. For each neighbor v: if v is unvisited, recurse, then `low[u] = min(low[u], low[v])`; after returning, check articulation/bridge conditions. If v is already visited and is not the parent in DFS, update `low[u] = min(low[u], disc[v])`. The single traversal completes in O(V + E). Track the DFS parent (or, better, the edge index used to reach the node) to handle multi-edges correctly.
