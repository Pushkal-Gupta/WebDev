const t=[{id:"r1",title:"1. Components & JSX",intro:"A React component is a function returning JSX — XML-like syntax that compiles to `React.createElement` calls. Each component is a self-contained UI building block. The root render mounts the tree.",code:`// In a real app this lives in App.jsx and uses ReactDOM.render.
...
// log sum of v
`,expected:"60"},takeaways:["Render arrays with .map and a stable `key` prop.","Keys should be stable IDs, not array indices (when order can change).","Mapping inline keeps the JSX declarative.","Avoid expensive transforms inside map — precompute in a useMemo."],mistakes:["Using array index as key for a reorderable list — produces UI bugs.","Missing key prop — warning, plus suboptimal reconciliation.","Generating new IDs on each render — defeats memoization."],next:{label:"useEffect",href:"#/courses/react-basics/r6"}},{id:"r6",title:"6. useEffect",intro:"`useEffect(fn, deps)` runs `fn` after render, and again any time a dep changes. Return a cleanup function to unwind subscriptions/timers — it runs before the next effect and on unmount. `deps = []` runs once on mount; `deps = undefined` runs on every render (almost always a bug). Use it for side effects that touch the outside world: fetch data, subscribe to events, set timeouts.",code:`function Clock() {
...
`,expected:"ready"},takeaways:["React.lazy splits a component into its own chunk loaded on demand.","Wrap lazy components in <Suspense fallback={...}> for the loading UI.","Same machinery powers React 19 server data and streaming SSR.","Best for routes, modals, heavy editors — anything not needed on first paint."],mistakes:["Wrapping every component in lazy — too many round-trips, slow UX.","Forgetting the Suspense boundary — React throws.","Lazy-loading code used on initial render — defeats the purpose."],next:{label:"TypeScript: types & inference",href:"#/courses/typescript-basics/ts1"}}],n=[{id:"ts1",title:"1. Types & inference",intro:"TypeScript adds static types to JavaScript. Most of the time the compiler infers types — you only annotate when it can't guess (function parameters, hard cases). Core types: `string`, `number`, `boolean`, `string[]`, `{ x: number }`, plus literal types and unions.",code:`let name: string = "PGcode";
...
`,expected:"37"},takeaways:["Union types model alternatives; narrow them with typeof / instanceof / in.","After narrowing, TS knows the exact branch and allows safe access.","never is the type of impossible branches — useful for exhaustive checks.","Use discriminated unions when shapes share a common literal field."],mistakes:["Operating on a union before narrowing — type error on shared methods.",'Forgetting that `typeof null` is "object" — null needs an explicit check.',"Mixing union narrowing with `as` casts that lie about the type."],next:{label:"Generics",href:"#/courses/typescript-basics/ts5"}},{id:"ts5",title:"5. Generics",intro:"A generic lets a function or type work over many types while preserving the relationship. `function first<T>(arr: T[]): T | undefined { return arr[0]; }` — the return type matches the input element type.",code:`function first<T>(arr: T[]): T | undefined { return arr[0]; }
const n = first([1, 2, 3]);
const s = first(["a", "b"]);
...
`,expected:"30"},takeaways:["Generics parameterize types — function first<T>(arr: T[]): T.","Constraints: <T extends Comparable<T>> restricts the type set.","TypeScript infers most type arguments from usage.","Generic interfaces / types: interface Box<T> { value: T }."],mistakes:["Using `any` where a generic would do — loses type info downstream.","Over-constraining: <T extends string | number> when you only need one.","Confusing type parameters with values — they exist only at the type level."],next:{label:"Discriminated unions",href:"#/courses/typescript-basics/ts6"}},{id:"ts6",title:"6. Discriminated unions",intro:'A discriminated union shares a common literal field that lets TypeScript narrow safely in each branch. Pattern: every variant has `kind: "circle" | "square"` (or similar). TS knows which fields are valid inside each `if`.',code:`type Shape = { kind: "circle"; r: number } | { kind: "square"; side: number };
...
console.log(b.title);
`,expected:"Dune"},takeaways:["Partial / Required / Readonly toggle optionality / mutability across a type.","Pick / Omit slice an existing type by key.","Record<K, V> builds key-value shapes from a literal union.","ReturnType / Parameters / Awaited introspect functions and promises."],mistakes:['Using `Pick<T, "wrong">` — TS errors on unknown keys.',"Stacking Partial<Required<Partial<T>>> — same as Partial<T>; readable code wins.","Forgetting Awaited unwraps nested promises (a generalized ReturnType for async)."],next:{label:"Branded types",href:"#/courses/typescript-basics/ts12"}},{id:"ts12",title:"12. Branded types",intro:'Sometimes two values share a structural type but mean different things — a `UserId` and a `PostId` are both `number`, but mixing them is a bug. Branded types add a phantom field at the type level so they cannot be assigned to each other. Pattern: `type UserId = number & { readonly __brand: "UserId" }`. Construct via a smart constructor that asserts the brand. The brand never exists at runtime — pure compile-time discipline.',code:`type UserId = number & { readonly __brand: "UserId" };
...
The first imports a beginner writes are almost always fmt for formatted I/O and one or two of os, strings, strconv, sort. Resist the urge to wildcard-import — Go has no such syntax on purpose, and the explicit list at the top of each file makes dependencies grep-able.`,code:`package main
...
Constants are declared with const and can be typed or untyped. const Pi = 3.14159 is an untyped floating-point constant — it adopts whatever type the context demands, which means const Pi can be added to either a float32 or a float64 without a conversion.`,code:`package main
...
break, continue, goto, and return are the loop-and-function escape hatches. Labels (Loop: for { ... break Loop }) let break and continue target an outer loop when nested.`,code:`package main
...
`,expected:"55"},takeaways:["for is the only loop keyword — three-clause, condition-only, infinite, and range forms all use it.","if and switch both support an init statement that scopes a variable to the block.","switch cases are implicitly break — fall-through requires an explicit fallthrough.","No ternary — favour a short if/else block or a helper function."],mistakes:["Adding a semicolon at the end of an if/for header expecting C-style — Go inserts them automatically.","Expecting switch to fall through and forgetting fallthrough — silent behavioural change.","Writing while cond { } — that does not parse; use for cond { } instead."],next:"functions-and-multi-return"},{id:"functions-and-multi-return",title:"Foundations · Functions and Multi-Return",subtitle:"Functions are first-class, and returning multiple values is the rule, not the exception.",intro:`Functions are declared with func name(params) returnType. The return type goes after the parameter list, which reads naturally as "function that takes these arguments and returns this." Parameter declarations that share a type can collapse: func add(a, b int) int omits the type on a. Functions with no return write no return type; functions that return nothing are typed (). The unit type only matters when a function value's signature must be written down.
...
Variadic functions accept zero or more arguments of a single type: func Println(args ...interface{}). Inside the function, args is a slice. Callers can pass individual values or spread an existing slice with the ... suffix: Println(items...).`,code:`package main
...
Keep interfaces small. Single-method interfaces are the most reusable; the bigger the interface, the weaker the abstraction is the Go proverb.`,code:`package main
...
The errors-as-values approach is verbose. That verbosity is the feature — every failure point is visible in the call site, no invisible try/catch flow. A function with twenty if err != nil checks is doing twenty things that can fail, and you can see them.`,code:`package main
...
A nil map can be read but not written. m := map[string]int{} and m := make(map[string]int) both produce writable maps; var m map[string]int leaves m nil and panics on any assignment. Always initialise before writing.`,code:`package main
...
A nil pointer dereferenced panics with invalid memory address or nil pointer dereference. The fix is always to check for nil before dereferencing, or to design so the value can never be nil at the call site. There is no equivalent of the C cast that says I promise this is not nil.`,code:`package main
...
Reach for panic for truly unrecoverable programmer errors — failed invariants, impossible enum branches. Use it for this-should-never-happen assertions, not for ordinary failure paths. Library code should almost never panic across its API boundary — return errors instead.`,code:`package main
...
`,expected:"10"},takeaways:["go f() launches f concurrently; the call returns immediately.","Goroutines start at 2 KB and grow — millions on one machine is realistic.","main returning kills every goroutine — coordinate explicitly to keep work alive.","go run -race instruments accesses and flags unsynchronised reads/writes."],mistakes:["Launching goroutines and forgetting to wait — main exits before they print.","Capturing a loop variable by closure in Go < 1.22 — all goroutines see the final value.","Sharing a map or slice across goroutines without a mutex — data race, undefined behaviour."],next:"channels-basic"},{id:"channels-basic",title:"Concurrency · Channels Basics",subtitle:"Typed pipes between goroutines — send, receive, close, range.",intro:`A channel is a typed conduit through which goroutines send and receive values. Create with make(chan T) for an unbuffered channel or make(chan T, N) for a buffered one of capacity N. Send a value with ch <- v; receive with v := <-ch. Both operations block under the right conditions, and that blocking is the synchronisation primitive that makes channels useful.
...
Channels are not free. Each send/receive involves the scheduler. For very high-throughput inner loops, atomic operations or batching reduces the per-message cost. For most application work — a few thousand to a few million ops/sec — channels are the readable choice.`,code:`package main
...
A nil channel inside select disables its case — the case never fires. Set a channel variable to nil to temporarily remove it from contention, then restore it later. This trick is rare but useful when you need conditional cases without restructuring the select.`,code:`package main
...
You can write a method or function that returns a directional channel even though it created a bidirectional one internally — the conversion happens automatically: func gen() <-chan int { ch := make(chan int); go func() { defer close(ch); for i := 0; i < 10; i++ { ch <- i } }(); return ch }.`,code:`package main
...
Context is immutable. Each With* call returns a new context layered over the parent. Reading a value walks up the chain until it finds the key. Cancellation propagates from parent to children, never the other way.`,code:`package main
...
Once is safe to use as a struct field. The zero value is ready to use. Do not try to reset a Once by reassigning — there is no API for that and reassignment is racy. If you need re-runnable initialisation, use a different pattern.`,code:`package main
...
`,expected:"hello world"},takeaways:["http.HandleFunc binds a path to a handler; http.ListenAndServe blocks serving.","WriteHeader before any Write — the first Write defaults the status to 200.","Handlers run concurrently — shared state needs synchronisation.","http.Server with Shutdown(ctx) provides graceful in-flight-aware termination."],mistakes:["Calling WriteHeader after Write — the second call is ignored and a warning is logged.","Retaining the *http.Request past handler return — its body is closed by then.","Building a global mux for a large app — prefer an explicit *http.ServeMux you can pass around."],next:"json-encoding-decoding"},{id:"json-encoding-decoding",title:"Stdlib · JSON Encoding and Decoding",subtitle:"encoding/json maps between Go structs and JSON via struct tags and reflection.",intro:`encoding/json is the standard JSON codec. Marshal a Go value to JSON bytes with json.Marshal(v) — it returns ([]byte, error). Unmarshal bytes into a pointer to a Go value with json.Unmarshal(data, &v). For streaming, json.NewEncoder(w).Encode(v) writes to an io.Writer and json.NewDecoder(r).Decode(&v) reads from an io.Reader — both add or expect a trailing newline, which matters for line-delimited JSON.
...
Performance trade-offs are worth knowing. encoding/json uses reflection and is fast enough for most APIs (tens to hundreds of MB/s), but heavily benchmarked workloads sometimes adopt code-generated codecs like easyjson or sonic for 2-5x speedups. Profile before switching.`,code:`package main
...
`,expected:'{"title":"Go","pages":200}'},takeaways:["json.Marshal / Unmarshal convert between Go values and JSON; Encoder/Decoder stream them.",'Struct tags `json:"name,omitempty"` control field naming and zero-value omission.',"Only exported (uppercase) fields are serialised — unexported fields silently vanish.","Decoding numbers into interface{} produces float64; use json.Number for precise integers."],mistakes:["Forgetting to capitalise a struct field — encoding/json silently skips it.","Passing a value (not a pointer) to Unmarshal — compile error or wrong target.","Assuming float64 round-trips a 64-bit ID without loss — it does not past 2^53."],next:"testing-with-table-driven-tests"},{id:"testing-with-table-driven-tests",title:"Stdlib · Testing with Table-Driven Tests",subtitle:"The testing package plus go test plus the table-driven pattern — the standard Go rig.",intro:`Go's test framework lives in the standard library. Any file named *_test.go in the same package as the code under test is a test file; it does not ship in the production binary. A test function has the signature func TestX(t *testing.T) and lives at package level. Run go test from the package directory; go test ./... runs every test in the module.
...
For HTTP handlers, net/http/httptest provides a NewRecorder() to capture the response and a NewServer() to spin up a real socket-backed test server. For temp files and directories, t.TempDir() returns a path automatically cleaned up at test end.`,code:`package main
...
Today most of the modern backend stack — Express, Next.js, Vercel functions, GitHub's build pipeline, Slack's desktop app, VS Code itself — runs on Node.`,code:`// Save as hello.js, run with: node hello.js
const os = require('os');
...
Node updates V8 with every major release; new ES features land in Node a few months after Chrome.`,code:`// %FunctionName intrinsics are only available with --allow-natives-syntax
...
A practical workflow: when learning a new module, npm i it, then node, then const lib = require('libname'), then Object.keys(lib) and explore. Faster than reading docs. Be aware the top-level await in modern Node REPLs lets you write await fetch(...) directly without wrapping it in an async function.`,code:`// Start with: node
...
`,expected:"REPL ready"},takeaways:["node with no args opens the REPL; .exit or Ctrl+C twice leaves.","Dot-commands: .help, .load, .save, .editor, .clear.","The _ variable holds the last evaluated value.","Top-level await works in the modern REPL — useful for fetch and dynamic import."],mistakes:["Treating the REPL like a script — variables defined with let/const do not persist across .clear.","Forgetting that requiring the same module twice returns the cached instance.","Pasting large blocks line by line and missing closing braces — use .editor for multi-line."],next:{label:"CommonJS modules",href:"#/courses/node-basics/modules-cjs"}},{id:"modules-cjs",title:"Foundations · CommonJS modules",intro:`Before ES modules, Node used CommonJS — a synchronous, function-wrapped module format invented for server-side JavaScript. Every .js file (without "type": "module") is a CommonJS module. Each file is wrapped in an implicit function so its top-level variables stay private.
...
Today new projects should prefer ES modules, but you will see CommonJS everywhere: legacy packages, internal tooling, the entire pre-2020 npm ecosystem. Knowing both formats is non-negotiable.`,code:`// math.js
function add(a, b) { return a + b; }
...
For new projects, default to ESM. Use CommonJS only when a dependency forces it.`,code:`// math.mjs
...
`,expected:"esm"},takeaways:['ESM is opt-in via "type":"module" in package.json or the .mjs extension.',"Import specifiers must include the file extension — no auto-append.","Top-level await works in ESM and waits before the parent module proceeds.","ESM can import CommonJS as a default; the reverse needs dynamic import()."],mistakes:["Omitting the file extension on relative imports — Node throws ERR_MODULE_NOT_FOUND.","Trying to require() an ESM file from CommonJS — must use dynamic import() instead.","Mixing named and default exports without thinking — pick a convention per module."],next:{label:"npm and package.json",href:"#/courses/node-basics/npm-package-json"}},{id:"npm-package-json",title:"Foundations · npm and package.json",intro:`npm — Node Package Manager — is both a CLI and a public registry. package.json is the manifest that describes your project to npm, to bundlers, and to other developers. Running npm init -y creates one with sensible defaults; npm i <pkg> installs a dependency and records it.
...
Alternatives exist: pnpm uses a global content-addressable store with symlinks (fast, disk-efficient), Yarn pioneered modern lockfiles, Bun bundles its own runtime + package manager. For new projects, pnpm is the modern default; npm is the safe default if you're collaborating with the wider community.`,code:`// package.json (minimum useful shape)
const pkg = {
  name: 'my-app',
  version: '0.1.0',
  type: 'module',
...
When debugging a regression after npm update, check the diff in package-lock.json — the minor version bumps under your ^ ranges are the prime suspects.`,code:`function semverBucket(prev, next) {
...
Modern best practice: lockfile committed, pnpm or npm ci in CI, peerDependencies declared honestly, overrides used surgically when you need to pin a transitive dep across a security issue.`,code:`// Inspect your installed tree shape:
// $ npm ls react
...
A practical pattern: read configuration from process.env at module top, validate it once, and export typed constants. Crashing at startup with a clear error beats silently misbehaving in production.`,code:`console.log('argv:', process.argv);
...
A useful pattern: at process boundaries (HTTP handler, queue worker, CLI entry), wrap the whole operation in a single try/catch, log the error with context, and return a meaningful failure response. Inside the operation, let errors propagate — do not try/catch at every layer.`,code:`class ValidationError extends Error {
...
Modern Node code prefers promises and async/await, but every promise is built on a callback underneath, and you will work with callback APIs forever — fs.readFile, http.request, dns.lookup, every EventEmitter.`,code:`const fs = require('fs');
...
You will still see callback hell in legacy code. The path to clean it up: util.promisify a callback API into a promise-returning function, then await it. The fs/promises module already does this for the filesystem.`,code:`// The classic callback-hell shape (do not write code like this anymore):
function classic(cb) {
  setTimeout(() => {
    setTimeout(() => {
      setTimeout(() => {
...
Promises are integrated with the event loop's microtask queue, which means .then callbacks run before setTimeout(..., 0) but after the current synchronous code. We'll dig into that next.`,code:`function delay(ms, value) {
...
`,expected:"ok"},takeaways:["Three states: pending, fulfilled, rejected — settled state is immutable.","Each .then returns a new promise; throwing inside .then rejects the chain.","Combinators: Promise.all, allSettled, race, any — pick by the semantics you need.","Promise executor swallows sync throws as rejections — equivalent to reject(e)."],mistakes:["Forgetting to return the inner promise from a .then — chain detaches and races.","Mixing .then with await in the same function — pick one style per function.","Using Promise.all when you wanted allSettled — one rejection kills every result."],next:{label:"async / await",href:"#/courses/node-basics/async-await"}},{id:"async-await",title:"Async & I/O · async / await",intro:`async / await is syntactic sugar over promises. Mark a function async and it always returns a promise. Use await inside it to pause until a promise settles, then resume with the resolved value (or throw the rejected reason). Errors flow through try/catch like synchronous code.
...
await on a non-promise value resolves immediately with that value — useful when a function sometimes returns sync, sometimes async.`,code:`function delay(ms, value) {
...
`,expected:"hi"},takeaways:["async functions always return a promise; await pauses for one to settle.","Errors flow through try/catch — same shape as synchronous error handling.","Top-level await works in ESM; useful for module init.","await on a non-promise resolves immediately — works for sometimes-sync APIs."],mistakes:["await inside a for loop when items are independent — serial when it should be parallel.","Forgetting that await inside .map runs in parallel — map returns an array of promises.","Marking a function async but never awaiting inside it — pointless wrap."],next:{label:"Event loop",href:"#/courses/node-basics/event-loop"}},{id:"event-loop",title:"Async & I/O · The event loop",intro:`The Node event loop is the single thread that runs your JavaScript and processes callbacks for completed async work. Understanding its phases explains why some callbacks run "before" others even when they "should" be simultaneous.
...
The takeaway: don't block the loop. Any long synchronous operation (parsing a giant JSON file, a CPU-heavy compute) freezes every other connection. Offload to worker threads or break the work into chunks with setImmediate.`,code:`console.log('1 sync');
...
Knowing which queue a callback lands on is the difference between "this runs before the next render" and "this runs whenever it gets a chance".`,code:`console.log('A sync');
...
Streams are the right tool when memory matters or when you want to start work before all data arrives.`,code:`const { Readable } = require('stream');
...
The promise-based stream/promises pipeline() helper handles all this correctly. Use it.`,code:`const { Readable, Writable } = require('stream');
...
`,expected:"drain"},takeaways:["Backpressure signals when consumers cannot keep up so producers slow down.","Writable.write() returns false when the buffer is full; wait for the drain event.",".pipe() and stream/promises pipeline() handle backpressure automatically.","highWaterMark controls buffer size; tune it only when measurements demand it."],mistakes:["Writing in a tight loop without checking the .write() return — unbounded buffering.","Listening to data with .on(data, ...) and not pausing on slow consumers.","Using .pipe() and ignoring errors — pipeline() forwards them safely."],next:{label:"Worker threads",href:"#/courses/node-basics/worker-threads"}},{id:"worker-threads",title:"Async & I/O · Worker threads",intro:`Node is single-threaded for JavaScript, but the worker_threads module lets you spawn additional OS threads each running their own V8 isolate. They are the right answer for CPU-bound work — image encoding, video transcoding, pricing models, ML inference — that would otherwise block the main event loop.
...
A subtler use: worker_threads can also run untrusted code in isolation. Each worker has its own globals; a malicious script cannot reach the parent's process object. Combined with --experimental-permission, this gives you a small sandbox.`,code:`// Conceptual — workers must live in separate files normally.
...
The asymmetry: child processes are heavier than worker threads but more isolated and can run any binary, not just JS.`,code:`const { spawn } = require('child_process');
...
`,expected:"spawn"},takeaways:["child_process spawns OS processes (heavier than worker threads, fully isolated).","spawn for streaming I/O; exec/execFile for buffered output; fork for Node-to-Node IPC.","Prefer spawn/execFile with array args to exec with strings — avoids shell injection.","stdio config: pipe, inherit, ignore — mix per fd with an array."],mistakes:["Using exec on untrusted input — shell metacharacters become command injection.","Using exec for commands that produce gigabytes of output — buffers blow up memory.","Forgetting to handle the close event — child process becomes a zombie."],next:{label:"Signals",href:"#/courses/node-basics/signals"}},{id:"signals",title:"Async & I/O · Signals",intro:`Unix signals are the OS-level messages a process can receive: SIGINT when you press Ctrl+C, SIGTERM when systemd asks you to stop, SIGKILL when patience runs out, SIGHUP when the controlling terminal disconnects, SIGUSR1 and SIGUSR2 as user-defined channels. Node exposes them as events on the process object.
...
The graceful shutdown checklist: stop accepting work, wait for in-flight work, close external resources, exit. We will build this end-to-end in the next lesson.`,code:`process.on('SIGINT', () => {
...
Tools like the http-graceful-shutdown package package this up, but rolling your own is ~30 lines and worth knowing how it works.`,code:`const http = require('http');
...
`,expected:"graceful"},takeaways:["Catch SIGTERM, stop accepting work, drain in-flight, close resources, exit.","server.close() refuses new connections but waits for existing ones.","Always set a hard timeout (~10–30s) so a stuck connection cannot block exit.","Test shutdown locally — production at 2 AM is the wrong place to discover bugs."],mistakes:["Skipping the timeout — a hung connection holds shutdown forever.","Calling process.exit(0) before server.close() callback — drops in-flight requests.","Not propagating shutdown to cluster/worker children — only the master exits."],next:{label:"HTTP server",href:"#/courses/node-basics/http-server"}},{id:"http-server",title:"HTTP & APIs · Built-in HTTP server",intro:`Node's standard library ships a complete HTTP server in the http module. No frameworks needed — http.createServer((req, res) => {...}) gives you a functioning server in three lines. Frameworks build on top of this.
...
In production you usually put this server behind a reverse proxy (nginx, Cloudflare, an L7 LB) that handles TLS, caching, and rate limiting.`,code:`const http = require('http');
...
Alternatives worth knowing: Fastify is faster and has built-in schema validation; Koa is Express by the same author with a smaller surface and async-first design; Hono targets edge runtimes (Cloudflare Workers, Bun, Deno) with the same Express-ish ergonomics.`,code:`// const express = require('express');
// const app = express();
//
...
For complex routing (query-param-based dispatch, content negotiation, versioning), middleware is the cleaner mechanism. Versioning with /v1/... or via an Accept header are both common; choose one and stick to it across the API.`,code:`// app.get('/users', listUsers);
...
Per-route middleware is the cleanest way to enforce different policies for different endpoints: auth on /api/*, no auth on /public/*, admin role on /admin/*. Keep each middleware small and single-purpose — they compose better that way.`,code:`// const express = require('express');
// const app = express();
//
...
Schema validation (zod, joi, ajv) is usually a layer above body parsing: parse the raw body into JS, then validate the shape and types.`,code:`// const express = require('express');
// const app = express();
//
...
`,expected:"body"},takeaways:["express.json / urlencoded / text / raw handle the common Content-Type bodies.","Multipart form data (file uploads) needs multer or busboy — not built in.","Always set a limit on body size — DoS defense and memory safety.","Validate the parsed body with a schema (zod/joi/ajv) before using it."],mistakes:["Forgetting express.json() — req.body is undefined and you blame the client.","Leaving body size unlimited — 1 GB POSTs exhaust memory.","Trusting the parsed body without validation — type confusion bugs and NoSQL injection."],next:{label:"REST conventions",href:"#/courses/node-basics/rest-conventions"}},{id:"rest-conventions",title:"HTTP & APIs · REST conventions",intro:`REST is a convention for designing HTTP APIs around resources. Done well, it makes APIs predictable: a consumer who knows GET /users can guess GET /users/42, POST /users, DELETE /users/42. Done poorly, every endpoint is a one-off and the docs become essential.
...
GraphQL and tRPC sit alongside REST as alternatives; REST is still the default for public APIs and the path of least resistance.`,code:`const routes = [
...
`,expected:"rest"},takeaways:["Resources are nouns (plural), HTTP methods are verbs — predictability by convention.","Status codes carry semantics: 201 with Location for create, 4xx for client errors, 5xx for server.","GET / PUT / DELETE are idempotent; POST is not — retries should respect this.","Pagination, filtering, sorting in query strings; consistent JSON error shape."],mistakes:["Verbs in URLs (/createUser, /getUsers) — RPC-ish, breaks the REST mental model.","Returning 200 with { error: ... } for failures — clients cannot distinguish from success.","Mixing pagination styles (?page= vs ?cursor= vs Link header) across endpoints."],next:{label:"Error middleware",href:"#/courses/node-basics/error-middleware"}},{id:"error-middleware",title:"HTTP & APIs · Error middleware",intro:`Express distinguishes error middleware from regular middleware by arity: a function with four parameters (err, req, res, next) is treated as an error handler. The framework skips it during normal request flow and routes to it whenever a middleware or handler calls next(err) or throws (in async-aware mode).
...
A common omission: 404 handling. A route that no router matched falls through every middleware. Add a catch-all app.use((req, res) => res.status(404).json({error: 'not found'})) before the error handler.`,code:`// Standard error-middleware skeleton:
...
Security checklist: size limits, type allow-list, filename sanitization, virus scanning if user-generated, separate domain for serving uploaded content (avoids cookies leaking to attacker-controlled HTML).`,code:`// const multer = require('multer');
// const upload = multer({
//   dest: 'uploads/',
...
Heartbeats: connections die silently across NAT or load balancers. Send ping frames periodically and close on missed pongs to detect dead clients.`,code:`// const { WebSocketServer } = require('ws');
...
End-to-end tests (Playwright, Cypress) belong above unit tests. The pyramid: many unit tests, fewer integration tests, very few e2e tests.`,code:`// math.js
function add(a, b) { return a + b; }
module.exports = { add };
...
Indexes help WHERE perform on big tables: a B-tree index on country lets the database find country = 'US' rows without scanning everything. Functions on the indexed column (WHERE LOWER(email) = ...) usually defeat the index unless you also have a functional index.`,code:`-- Filter by equality, range, membership, and pattern
SELECT name, salary
FROM employees
WHERE country = 'US'
  AND salary BETWEEN 80000 AND 150000
  AND department_id IN (1, 3, 5)
  AND email LIKE '%@pgcode.com';`,exercise:{prompt:"Return the names of employees in country US.",starter:"-- write your SELECT here",expected:"SELECT name FROM employees WHERE country = 'US';"},takeaways:["WHERE evaluates a boolean per row; only TRUE rows pass — FALSE and UNKNOWN are dropped.","BETWEEN is inclusive, IN is set membership, LIKE matches patterns with % and _.","Use IS NULL / IS NOT NULL — never = NULL or != NULL.","Wrapping an indexed column in a function in WHERE usually defeats the index."],mistakes:['WHERE col != "X" silently drops NULL rows — add OR col IS NULL when you want them.',"Forgetting precedence — a OR b AND c is a OR (b AND c); parenthesise to be safe.","Using LIKE with a leading % — prevents index prefix lookup, forces a full scan."],next:"order-and-limit"},{id:"order-and-limit",title:"Foundations · ORDER BY and LIMIT",subtitle:"Sorting results deterministically and capping how many rows come back.",intro:`Rows in a relational table have no inherent order — the storage engine is free to return them in whatever order is convenient. If your query needs a specific order, you must ask for it with ORDER BY. The clause takes a comma-separated list of expressions; each can be followed by ASC (default) or DESC. Sorting by multiple columns is lexicographic — sort by the first, then break ties with the second, and so on.
...
SQL Server uses TOP instead of LIMIT (SELECT TOP 10 *); Oracle traditionally used ROWNUM but now supports FETCH FIRST 10 ROWS ONLY from the SQL:2008 standard.`,code:`-- Top 5 highest-paid US employees, then break ties by name
SELECT name, salary
FROM employees
WHERE country = 'US'
ORDER BY salary DESC, name ASC
LIMIT 5;`,exercise:{prompt:"Return the names of the 10 most-recently-hired employees, most recent first.",starter:"-- write your SELECT here",expected:"SELECT name FROM employees ORDER BY hired_at DESC LIMIT 10;"},takeaways:["Table rows have no inherent order — ORDER BY is the only guarantee.","ORDER BY a, b sorts lexicographically — by a, then b for ties.","LIMIT without ORDER BY is non-deterministic — pair them whenever reproducibility matters.","OFFSET is fine for shallow pages; use keyset pagination for deep ones."],mistakes:["Trusting the apparent order from a previous query — engines reorder freely between runs.","Paginating with huge OFFSETs and wondering why page 1000 is slow.","Forgetting that NULLs may sort first or last depending on dialect."],next:"distinct-and-aliases"},{id:"distinct-and-aliases",title:"Foundations · DISTINCT and Aliases",subtitle:"Deduplicating rows and renaming columns or tables for readability.",intro:`DISTINCT removes duplicate rows from a result. Written immediately after SELECT, it deduplicates based on the entire row — not just one column. SELECT DISTINCT country, role FROM employees returns every unique (country, role) pair, not every distinct country with one arbitrary role.
...
Bulk UPDATEs on big tables can block other writers, generate huge WAL/redo logs, and stall replication. Split them into batches of a few thousand rows when working in production.`,code:`-- Single-row update, expression update, and join update
UPDATE employees
SET salary = 95000
WHERE id = 42;
...
For periodic cleanups — old logs, expired sessions — prefer batched deletes (DELETE ... LIMIT 5000 in a loop) so you don't lock the table for minutes.`,code:`-- Single-row, conditional, and batched deletes
DELETE FROM employees WHERE id = 42;

DELETE FROM sessions WHERE expires_at < NOW();

-- Truncate the whole table fast (DDL, no per-row triggers)
TRUNCATE TABLE staging_imports;`,exercise:{prompt:"Delete every employee whose country is 'XX' (test data).",starter:"-- write your DELETE here",expected:"DELETE FROM employees WHERE country = 'XX';"},takeaways:["DELETE without WHERE empties the table row by row — usually catastrophic.","TRUNCATE is faster but skips triggers and may reset auto-increments.","Foreign keys decide what happens to dependents — pick the ON DELETE behaviour upfront.","Soft deletes ease restore but force every query to filter deleted_at IS NULL."],mistakes:["Forgetting that DELETE writes to the transaction log — multi-million-row deletes can fill it.","Issuing TRUNCATE thinking it can be rolled back the same way DELETE can.","Adding soft delete and then writing one query that forgets the deleted_at filter."],next:"null-semantics"},{id:"null-semantics",title:"Foundations · NULL Semantics",subtitle:"Three-valued logic, IS NULL vs =, and the COALESCE escape hatch.",intro:`NULL is SQL's marker for unknown or missing. It is not a value — it is the absence of a value. This single fact rewrites the rules of boolean logic. Anything compared to NULL is UNKNOWN, not TRUE or FALSE. NULL = NULL is UNKNOWN. NULL != NULL is UNKNOWN. NULL OR FALSE is UNKNOWN. NULL AND FALSE is FALSE (because AND with FALSE is always FALSE regardless of the other operand). This is three-valued logic — TRUE, FALSE, UNKNOWN — and it surprises everyone.
...
COALESCE(a, b, c) returns the first non-NULL argument. NULLIF(a, b) returns NULL if a = b, otherwise a — useful for turning sentinel values into NULLs. CASE WHEN col IS NULL THEN ... ELSE ... END is the verbose escape hatch.`,code:`-- Counting, defaulting, and filtering NULLs correctly
SELECT
  COUNT(*) AS total_rows,
  COUNT(email) AS rows_with_email,
  COALESCE(country, 'unknown') AS country_or_default
FROM employees
WHERE email IS NOT NULL;`,exercise:{prompt:"Return the names of employees whose email is NULL.",starter:"-- write your SELECT here",expected:"SELECT name FROM employees WHERE email IS NULL;"},takeaways:["NULL is unknown, not a value — comparison with = always yields UNKNOWN.","Use IS NULL / IS NOT NULL; never = NULL or != NULL.","COUNT(*) counts rows; COUNT(col) and aggregates skip NULL rows.","COALESCE(a, b, c) and NULLIF(a, b) are the everyday NULL utilities."],mistakes:["Filtering with col != X and being surprised NULLs vanished from the result.","Computing AVG on a column with NULLs and assuming the NULLs were treated as 0.","Joining on a nullable column with no IS NOT NULL guard and producing unwanted Cartesian sections."],next:"data-types-overview"},{id:"data-types-overview",title:"Foundations · Data Types Overview",subtitle:"Integers, decimals, strings, dates, booleans, JSON — picking the right one.",intro:`Choosing the right data type pays compounding dividends — storage, index size, query speed, and bug class all depend on it. The major categories every database supports are integers, floating-point, exact decimal, strings, dates and timestamps, booleans, binary blobs, and increasingly JSON.
...
JSON / JSONB: Postgres JSONB is binary-encoded and indexable; JSON is text. Use JSONB for searchable nested data; promote frequently queried keys to real columns once the shape stabilises.`,code:`-- Picking types deliberately
...
RIGHT JOIN is the mirror image but rarely used; it is clearer to swap the operand order and use LEFT JOIN.`,code:`-- Every employee plus their department; unassigned employees still appear
SELECT e.name AS employee, d.name AS department
FROM employees e
LEFT JOIN departments d ON e.department_id = d.id
ORDER BY e.name;`,exercise:{prompt:"Return every employee's name and their department name; employees without a department should still appear with NULL.",starter:"-- write your LEFT JOIN here",expected:"SELECT e.name, d.name FROM employees e LEFT JOIN departments d ON e.department_id = d.id;"},takeaways:["LEFT JOIN keeps every left-side row; right-side columns are NULL when unmatched.","Anti-join idiom: LEFT JOIN ... WHERE right.id IS NULL — rows with no match.","Filtering the right table in WHERE silently converts LEFT JOIN to INNER JOIN.","A join with multiple matches multiplies the row count — even a LEFT JOIN."],mistakes:["Putting r.col = X in WHERE instead of ON and losing the unmatched left rows.","Assuming a LEFT JOIN cannot increase the row count — it can, if the right side has duplicates.","Using RIGHT JOIN when swapping the operands and writing LEFT JOIN would be clearer."],next:"right-join"},{id:"right-join",title:"Joins & Aggregation · RIGHT JOIN",subtitle:"The mirror image of LEFT JOIN — useful occasionally, swap-able always.",intro:`RIGHT JOIN (RIGHT OUTER JOIN) is LEFT JOIN's mirror image. It keeps every row from the right table and pads with NULLs when the left has no match. Semantically, a RIGHT b is identical to b LEFT a — so RIGHT JOIN is mostly redundant. Most style guides discourage it because reading left-to-right matches reading a LEFT JOIN.
...
In legacy codebases or generated SQL, you will see RIGHT JOIN occasionally. When refactoring, you can always rewrite a RIGHT b ON p as b LEFT a ON p and swap the SELECT column references — the result is the same.`,code:`-- Every department plus its employees; departments with no employees still appear
...
ORDER BY d.name, e.name;`,exercise:{prompt:"Return every department name with the names of its employees, including departments with no employees. Use RIGHT JOIN.",starter:"-- write your RIGHT JOIN here",expected:"SELECT d.name, e.name FROM employees e RIGHT JOIN departments d ON e.department_id = d.id;"},takeaways:["RIGHT JOIN is LEFT JOIN with the operands swapped — semantically equivalent.","Most style guides prefer LEFT JOIN for left-to-right readability.","All LEFT JOIN gotchas apply symmetrically — beware WHERE filters on the left side.","Query planners normalise RIGHT and LEFT identically; performance is the same."],mistakes:["Writing RIGHT JOIN out of habit when LEFT JOIN with swapped operands reads better.","Filtering the left table in WHERE and silently dropping the unmatched right rows.","Mixing RIGHT and LEFT in the same multi-table query — hard to reason about."],next:"full-outer-join"},{id:"full-outer-join",title:"Joins & Aggregation · FULL OUTER JOIN",subtitle:"Keep every row from both sides; NULL out wherever a match is missing.",intro:`FULL OUTER JOIN keeps every row from both tables. Where a match exists, the row contains values from both sides; where it does not, the missing side is NULL. It is the union of LEFT and RIGHT outer joins minus the double-counted matches.
...
A practical use case is data migration verification: FULL OUTER JOIN the old table to the new table by id, count rows where either side is NULL, and you have a one-line audit of what migrated cleanly.`,code:`-- Reconcile employees and badge_assignments — find mismatches on either side
SELECT e.id AS employee_id, b.id AS badge_id
FROM employees e
FULL OUTER JOIN badge_assignments b ON e.id = b.employee_id
WHERE e.id IS NULL OR b.id IS NULL;`,exercise:{prompt:"Combine every row from employees and contractors on shared email, keeping unmatched rows from both sides.",starter:"-- write your FULL OUTER JOIN here",expected:"SELECT e.name, c.name FROM employees e FULL OUTER JOIN contractors c ON e.email = c.email;"},takeaways:["FULL OUTER JOIN keeps every row from both sides; missing matches become NULL.","Perfect for reconciliations — find rows in exactly one table with IS NULL filters.","MySQL lacks native FULL OUTER JOIN; simulate with UNION ALL of LEFT and anti-LEFT.","Use COALESCE to fold matching-side ids into a single column."],mistakes:["Assuming FULL OUTER JOIN exists everywhere — check the dialect first.","Forgetting that aggregates must skip the NULLs that FULL OUTER JOIN introduces.","Using FULL OUTER JOIN when LEFT JOIN + WHERE IS NULL would answer the same question cheaper."],next:"cross-join"},{id:"cross-join",title:"Joins & Aggregation · CROSS JOIN",subtitle:"The Cartesian product — every row paired with every other row.",intro:`CROSS JOIN is the unfiltered Cartesian product: every row in a paired with every row in b. The result has |a| * |b| rows. Shape: FROM a CROSS JOIN b. There is no ON clause — by definition, every pair is in the result.
...
Aliases are mandatory in self joins because otherwise every column reference (employees.id) is ambiguous. Pick short, evocative aliases — e for employee, m for manager, child / parent for tree edges — and stick to them across the codebase.`,code:`-- Each employee with their manager's name; top-level employees still appear
SELECT e.name AS employee, m.name AS manager
FROM employees e
LEFT JOIN employees m ON e.manager_id = m.id
ORDER BY m.name NULLS FIRST, e.name;`,exercise:{prompt:"Return each employee's name paired with their manager's name.",starter:"-- write your self join here",expected:"SELECT e.name, m.name FROM employees e JOIN employees m ON e.manager_id = m.id;"},takeaways:["A self join uses the same table twice with different aliases — common for hierarchies.","Use a.id < b.id to avoid duplicate pair rows and trivial self-pairs.","LEFT self join preserves top-of-tree rows with no parent.","For arbitrary-depth tree walks, switch from chained self joins to recursive CTEs."],mistakes:["Forgetting aliases — every column is ambiguous and the query fails to parse.","Using INNER JOIN where LEFT JOIN was intended — losing the root or top-level rows.","Producing duplicate pair rows by not applying a < or > tie-breaker."],next:"group-by-basics"},{id:"group-by-basics",title:"Joins & Aggregation · GROUP BY Basics",subtitle:"Collapsing many rows into one per group — the foundation of every report.",intro:`GROUP BY collapses many rows into one row per distinct value of the grouping columns. The SELECT list can then either reference a grouping column directly or apply an aggregate (COUNT, SUM, AVG, MAX, MIN, etc.) to non-grouping columns. Shape: SELECT col_a, SUM(col_b) FROM t GROUP BY col_a.
...
GROUP BY ROLLUP and GROUP BY CUBE generate subtotals and cross-totals for free — useful for pivot reports without writing a UNION of multiple queries.`,code:`-- Employee count and average salary per country
SELECT
  country,
  COUNT(*) AS headcount,
...
You can mix WHERE and HAVING freely: WHERE for row-level filters, HAVING for group-level filters. Both can use boolean combinators (AND, OR, NOT) and any predicate type — including IN, BETWEEN, LIKE on the aggregates themselves: HAVING COUNT(*) BETWEEN 5 AND 50.`,code:`-- Departments with more than 10 employees whose average salary exceeds 90k
SELECT
  department_id,
  COUNT(*) AS headcount,
  AVG(salary) AS avg_salary
...
FILTER (Postgres, SQL standard) lets you compute conditional aggregates inline: COUNT(*) FILTER (WHERE country = 'US') AS us_count, COUNT(*) FILTER (WHERE country = 'UK') AS uk_count — much cleaner than CASE WHEN tricks.`,code:`-- Headcount, total salary, distinct countries, and longest name per department
SELECT
  department_id,
  COUNT(*) AS headcount,
...
Window functions run after GROUP BY in the logical evaluation order — they see the already-aggregated rows. To filter on a window function's output, wrap the query in a subquery or CTE and filter in the outer SELECT.`,code:`-- Each employee plus their rank inside their department and the department average
SELECT
  name,
  department_id,
  salary,
...
The PK index is what makes lookups by id constant-time. Every other index is built on top of the PK ordering, so picking a good PK affects every secondary index's size.`,code:`-- A surrogate PK and a composite PK in one schema
CREATE TABLE users (
  id BIGSERIAL PRIMARY KEY,
...
DEFERRABLE INITIALLY DEFERRED (Postgres) postpones the constraint check until commit — useful for circular references and bulk loads.`,code:`-- FK with cascade, plus an explicit index on the FK column
CREATE TABLE orders (
  id BIGSERIAL PRIMARY KEY,
...
Expression indexes index the result of a function: CREATE INDEX idx_users_email_lower ON users(LOWER(email)) — now WHERE LOWER(email) = X uses the index.`,code:`-- Single-column index, partial index, and expression index
...
When unsure, build the index, test the query, then EXPLAIN. Reading the plan is faster than guessing.`,code:`-- Composite index for queries that filter user_id and (sometimes) status
...
Read EXPLAIN: Index Only Scan = the heap was skipped; Heap Fetches: N tells you how many tuples still needed a heap check. A high Heap Fetches count means the visibility map is stale or the index does not actually cover.`,code:`-- Covering index for a frequent dashboard query
...
Tooling: explain.depesz.com, explain.dalibo.com visualise Postgres EXPLAIN output, highlighting expensive nodes. Use them — text plans are hard to read past 20 lines.`,code:`-- Inspect estimated plan
EXPLAIN
...
SELECT id, total_cents
FROM orders
WHERE user_id = 42 AND status = 'open';`,exercise:{prompt:"Use EXPLAIN to show the plan for selecting all employees where country = 'US'.",starter:"-- write your EXPLAIN here",expected:"EXPLAIN SELECT * FROM employees WHERE country = 'US';"},takeaways:["EXPLAIN shows the planner-picked tree; EXPLAIN ANALYZE adds actual runtimes.","Big divergence between estimated and actual rows is the smoking gun for bad plans.","Seq Scan on a large table with a selective WHERE means a missing index.","explain.depesz.com / explain.dalibo.com make long plans actually readable."],mistakes:["Running EXPLAIN without ANALYZE and missing the actual-vs-estimated gap.","Optimising plans by guessing instead of reading the node-by-node row counts.","Forgetting that EXPLAIN ANALYZE actually runs the query — never on a destructive UPDATE."],next:"normalization-1nf-2nf-3nf"},{id:"normalization-1nf-2nf-3nf",title:"Performance & Schema · Normalization 1NF / 2NF / 3NF",subtitle:"Codd-era forms that eliminate redundancy and update anomalies.",intro:`Normalization is the systematic process of decomposing tables to eliminate redundancy and update anomalies. Codd's original three normal forms cover the cases most modern apps care about. Each form builds on the previous one.
...
Beyond 3NF: BCNF, 4NF, 5NF handle exotic cases most apps never encounter. Reach for them when teaching, not when shipping.`,code:`-- Normalized: employees reference departments via FK
...
fn main() is the entry point of a binary crate. Library crates have no main — they expose functions, structs, and traits other crates use. The println! macro prints to stdout with a newline; the trailing ! marks it as a macro, not a function.`,code:`fn main() {
    println!("Hello, Rust");
}
...
`,expected:"Hello, Rust"},takeaways:["cargo is the daily driver — rustc rarely shows up directly in your workflow.","Debug builds compile fast but run slow; --release flips that trade-off.","cargo check skips codegen and is the fastest way to validate the borrow checker.","println! is a macro, not a function — the trailing ! is the giveaway."],mistakes:["Benchmarking debug builds — they are 10-100x slower than release. Always cargo build --release.","Editing target/ files or committing them — that directory is generated; add it to .gitignore.",'Forgetting the trailing ! on println — println("...") fails with cannot find function println.'],next:"variables-and-mutability"},{id:"variables-and-mutability",title:"Foundations · Variables and Mutability",subtitle:"let, mut, shadowing, and the const vs static distinction.",intro:`Variables in Rust are immutable by default. let x = 5; binds x to 5 and forbids assignment. x = 6; fails to compile with cannot assign twice to immutable variable. Add mut to opt in: let mut x = 5; x = 6; works. This default-immutable stance pushes you toward designs where state changes are explicit and localised — when you read code and see no mut, you know the variable's value cannot change after binding.
...
Variables live until the end of their scope, which is when Rust runs Drop on them. This deterministic destruction (no GC pauses) is the whole foundation of the ownership model that follows in later lessons.`,code:`fn main() {
    let x = 5;
    let mut y = 10;
    y += 1;
...
`,expected:"3"},takeaways:["let bindings are immutable; opt in to mutation with let mut.","Shadowing rebinds a name — new type allowed; mutation requires the same type.","Integer literals default to i32, floats to f64 — annotate to override.","const inlines a compile-time value; static has a fixed address for the whole program."],mistakes:["Reaching for mut to mute the compiler — usually a sign the design should be functional instead.","Confusing shadowing with mutation — they have different scoping and type rules.","Using static mut without a Mutex/lock — it is unsafe to touch from multiple threads."],next:"primitive-types"},{id:"primitive-types",title:"Foundations · Primitive Types",subtitle:"Integers, floats, bool, char, tuples, arrays — what is built in before any std types.",intro:`Rust's primitive types are deliberately concrete — no implicit number conversions, no surprise truncation. Integer types come in signed (i8, i16, i32, i64, i128, isize) and unsigned (u8 through u128, usize) flavours. The number is the bit width; isize and usize match the pointer size of the target (32 or 64 bits). isize/usize is the type of array indices, lengths, and pointer offsets — that is the only time you should pick it deliberately. For everything else, prefer i32 for general numbers, u32 for non-negative counts, i64 when 2 billion is not enough, and u8 for byte buffers.
...
Arithmetic overflow panics in debug builds and wraps in release builds. If you need explicit semantics, use the wrapping_add, checked_add, saturating_add, overflowing_add families on every integer type.`,code:`fn main() {
...
A function that always diverges (panic!, loop {}, return) has the ! never type, which coerces to anything — convenient for one-arm matches.`,code:`fn main() {
    let n = 5;
...
Naming follows snake_case for functions and variables, PascalCase for types and traits, SCREAMING_SNAKE_CASE for constants. The compiler warns when you violate these — taking the warnings seriously keeps your code consistent with std and crates.io.`,code:`fn add(a: i32, b: i32) -> i32 {
    a + b
}
...
#[derive(Debug, Clone)] auto-generates implementations of common traits. Derive Debug to get {:?} printing during development; derive Clone if you need an explicit deep copy method. Derive PartialEq when you want struct equality.`,code:`#[derive(Debug)]
struct User {
    id: u64,
    email: String,
    active: bool,
...
if let Variant(x) = value { ... } is sugar for the one-arm match — concise when you only care about one variant. It pairs with else for the fall-through case.`,code:`enum Status {
    Active,
    Pending(u32),
...
panic! is for invariants you believe cannot fail. unreachable!() is panic with a "this branch should never run" message. todo!() is a placeholder during development that compiles but panics if hit.`,code:`use std::num::ParseIntError;
...
Capacity matters for performance. v.shrink_to_fit() drops the unused capacity. v.clear() empties without freeing — useful for reusing a buffer.`,code:`fn sum_above(slice: &[i32], threshold: i32) -> i32 {
...
`,expected:"15"},takeaways:["Vec<T> is the growable, heap-allocated array — capacity grows by doubling on push.","Slice &[T] is a borrowed view — pointer + length, no ownership.","Prefer &[T] over &Vec<T> in function parameters — accepts arrays and sub-slices too.","Vec::with_capacity(n) skips the reallocation chain when you know the size."],mistakes:["Indexing v[i] with an out-of-range i — panic; use v.get(i) for safe access.","Cloning a slice into a Vec just to pass to a function that wanted &[T] — wasted allocation.","Mutating a vector while iterating it — borrow checker prevents the obvious cases; logic bugs in the rest."],next:"strings-and-string-views"},{id:"strings-and-string-views",title:"Foundations · Strings and String Views",subtitle:"String, &str, why UTF-8 makes indexing illegal, and how to do it anyway.",intro:`Rust has two main string types. String is an owned, heap-allocated, growable UTF-8 buffer — analogous to Vec<u8> with the invariant that the bytes form valid UTF-8. &str (string slice) is a borrowed view into UTF-8 bytes — pointer plus length, no ownership. String literals in source code ("hello") have type &'static str — they live in the binary's read-only data segment.
...
When you really need a raw byte buffer (network protocols, binary files), use Vec<u8>. The functions that bridge are .as_bytes() (slice to &[u8]) and str::from_utf8 (slice from &[u8], returns Result because not every byte sequence is valid UTF-8).`,code:`fn main() {
...
The Rule against multiple mutable borrows can feel restrictive at first. The escape hatches are interior mutability types — RefCell for single-threaded, Mutex / RwLock for multi-threaded — covered later when we hit smart pointers and threading.`,code:`fn change(s: &mut String) {
    s.push_str(" world");
}
...
Common newcomer trap: trying to return a reference to a local. fn build() -> &String { let s = String::from("..."); &s } cannot compile because s dies at function end. Return the owned value (String) instead. Lifetimes do not magically extend the value they reference.`,code:`fn longest<'a>(x: &'a str, y: &'a str) -> &'a str {
...
Derive macros (#[derive(Debug, Clone, PartialEq)]) generate trait implementations from the struct's field types. Use them aggressively for standard traits; only hand-implement when you need custom semantics.`,code:`trait Shape {
    fn area(&self) -> f64;
...
`,expected:"Kitty"},takeaways:["Traits define shared behaviour; impl Trait for Type wires up an implementation.","Default method bodies let one implementation cover most cases automatically.","Static dispatch (generics) monomorphises; dynamic dispatch (dyn Trait) uses a vtable.","Orphan rule: own the trait or own the type — newtype wrapper escapes the rest."],mistakes:["Trying to impl an external trait for an external type — orphan rule rejects it.","Reaching for dyn Trait by reflex — generics are usually cheaper and just as flexible.","Forgetting to import a trait — methods are not in scope until you use trait::Name."],next:"generics"},{id:"generics",title:"Ownership & Traits · Generics",subtitle:"Type parameters on functions and structs, monomorphisation, and bound design.",intro:`Generics let you write code that works for many types without repeating yourself. The syntax mirrors traits: angle brackets after the function or type name. fn largest<T: PartialOrd>(items: &[T]) -> &T returns a reference to the largest item in any slice whose elements can be compared with >. Structs work the same: struct Pair<A, B> { first: A, second: B } holds two values of arbitrary types.
...
Generic types are also the foundation for traits with associated types (later) and for higher-kinded patterns like Iterator<Item = T>.`,code:`fn largest<T: PartialOrd + Copy>(items: &[T]) -> T {
...
Closing the channel is automatic — drop every Sender clone and the Receiver loop ends. Drop the Receiver while Senders still exist and subsequent send() calls return Err. The type system surfaces these failures so they are hard to ignore.`,code:`use std::sync::mpsc;
use std::thread;
...
Async is a learning curve, but the payoff is exceptional throughput for network-heavy services. Start with the official tokio tutorial once the synchronous fundamentals from this course are solid.`,code:`// Compiles with tokio = { version = "1", features = ["full"] } in Cargo.toml.
//
// #[tokio::main]
// async fn main() {
//     let (a, b) = tokio::join!(
...
// log "minor" or "adult"
`,expected:"minor"},takeaways:["Always use === / !== — never ==.","Ternaries (cond ? a : b) are great for short branches.",'Falsy values: false, 0, "", null, undefined, NaN — everything else is truthy.',"?? gives a default only for null/undefined; || treats all falsy as missing."],mistakes:["`if (x = 5)` is an assignment, not comparison — always truthy.","Assuming 0 || x returns 0 — it does not, because 0 is falsy.","Switch fall-through without a break statement (sometimes intended, often a bug)."],next:{label:"Loops & arrays",href:"#/courses/javascript-basics/j3"}},{id:"j3",title:"3. Loops & arrays",intro:"Use `for (let i = 0; i < arr.length; i++)` for index-based loops, `for (const x of arr)` for value iteration. Arrays have `.map`, `.filter`, `.reduce`, `.forEach` — most algorithms become one-liners.",code:`const nums = [1, 2, 3, 4, 5];
...
`,expected:"3"},takeaways:["Map keys can be any type, including objects and functions.","Map preserves insertion order during iteration.","map.get / set / has / delete are all O(1) amortized.","Prefer Map over a plain object when keys are dynamic or non-string."],mistakes:["Using `{}` as a hash map and tripping over inherited keys like __proto__.","Calling map[key] — that does not work; use map.get(key).","Forgetting that map.size is a property, not map.length or map.size()."],next:{label:"Strings & templates",href:"#/courses/javascript-basics/j7"}},{id:"j7",title:"7. Strings & template literals",intro:"Strings are primitives but have methods via auto-boxing. Slice with `slice(a, b)`, find with `indexOf` / `includes`, split with `split(sep)`, join with `arr.join(sep)`. Template literals `` `Hi ${name}` `` are the modern way to interpolate. Strings are immutable — every method returns a new string.",code:`const s = "the quick brown fox";
...
`,expected:"bad json"},takeaways:["Always throw Error or a subclass — get a stack trace.","finally runs no matter what — perfect for cleanup.","await inside try/catch handles rejected promises naturally.","Custom Error subclasses (class NotFoundError extends Error) clarify intent."],mistakes:["Throwing strings or numbers — no stack, brittle for catch blocks.","Empty catch blocks swallow bugs — log or rethrow.","Forgetting that errors in setTimeout callbacks bypass surrounding try/catch."],next:{label:"ES modules",href:"#/courses/javascript-basics/j17"}},{id:"j17",title:"17. ES modules",intro:'ESM is the standard module system. `export default x` exports one value imported as any name; `export const foo` is a named export imported via `import { foo }`. `import "./styles.css"` is a side-effect import that runs the module for its effects only. Because imports are static, bundlers can tree-shake — drop named exports nothing imports.',code:`// math.js
// export const add = (a, b) => a + b;
// export default (a, b) => a * b;
//
// main.js
...
`,expected:"5 6"},takeaways:["ESM is the standard: import / export, statically analysed.","Default export: one per module; named exports: many, by name.","Side-effect imports run a module without binding any name.","Tree-shaking drops unused named exports at build time."],mistakes:["Mixing ESM and CommonJS in the same file — bundlers tolerate it, runtimes may not.","Importing a default export by destructuring: import { default as x } is verbose; just import x.","Forgetting the .js extension in browser ESM — required."],next:{label:"TypeScript types & inference",href:"#/courses/typescript-basics/ts1"}}]},"java-basics":{id:"java-basics",title:"Java Basics",language:"java",color:"#E76F00",blurb:"The Java you need for FAANG interview solutions: types, control flow, methods, collections.",estimatedHours:2,externalResources:[{title:"dev.java — Java tutorial (Oracle)",url:"https://dev.java/learn/",type:"course"},{title:"Stanford CS106A Programming Methodology",url:"https://see.stanford.edu/Course/CS106A",type:"course"},{title:"Baeldung — Java articles index",url:"https://www.baeldung.com/java-tutorial",type:"blog"}],lessons:[{id:"jv1",title:"1. Hello + types",intro:"Every Java program lives in a class. `main` is the entry point. The primitive types are `int`, `long`, `double`, `boolean`, `char`. `String` is an object. Variables must be declared with a type.",code:`public class Main {
...
`,expected:"30"},takeaways:["Method signature: returnType name(paramType param, ...) — always declared explicitly.","Arrays are fixed-size and length is a field, not a method.","Enhanced-for (for (int x : arr)) walks values without an index.","Arrays.toString / Arrays.deepToString print arrays sensibly."],mistakes:["Calling arr.length() — it is arr.length (no parens).","ArrayIndexOutOfBoundsException from off-by-one in loops.","Returning a primitive from a method declared to return its wrapper — auto-unboxing surprises."],next:{label:"HashMap",href:"#/courses/java-basics/jv4"}},{id:"jv4",title:"4. HashMap",intro:"`HashMap<K, V>` is Java's hash table. `put`, `get`, `containsKey`. `getOrDefault(key, default)` is your friend for counting. Don't forget the import.",code:`import java.util.HashMap;
...
`,expected:"5"},takeaways:["Streams are lazy: nothing runs until a terminal op (collect/count/sum/reduce).","Use IntStream / LongStream / DoubleStream for primitive perf.","collect with Collectors.toList / toMap / groupingBy is the typical sink.","Stream pipelines are one-shot — they cannot be reused."],mistakes:["Reusing a stream after a terminal op — IllegalStateException.","Calling .parallel() on tiny work — overhead dominates.","Mixing stateful side-effects with stream ops — order is not guaranteed."],next:{label:"Optional",href:"#/courses/java-basics/jv9"}},{id:"jv9",title:"9. Optional",intro:"`Optional<T>` is the type-safe alternative to returning null. Build with `Optional.of(x)`, `Optional.empty()`, `Optional.ofNullable(maybe)`. Consume with `.isPresent()`, `.ifPresent(c)`, `.orElse(default)`, `.map(fn)`, `.flatMap(fn)`. Never call `.get()` without first checking presence — it throws if empty.",code:`import java.util.*;
public class Main {
...
`,expected:"[3, 2, 1]"},takeaways:["Generics give compile-time type safety with zero runtime cost (erasure).","<T extends Comparable<T>> constrains T to types you can compare.","PECS: Producer Extends, Consumer Super for wildcard usage.","You cannot create T[] or `instanceof T` due to type erasure."],mistakes:["Raw types (List instead of List<T>) — defeats type safety.","Mixing <? extends T> and writes — wildcards are read-only producers.","Forgetting that arrays are covariant but generics are invariant."],next:{label:"Lambda expressions",href:"#/courses/java-basics/jv12"}},{id:"jv12",title:"12. Lambda expressions",intro:"Lambdas are concise instances of functional interfaces — interfaces with one abstract method. Syntax: `(params) -> expr` or `(params) -> { ...; return v; }`. The compiler matches them to `Runnable`, `Comparator<T>`, `Function<T, R>`, `Predicate<T>`, `Consumer<T>`, `Supplier<T>`, etc. Method references (`String::length`, `System.out::println`) are even tighter. Lambdas capture effectively-final variables; you cannot reassign captured locals.",code:`import java.util.*;
...
`,expected:"385"},takeaways:[".parallel() splits work across the common ForkJoinPool.","Operations should be stateless, side-effect-free, and associative.","Use reduce(identity, op) for safe combination.","Parallelism pays off only beyond a few thousand elements of CPU work."],mistakes:["Mutating shared state inside a parallel stream — race conditions.","Using forEach when ordered output matters — use forEachOrdered.","Calling .parallel() on small data — overhead beats any speed-up."],next:{label:"Algorithms: Sorting",href:"#/learn/algorithms/heap-sort"}}]},"react-basics":{id:"react-basics",title:"React Basics",language:"javascript",color:"#61dafb",blurb:"Components, JSX, props vs state, events, controlled forms, lists & keys. Five lessons to get you productive.",estimatedHours:1.5,externalResources:[{title:"react.dev — Learn React",url:"https://react.dev/learn",type:"course"},{title:"Kent C. Dodds blog",url:"https://kentcdodds.com/blog",type:"blog"},{title:"Epic React (Kent C. Dodds, free intro)",url:"https://www.epicreact.dev/",type:"course"}],lessons:t},"typescript-basics":{id:"typescript-basics",title:"TypeScript Basics",language:"typescript",color:"#3178c6",blurb:"Types, interfaces, unions, generics — the typed JS subset that catches half your bugs at compile time.",estimatedHours:2,externalResources:[{title:"TypeScript Handbook",url:"https://www.typescriptlang.org/docs/handbook/intro.html",type:"book"},{title:"type-challenges",url:"https://github.com/type-challenges/type-challenges",type:"repo"},{title:"Total TypeScript (Matt Pocock) — free essentials",url:"https://www.totaltypescript.com/tutorials/typescript-essentials",type:"course"}],lessons:n},"go-basics":{id:"go-basics",title:"Go",language:"go",color:"#00add8",blurb:"Concurrency-first language for servers and tooling. 20 lessons across Foundations and Concurrency & Stdlib.",estimatedHours:4,externalResources:[{title:"A Tour of Go",url:"https://go.dev/tour/welcome/1",type:"course"},{title:"Go by Example",url:"https://gobyexample.com/",type:"blog"},{title:"Effective Go",url:"https://go.dev/doc/effective_go",type:"book"},{title:"The Go Programming Language (Donovan, Kernighan)",url:"https://www.gopl.io/",type:"book"}],lessons:a},"node-basics":{id:"node-basics",title:"Node.js Basics",language:"javascript",color:"#3c873a",blurb:"Node from V8 to production HTTP servers. 32 lessons across Foundations, Async & I/O, and HTTP & APIs.",estimatedHours:6,externalResources:[{title:"Node.js official docs",url:"https://nodejs.org/api/",type:"book"},{title:"Node.js Design Patterns (Casciaro, Mammino)",url:"https://www.nodejsdesignpatterns.com/",type:"book"},{title:"libuv design overview",url:"https://docs.libuv.org/en/v1.x/design.html",type:"blog"},{title:"Express docs",url:"https://expressjs.com/",type:"book"}],lessons:s},"sql-basics":{id:"sql-basics",title:"SQL Basics",language:"sql",color:"#0075a8",blurb:"From SELECT to query plans. 30 lessons across Foundations, Joins & Aggregation, and Performance & Schema.",estimatedHours:5,externalResources:[{title:"Use The Index, Luke!",url:"https://use-the-index-luke.com/",type:"book"},{title:"PostgreSQL official docs",url:"https://www.postgresql.org/docs/current/",type:"book"},{title:"Mode Analytics — SQL tutorial",url:"https://mode.com/sql-tutorial/",type:"course"},{title:"Markus Winand — Modern SQL",url:"https://modern-sql.com/",type:"blog"}],lessons:r},"cpp-basics":{id:"cpp-basics",title:"C++ Basics",language:"cpp",color:"#00599C",blurb:"Modern C++17 essentials: types, control flow, vectors, unordered_map.",estimatedHours:2,externalResources:[{title:"learncpp.com — Modern C++ tutorial",url:"https://www.learncpp.com/",type:"book"},{title:"cppreference",url:"https://en.cppreference.com/w/",type:"book"},{title:"The Cherno — Modern C++ YT series",url:"https://www.youtube.com/c/TheChernoProject",type:"video"}],lessons:[{id:"cpp1",title:"1. Hello + types",intro:"Every C++ program needs `main()`. Use `#include <iostream>` for I/O and `using namespace std;` to skip prefixes. Common types: `int`, `long long`, `double`, `bool`, `string` (from `<string>`).",code:`#include <iostream>
#include <string>
using namespace std;
int main() {
  int x = 7;
...
`,expected:"minor"},takeaways:["if / else if / else; switch needs break unless using [[fallthrough]].","== works for std::string (member operator); &&, ||, ! for logical ops.","Ternary cond ? a : b is an expression, useful in initializations.","C++17 `if (auto x = expr; cond)` scopes x to the if/else block."],mistakes:["Comparing C-style char* with == — compares pointers; use strcmp or std::string.","Switch fall-through without [[fallthrough]] — warning, sometimes a bug.","Assignment vs equality: if (x = 5) compiles, always true — enable -Wall."],next:{label:"Vectors + loops",href:"#/courses/cpp-basics/cpp3"}},{id:"cpp3",title:"3. Vectors + loops",intro:"`std::vector<T>` is the go-to dynamic array. `push_back`, `size()`, index with `[]`. Range-based for: `for (int x : v)`. Algorithm header has sort, etc.",code:`#include <iostream>
#include <vector>
#include <algorithm>
using namespace std;
int main() {
...
`,expected:"3"},takeaways:["unordered_map is hashed, O(1) average; map is a red-black tree, O(log n) and sorted.","m[k] default-constructs missing keys — convenient for counters.","Use m.find(k) when you must distinguish missing from zero.","Iteration order of unordered_map is unspecified."],mistakes:["Calling m[k] in a const context — does not compile (modifies the map).","Storing pointers into the map and relying on them after rehash — invalidated.","Using a non-hashable struct as key without a std::hash specialization."],next:{label:"Strings",href:"#/courses/cpp-basics/cpp5"}},{id:"cpp5",title:"5. Strings",intro:"`std::string` (from `<string>`) is mutable, with `+`/`+=` concatenation, `.length()`, `.substr(pos, n)`, indexing via `s[i]`. `std::getline(cin, s)` reads a whole line. For splits or fancy ops, you typically reach for `<sstream>` (`std::stringstream`).",code:`#include <iostream>
#include <sstream>
#include <string>
using namespace std;
int main() {
...
`,expected:"11"},takeaways:["std::string owns its data and grows automatically.",".size() and .length() are equivalent; both O(1).","std::stringstream is the idiomatic way to split or parse formatted text.","string_view (C++17) is a non-owning view — cheaper for read-only params."],mistakes:["Mixing C-string functions (strlen, strcpy) with std::string — convert with .c_str().","Returning a string_view of a local — dangling reference.","Building strings with += inside a hot loop — fine for small N, costly for big N (reserve!)."],next:{label:"Sort & algorithms",href:"#/courses/cpp-basics/cpp6"}},{id:"cpp6",title:"6. Sort & algorithms",intro:"`<algorithm>` is full of one-liners: `sort(begin, end)`, `reverse`, `max_element`, `accumulate` (from `<numeric>`). Pass a custom comparator: `sort(v.begin(), v.end(), greater<int>())` for descending. These are how interview C++ stays terse.",code:`#include <iostream>
#include <vector>
#include <algorithm>
#include <numeric>
using namespace std;
...
`,expected:"2"},takeaways:["<algorithm> works on iterator ranges (begin(), end()).","sort is O(n log n) (intro-sort); stable_sort keeps equal-element order.","accumulate (from <numeric>) folds with a binary op + init value.","Custom comparators: pass a lambda or a function-object."],mistakes:["Forgetting accumulate is in <numeric>, not <algorithm>.","Passing accumulate an int init when summing long long — overflow.","Comparator that returns true for equal items — strict weak ordering violated, UB."],next:{label:"Smart pointers",href:"#/courses/cpp-basics/cpp7"}},{id:"cpp7",title:"7. Smart pointers",intro:"Avoid raw `new`/`delete`. `std::unique_ptr<T>` owns its object exclusively and frees it on scope exit. `std::shared_ptr<T>` shares ownership via a ref-count. Create with `std::make_unique<T>(args...)` / `std::make_shared<T>(args...)`. Use `.get()` only when you need a raw pointer for legacy APIs.",code:`#include <iostream>
#include <memory>
using namespace std;
