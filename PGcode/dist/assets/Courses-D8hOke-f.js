...
`,expected:"Hi, Asha!"}},{id:"r2",title:"2. Props vs state",intro:"Props are inputs — set by the parent, read-only inside the component. State is internal mutable data managed with `useState`. Re-renders are triggered by state changes (or new props).",code:`// Conceptual — useState lives inside a component.
...
`,expected:"42"}},{id:"r3",title:"3. Event handlers",intro:"In JSX, attach handlers via camelCase props: `onClick`, `onChange`, `onSubmit`. The handler receives a React SyntheticEvent. Inline arrow functions are common but watch perf if a child re-renders on every render.",code:`// Pseudocode for an onClick handler:
...
`,expected:"type=click"}},{id:"r4",title:"4. Controlled inputs & forms",intro:"A controlled input has its `value` bound to state and an `onChange` to update state. Validation runs on each change or on submit. Show error messages near the input field. `e.preventDefault()` in the form's onSubmit stops the page reload.",code:`function validateEmail(s) {
  return /^[^@]+@[^@]+\\.[^@]+$/.test(s);
}
...
`,expected:"true"}},{id:"r5",title:"5. Lists & keys",intro:"Render arrays with `arr.map(item => <Item key={item.id} {...item} />)`. The `key` prop tells React which element is which between re-renders — crucial for correctness and performance. Never use array index as key if the list reorders.",code:`const items = [
  { id: 1, name: "alpha" },
  { id: 2, name: "beta" },
  { id: 3, name: "gamma" },
];
...
// log ms
`,expected:"1000"}},{id:"r7",title:"7. useMemo & useCallback",intro:"`useMemo(() => compute(), [deps])` caches an expensive computed value across renders. `useCallback(fn, [deps])` caches a function reference (so it stays stable when passed to `React.memo` children). Don't reach for them by default — they cost memory and a deps comparison; profile first. Use when an effect depends on an object/function reference, or when a child is doing expensive work.",code:`function Sum({ nums }) {
...
// log sum of squares
`,expected:"29"}},{id:"r8",title:"8. Custom hooks",intro:'A custom hook is just a function whose name starts with `use` that calls other hooks. They encapsulate reusable stateful logic without component nesting (Higher-Order Component / render-props problems). Examples: `useLocalStorage`, `useDebouncedValue`, `useFetch`. The "rules of hooks" still apply inside — same order every render, no conditionals around hook calls.',code:`// function useCounter(init = 0) {
...
console.log(n);
`,expected:"13"}},{id:"r9",title:"9. Context",intro:"`React.createContext(default)` creates a context. Wrap a tree in `<Ctx.Provider value={...}>` so any descendant can read it via `useContext(Ctx)` — no prop-drilling. Best for cross-cutting data that rarely changes: theme, locale, current user. For frequently-changing state, prefer state managers (Zustand, Redux, TanStack Query) since every Context value change re-renders every consumer.",code:`// const ThemeCtx = React.createContext("light");
...
`,expected:"ready"}}],n=[{id:"ts1",title:"1. Types & inference",intro:"TypeScript adds static types to JavaScript. Most of the time the compiler infers types — you only annotate when it can't guess (function parameters, hard cases). Core types: `string`, `number`, `boolean`, `string[]`, `{ x: number }`, plus literal types and unions.",code:`let name: string = "PGcode";
...
`,expected:"7"}},{id:"ts4",title:"4. Union types & narrowing",intro:"A param can be a union like `string | number`. Use `typeof` to narrow before operating: TS understands the narrowed branch.",code:`function len(x: string | number): number {
...
`,expected:"37"}},{id:"ts5",title:"5. Generics",intro:"A generic lets a function or type work over many types while preserving the relationship. `function first<T>(arr: T[]): T | undefined { return arr[0]; }` — the return type matches the input element type.",code:`function first<T>(arr: T[]): T | undefined { return arr[0]; }
const n = first([1, 2, 3]);
const s = first(["a", "b"]);
...
`,expected:"30"}},{id:"ts6",title:"6. Discriminated unions",intro:'A discriminated union shares a common literal field that lets TypeScript narrow safely in each branch. Pattern: every variant has `kind: "circle" | "square"` (or similar). TS knows which fields are valid inside each `if`.',code:`type Shape = { kind: "circle"; r: number } | { kind: "square"; side: number };
...
`,expected:"12"}},{id:"ts7",title:"7. Type guards (user-defined)",intro:'A guard like `function isString(x: unknown): x is string { return typeof x === "string"; }` lets TS narrow inside `if (isString(x))`. The `x is string` return type is the magic — it teaches the compiler about your runtime check.',code:`function isUser(x: unknown): x is { name: string } {
...
`,expected:"6"}},{id:"ts8",title:"8. Conditional types",intro:"`T extends U ? X : Y` returns one of two types based on whether T is assignable to U. Combine with `infer` to extract pieces — like the return type of a function: `type R<F> = F extends (...a: any[]) => infer R ? R : never;`.",code:`type NonNullableX<T> = T extends null | undefined ? never : T;
...
console.log("number");
`,expected:"number"}},{id:"ts9",title:"9. Mapped types",intro:"Iterate over keys with `{ [K in keyof T]: ... }` to transform shapes. The standard library uses this: `Partial<T> = { [K in keyof T]?: T[K] }`. `Readonly<T>`, `Pick<T,K>`, `Omit<T,K>` all use the same mechanism.",code:`type ReadonlyX<T> = { readonly [K in keyof T]: T[K] };
...
`,expected:"default"}},{id:"ts10",title:"10. Async typing & Promise<T>",intro:"`async function` always returns `Promise<T>`. Inside, `await p` unwraps to T. Use `Promise.all` (parallel) when independent. Type errors from `await` propagate as rejected promises — type the catch path.",code:`async function load(): Promise<string> { return "ok"; }
...
`,expected:"14"}}],r=[{id:"go1",title:"1. Hello + types",intro:"Go is statically typed but feels lightweight. `:=` declares + assigns with inferred type. Primitives: `int`, `int64`, `float64`, `bool`, `string`. Every program starts in `main()` of package `main`.",code:`package main
import "fmt"
func main() {
  name := "PGcode"
  x := 7
...
`,expected:"Math: 95"}},{id:"go2",title:"2. Control flow",intro:"Go has no parens around conditions but braces are mandatory. `if x > 0 { ... }`. Loops are all `for`; while-style: `for cond { }`; infinite: `for { }`.",code:`package main
import "fmt"
func main() {
...
`,expected:"55"}},{id:"go3",title:"3. Slices",intro:"Slices are dynamic arrays. `nums := []int{1,2,3}`. Append with `append(nums, 4)`. Length: `len(nums)`. Range over both index and value: `for i, v := range nums { … }`.",code:`package main
import "fmt"
func main() {
  nums := []int{3, 1, 4, 1, 5}
  total := 0
...
`,expected:"9"}},{id:"go4",title:"4. Maps",intro:'Go\'s hash map: `counts := map[string]int{}`. Read with `counts["key"]` (returns zero value if missing). Set with `counts[k] = v`. Test existence: `v, ok := counts[k]`.',code:`package main
import "fmt"
func main() {
  c := map[rune]int{}
...
`,expected:"3"}},{id:"go5",title:"5. Structs & methods",intro:"Structs group fields. Methods attach to a receiver: `func (p Point) String() string`. Use a pointer receiver `(*Point)` when the method mutates the struct or the struct is large (avoid copy).",code:`package main
import "fmt"
...
`,expected:"5"}},{id:"go6",title:"6. Interfaces",intro:"Interfaces in Go are implicitly satisfied — no `implements` keyword. If a type has all the methods, it satisfies the interface. `interface{}` (or `any` in Go 1.18+) accepts anything.",code:`package main
import "fmt"
...
`,expected:"16"}},{id:"go7",title:"7. Error handling",intro:'Go uses explicit error returns: `result, err := op()`. Check `if err != nil { ... }`. Build errors with `errors.New("msg")` or `fmt.Errorf("ctx: %w", err)` to wrap.',code:`package main
import (
  "errors"
  "fmt"
)
...
`,expected:"divide by zero"}},{id:"go8",title:"8. Goroutines",intro:"A goroutine is a lightweight green thread. `go f()` schedules f to run concurrently. Cheap — millions are fine. Use sync primitives (channels, WaitGroup) to coordinate.",code:`package main
import (
  "fmt"
  "sync"
)
...
`,expected:"10"}},{id:"go9",title:"9. Channels",intro:"Channels are typed pipes between goroutines. `ch := make(chan int)`. Send: `ch <- 7`. Receive: `v := <-ch`. Unbuffered channels block until both sender and receiver are ready (synchronous handoff).",code:`package main
import "fmt"
func main() {
  ch := make(chan int, 3)
  ch <- 1; ch <- 2; ch <- 3
...
`,expected:"15"}},{id:"go10",title:"10. Defer & cleanup",intro:"`defer f()` schedules f to run when the surrounding function returns (even on panic). Use for closing files, releasing locks, restoring state. Deferred calls run in LIFO order.",code:`package main
import "fmt"
func main() {
...
subject = ...
print(...)
`,expected:"Math: 95"}},{id:"p2",title:"2. Conditionals",intro:"`if`/`elif`/`else` lets your code branch. Comparisons return booleans (`==`, `!=`, `<`, `>`, `<=`, `>=`). Combine them with `and`, `or`, `not`. Indentation defines the block — Python is strict about 4 spaces.",code:`temp = 72
if temp >= 80:
    print("hot")
elif temp >= 60:
    print("nice")
...
`,expected:"minor"}},{id:"p3",title:"3. Loops",intro:"`for x in iterable` walks any sequence (list, string, range). `while cond:` repeats until the condition is false. `range(n)` generates 0..n-1 lazily. `break` exits the loop; `continue` skips to the next iteration.",code:`total = 0
for i in range(1, 6):
    total += i
...
for i in range(1, 11):
    ...
    pass
print(total)
`,expected:"30"}},{id:"p4",title:"4. Lists",intro:"Lists are ordered, mutable collections. Index with `lst[i]`, slice with `lst[a:b]`, append with `lst.append(x)`. Use `len(lst)` for length. Comprehensions like `[x*x for x in lst]` are the Pythonic way to map.",code:`nums = [3, 1, 4, 1, 5, 9, 2, 6]
...
# print the max
`,expected:"9"}},{id:"p5",title:"5. Functions",intro:"Define a function with `def`. Parameters can have defaults. `return` sends a value back. Functions are first-class — pass them around and return them. Docstrings (triple-quoted strings as the first line of a body) document behaviour.",code:`def greet(name, greeting="Hello"):
...
# count each word
# print(counts)
`,expected:"{'apple': 3, 'banana': 2, 'cherry': 1}"}},{id:"p7",title:"7. Strings",intro:'Strings are immutable sequences. Slice with `s[a:b]`, search with `in`, split with `.split(sep)`, join with `sep.join(list)`. Use f-strings (`f"x = {x}"`) for interpolation. Common methods: `.lower()`, `.upper()`, `.strip()`, `.replace(a, b)`, `.startswith()`, `.endswith()`.',code:`s = "  Hello, PGcode!  "
print(s.strip())
print(s.strip().lower())
print(",".join(["a", "b", "c"]))
name = "Asha"
...
`,expected:"4"}},{id:"p8",title:"8. Sets & tuples",intro:"A `set` is an unordered collection with no duplicates and O(1) membership tests (`x in s`). Build with `{1, 2, 3}` or `set(iterable)`. Tuples (`(1, 2, 3)`) are immutable lists — hashable, so they can be dict keys or set elements.",code:`nums = [1, 2, 2, 3, 3, 3, 4]
unique = set(nums)
print(sorted(unique))
print(3 in unique)
print(5 in unique)
...
print(triple(5))
`,expected:"30"}},{id:"p11",title:"11. Generators",intro:"A generator function uses `yield` to produce values one at a time, lazily. It returns a generator object you iterate with `for x in gen` or `next(gen)`. Generators keep memory tiny when streaming large data. Generator expressions `(x*x for x in nums)` look like comprehensions but produce a generator instead of a list.",code:`def fib():
    a, b = 0, 1
    while True:
        yield a
        a, b = b, a + b
...
with silent():
    print("hello")
print("done")
`,expected:"done"}},{id:"p14",title:"14. Dataclasses",intro:"`@dataclass` auto-generates `__init__`, `__repr__`, and `__eq__` for a class whose body is mostly field declarations. It eliminates boilerplate for value types. Use `field(default_factory=list)` for mutable defaults. Add `frozen=True` to make instances immutable and hashable.",code:`from dataclasses import dataclass, field
...
`,expected:"Math: 95"}},{id:"j2",title:"2. Conditionals",intro:"`if`/`else if`/`else` work like most languages. Use `===` and `!==` (strict equality) — never `==` (it does coercion). Ternaries `cond ? a : b` are great for one-liners.",code:`const temp = 72;
let label;
...
// log "minor" or "adult"
`,expected:"minor"}},{id:"j3",title:"3. Loops & arrays",intro:"Use `for (let i = 0; i < arr.length; i++)` for index-based loops, `for (const x of arr)` for value iteration. Arrays have `.map`, `.filter`, `.reduce`, `.forEach` — most algorithms become one-liners.",code:`const nums = [1, 2, 3, 4, 5];
...
console.log(total);
`,expected:"30"}},{id:"j4",title:"4. Functions & arrows",intro:"Two forms: `function name(...)` (hoisted) and `const name = (...) => ...` (not hoisted). Arrows have no own `this` — they capture the enclosing one. Use default params for nice ergonomics.",code:`const square = n => n * n;
...
`,expected:"120"}},{id:"j5",title:"5. Objects & destructuring",intro:"Objects map string keys to values. Use dot or bracket access. Destructure to pull fields out concisely. Spread (`...`) clones and merges.",code:`const user = { name: "Asha", age: 28, role: "dev" };
...
// sum the values
`,expected:"6"}},{id:"j6",title:"6. Map for counts",intro:"`Map` is JS's hash map — keys can be any type, iteration order is insertion order. Use `map.get(k)`, `map.set(k, v)`, `map.has(k)`. The most common interview pattern: count-the-thing.",code:`const counts = new Map();
...
`,expected:"3"}},{id:"j7",title:"7. Strings & template literals",intro:"Strings are primitives but have methods via auto-boxing. Slice with `slice(a, b)`, find with `indexOf` / `includes`, split with `split(sep)`, join with `arr.join(sep)`. Template literals `` `Hi ${name}` `` are the modern way to interpolate. Strings are immutable — every method returns a new string.",code:`const s = "the quick brown fox";
...
`,expected:"dlrow olleh"}},{id:"j8",title:"8. Set & uniqueness",intro:"`Set` holds unique values, with O(1) `has`/`add`/`delete`. Build from an iterable: `new Set([1,1,2])`. Convert back to an array with `[...set]` or `Array.from(set)`. Great for dedup + membership.",code:`const arr = [1, 2, 2, 3, 3, 3, 4];
...
// log o.x + o.y
`,expected:"7"}},{id:"j11",title:"11. Map, filter, reduce",intro:"The trio every JS interview tests. `map(fn)` transforms each element. `filter(pred)` keeps matching elements. `reduce(fn, init)` accumulates a single value. Each returns a new array — never mutates.",code:`const nums = [1, 2, 3, 4, 5, 6];
...
`,expected:"220"}},{id:"j12",title:"12. Closures",intro:"A closure is a function plus the variables it captured from its outer scope. The captured variables stay alive even after the outer function returns. Closures power module patterns, currying, and most callbacks.",code:`function counter() {
  let n = 0;
  return () => ++n;
}
const c = counter();
...
`,expected:"5 6"}}]},"java-basics":{id:"java-basics",title:"Java Basics",language:"java",color:"#E76F00",blurb:"The Java you need for FAANG interview solutions: types, control flow, methods, collections.",estimatedHours:2,lessons:[{id:"jv1",title:"1. Hello + types",intro:"Every Java program lives in a class. `main` is the entry point. The primitive types are `int`, `long`, `double`, `boolean`, `char`. `String` is an object. Variables must be declared with a type.",code:`public class Main {
...
`,expected:"Math: 95"}},{id:"jv2",title:"2. Control flow",intro:"`if`/`else if`/`else`, plus `switch`. Use `==` only for primitives. For Strings use `.equals()`. Boolean operators: `&&`, `||`, `!`.",code:`public class Main {
  public static void main(String[] args) {
    int temp = 72;
...
`,expected:"minor"}},{id:"jv3",title:"3. Methods + arrays",intro:"Methods need an explicit return type. Arrays are fixed-size: `int[] arr = new int[5]`. `arr.length` (not `arr.length()`). Use `Arrays.toString(arr)` to print.",code:`import java.util.Arrays;
public class Main {
...
`,expected:"30"}},{id:"jv4",title:"4. HashMap",intro:"`HashMap<K, V>` is Java's hash table. `put`, `get`, `containsKey`. `getOrDefault(key, default)` is your friend for counting. Don't forget the import.",code:`import java.util.HashMap;
...
`,expected:"3"}},{id:"jv5",title:"5. ArrayList",intro:"`ArrayList<T>` is Java's dynamic array. `add`, `get(i)`, `size()`, `set(i, x)`. Iterate with enhanced-for: `for (Integer x : list)`. Use `List.of(1, 2, 3)` for immutable factory.",code:`import java.util.ArrayList;
...
`,expected:"5"}},{id:"jv6",title:"6. Strings",intro:"Strings are immutable in Java. Common ops: `.length()`, `.charAt(i)`, `.substring(a, b)`, `.split(regex)`, `.toLowerCase()`, `.trim()`. Build long strings with `StringBuilder` (mutable, O(1) append) — concatenating `String` in a loop is O(n²).",code:`public class Main {
  public static void main(String[] args) {
...
`,expected:"[3, 2, 1]"}}]},"react-basics":{id:"react-basics",title:"React Basics",language:"javascript",color:"#61dafb",blurb:"Components, JSX, props vs state, events, controlled forms, lists & keys. Five lessons to get you productive.",estimatedHours:1.5,lessons:t},"typescript-basics":{id:"typescript-basics",title:"TypeScript Basics",language:"typescript",color:"#3178c6",blurb:"Types, interfaces, unions, generics — the typed JS subset that catches half your bugs at compile time.",estimatedHours:2,lessons:n},"go-basics":{id:"go-basics",title:"Go Basics",language:"go",color:"#00add8",blurb:"Go in 4 lessons: types, control flow, slices, maps. Enough to start solving LeetCode in Go.",estimatedHours:1.5,lessons:r},"cpp-basics":{id:"cpp-basics",title:"C++ Basics",language:"cpp",color:"#00599C",blurb:"Modern C++17 essentials: types, control flow, vectors, unordered_map.",estimatedHours:2,lessons:[{id:"cpp1",title:"1. Hello + types",intro:"Every C++ program needs `main()`. Use `#include <iostream>` for I/O and `using namespace std;` to skip prefixes. Common types: `int`, `long long`, `double`, `bool`, `string` (from `<string>`).",code:`#include <iostream>
#include <string>
using namespace std;
int main() {
  int x = 7;
...
`,expected:"Math: 95"}},{id:"cpp2",title:"2. Control flow",intro:"`if`/`else if`/`else`, plus ternary `cond ? a : b`. `==` works for both primitives and `std::string`. Boolean operators: `&&`, `||`, `!`.",code:`#include <iostream>
using namespace std;
int main() {
  int temp = 72;
...
`,expected:"minor"}},{id:"cpp3",title:"3. Vectors + loops",intro:"`std::vector<T>` is the go-to dynamic array. `push_back`, `size()`, index with `[]`. Range-based for: `for (int x : v)`. Algorithm header has sort, etc.",code:`#include <iostream>
#include <vector>
#include <algorithm>
using namespace std;
int main() {
...
`,expected:"30"}},{id:"cpp4",title:"4. unordered_map",intro:"`std::unordered_map<K, V>` is the hash table. Insert with `m[key] = v`; query with `m[key]` (default-constructs missing keys to zero — convenient for counts).",code:`#include <iostream>
...
`,expected:"3"}},{id:"cpp5",title:"5. Strings",intro:"`std::string` (from `<string>`) is mutable, with `+`/`+=` concatenation, `.length()`, `.substr(pos, n)`, indexing via `s[i]`. `std::getline(cin, s)` reads a whole line. For splits or fancy ops, you typically reach for `<sstream>` (`std::stringstream`).",code:`#include <iostream>
#include <sstream>
#include <string>
using namespace std;
int main() {
...
`,expected:"11"}},{id:"cpp6",title:"6. Sort & algorithms",intro:"`<algorithm>` is full of one-liners: `sort(begin, end)`, `reverse`, `max_element`, `accumulate` (from `<numeric>`). Pass a custom comparator: `sort(v.begin(), v.end(), greater<int>())` for descending. These are how interview C++ stays terse.",code:`#include <iostream>
#include <vector>
#include <algorithm>
#include <numeric>
using namespace std;
...
`,expected:"2"}},{id:"cpp7",title:"7. Smart pointers",intro:"Avoid raw `new`/`delete`. `std::unique_ptr<T>` owns its object exclusively and frees it on scope exit. `std::shared_ptr<T>` shares ownership via a ref-count. Create with `std::make_unique<T>(args...)` / `std::make_shared<T>(args...)`. Use `.get()` only when you need a raw pointer for legacy APIs.",code:`#include <iostream>
#include <memory>
using namespace std;
...
`,expected:"42"}},{id:"cpp8",title:"8. Move semantics",intro:'Move semantics let you transfer ownership of resources without a deep copy. `std::move(x)` casts to an rvalue, signalling "you can steal from me." A move constructor `T(T&&) noexcept` takes the resource and leaves the source empty. Standard containers (`vector`, `string`, `unique_ptr`) already implement moves — passing by value into a function then doing `member = std::move(arg)` is the idiomatic way to absorb a parameter.',code:`#include <iostream>
#include <vector>
#include <string>
using namespace std;
int main() {
...
`,expected:"4"}}]},"rust-basics":{id:"rust-basics",title:"Rust Basics",language:"rust",color:"#dea584",blurb:"Ownership, borrowing, traits, error handling, iterators, async — the Rust mental model in six lessons.",estimatedHours:2.5,lessons:[{id:"rs1",title:"1. Hello + types",intro:"Every Rust binary has `fn main()`. Variables are `let x = 5;` (immutable by default) — add `mut` to allow rebinding via assignment. Common scalar types: `i32`, `i64`, `f64`, `bool`, `char`, `&str`/`String`. Rust infers types but you can annotate: `let n: i64 = 42;`.",code:`fn main() {
    let x = 7;
    let name = "PGcode";
    let pi: f64 = 3.14159;
...
`,expected:"Math: 95"}},{id:"rs2",title:"2. Ownership & borrowing",intro:"Every value has exactly one owner. Assigning to another binding moves ownership (the original can no longer be used). Borrow with `&` (immutable, many at once) or `&mut` (exclusive, one at a time). The compiler enforces these rules at compile time — no garbage collector, no data races.",code:`fn main() {
...
`,expected:"14"}},{id:"rs4",title:"4. Result & Option",intro:"Rust replaces exceptions with `Result<T, E>` for failures and `Option<T>` for absence. Match on them, or use `?` to propagate errors up. `unwrap()` panics if empty — fine for prototypes, never for production. `map`, `and_then`, `unwrap_or` chain cleanly.",code:`fn divide(a: i32, b: i32) -> Result<i32, String> {
...
`,expected:"4"}},{id:"rs6",title:"6. Async with Tokio",intro:"Rust async is poll-based and zero-cost. Declare `async fn` and `await` futures. You need a runtime to actually run them — Tokio is the standard. `#[tokio::main]` on `main` lets you await at the top level. `tokio::join!` runs multiple futures concurrently. Use async only for I/O — CPU-bound work should still use threads (`std::thread` or Rayon).",code:`// Conceptual — would compile with tokio in Cargo.toml.
// #[tokio::main]
// async fn main() {
//     let (a, b) = tokio::join!(
//         async { 1 + 2 },
