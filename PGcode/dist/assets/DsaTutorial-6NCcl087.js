...
> Tip: Always allocate \`4·n\` slots for the recursive form — the worst-case tree height plus the off-by-one slack adds up.`},{heading:"When to reach for it",body:"Reach for a segment tree the moment a Fenwick tree (BIT) can't carry the problem. Fenwick handles **prefix sum / xor** under point updates beautifully in 8 lines — use it. Move to segment tree when you need **min / max / gcd** (no inverse, so Fenwick doesn't work), **range updates + range queries** (lazy propagation), arbitrary **associative merges** (matrix product, polynomial composition), or **walking the tree** to answer \"the first index `i` such that `a[i] >= x`\" in `O(log n)`. If the operation is offline and a simple **sort + sweep + Fenwick** would do, prefer that — segment tree is the heavier tool."},{heading:"Variants",body:'**Lazy propagation** — store pending range updates at each node; push down to children only on access. Enables range-add + range-sum (or range-assign + range-min) in `O(log n)`.\n\n**Persistent segment tree** — every update produces a new `O(log n)` path while sharing the rest of the tree. Powers the classic `k`-th smallest in a range via persistent counts on values.\n\n**Merge sort tree** — each node stores its range\'s sorted copy. `O(n log n)` space; answers "how many elements in `[l, r]` are `> x`" in `O(log^2 n)` via binary search per visited node.\n\n**2-D segment tree** — segment tree of segment trees. `O(log^2 n)` per op, `O(n^2)` space; usually overkill.\n\n**Segment tree beats (Chtholly tree)** — handles `chmin` / `chmax` range ops by tracking max / second-max per node.\n\n**Implicit / dynamic segment tree** — allocates children only when touched. Use for coordinate ranges up to `10^9` without coordinate compression.'},{heading:"Common interview problems",body:["Range Sum Query — Mutable (LeetCode 307) — the textbook intro.","Count of Smaller Numbers After Self (LeetCode 315) — merge-sort tree or BIT on compressed values.","My Calendar III (LeetCode 732) — range-add, max query, lazy propagation.","Skyline Problem (LeetCode 218) — segment tree on coordinate-compressed x with lazy max.","Range Module (LeetCode 715) — interval add / remove / query, frequently solved with segment tree.","Falling Squares (LeetCode 699) — range-assign max with lazy propagation.","Number of Longest Increasing Subsequence — segment tree keyed by value."]}],complexity:{build:"`O(n)` bottom-up — every node touched once",operation:"`O(log n)` per point update or range query; `O(log n)` per range update with lazy propagation",time:"Point query / update `O(log n)`; range query / range update `O(log n)`",space:"`O(4n)` for a recursive tree; `O(2n)` for the iterative variant on power-of-two sizes"},pitfalls:["**Off-by-one on the range convention.** Mixing `[l, r]` inclusive with `[l, r)` half-open is the #1 source of bugs. **Fix:** pick the half-open form, document it in a comment at the class top, and stick to it across query, update, and recursion.","**Forgetting to push the lazy tag down before reading children.** Each `query` / `update` that descends into children must first apply the parent's pending update and clear the tag. **Fix:** write a `push(p)` helper and call it as the first line of every recursive method.",'**Reaching for segment tree when Fenwick suffices.** For pure prefix sum or xor with point updates, BIT is 8 lines, twice as fast, and bug-free. **Fix:** ask "do I need min / max / range update / something non-invertible?" If no, write a Fenwick.',"**Under-allocating the tree array.** Sizing at `2n` for the recursive form crashes on non-power-of-two `n`. **Fix:** allocate `4n` for recursive, or round `n` up to the next power of two for iterative.",'**Combining non-associative aggregates.** Segment tree silently breaks if `f` isn\'t associative (e.g., subtraction). **Fix:** verify `f(f(a,b),c) == f(a,f(b,c))` on paper before coding; for "average," store `(sum, count)` and divide at the end.']}}]},{id:"basic-seg",label:"Basic Segment Tree",items:[{kind:"problem",label:"Segment Tree"},{kind:"problem",label:"Min-Max Range Queries"},{kind:"problem",label:"Range LCM Queries"},{kind:"problem",label:"XOR of a given range"}]},{id:"lazy",label:"Segment Tree with Updates",items:[{kind:"problem",label:"Lazy Propagation"},{kind:"problem",label:"Flipping Sign"}]},{id:"app",label:"Segment Tree Applications",items:[{kind:"problem",label:"Build a segment tree for N-ary rooted tree"},{kind:"problem",label:"Maximum of all subarrays of size K using Segment Tree"}]},{id:"fenwick",label:"Fenwick Tree",items:[{kind:"problem",label:"Fenwick Tree"},{kind:"problem",label:"Sum and Update Queries on 3D Matrix"}]}]}];function Z(n){return n.subsections.reduce((t,r)=>t+r.items.length,0)}function je(){return T.reduce((n,t)=>n+Z(t),0)}const Ne=[{id:"fundamentals",label:"Fundamentals",slugs:["fundamentals","maths-pattern-recursion"]},{id:"patterns",label:"Patterns",slugs:["two-pointer","sliding-window","prefix-sum","backtracking","bit-manipulation","hashing"]},{id:"data-structures",label:"Data Structures",slugs:["array-string","linked-list","stack","queue","deque","binary-tree","bst","heap","graph","trie"]},{id:"algorithms",label:"Algorithms",slugs:["searching","sorting","greedy","dp","number-theory","string-matching","range-query"]}];function q(n){return(n||"").toLowerCase().replace(/[^a-z0-9]+/g,"")}function He({session:n}){var W;const t=(W=n==null?void 0:n.user)==null?void 0:W.id,{data:r=[]}=ce(),{data:o=[]}=he(),{data:s}=ue(t),l=f.useMemo(()=>(s==null?void 0:s.byId)||{},[s]),c=f.useMemo(()=>{const a=new Map;return r.forEach(u=>a.set(q(u.name),u)),a},[r]),i=f.useMemo(()=>{const a=new Map;return o.forEach(u=>a.set(q(u.title),u)),a},[o]),g=f.useMemo(()=>{let a=0,u=0,h=0;return T.forEach(b=>{b.subsections.forEach(w=>{w.items.forEach(v=>{var O;if(v.kind==="problem"){a++;const S=c.get(q(v.label));S&&(h++,(O=l[S.id])!=null&&O.is_completed&&u++)}})})}),{totalProblems:a,solved:u,matched:h,totalAll:je()}},[c,l]),d=f.useMemo(()=>T.map(a=>{let u=0,h=0;return a.subsections.forEach(b=>{b.items.forEach(w=>{var O;if(w.kind!=="problem")return;u+=1;const v=c.get(q(w.label));v&&((O=l[v.id])!=null&&O.is_completed)&&(h+=1)})}),{slug:a.slug,title:a.title,solved:h,total:u,pct:u?h/u:0}}),[c,l]),[p,m]=f.useState(""),y=p.trim().toLowerCase(),[x,k]=f.useState(()=>new Set(T.map(a=>a.slug))),[F,ee]=f.useState("all"),[A,B]=f.useState(!1),ne=a=>k(u=>{const h=new Set(u);return h.has(a)?h.delete(a):h.add(a),h}),te=()=>k(new Set(T.map(a=>a.slug))),ae=()=>k(new Set),L=f.useMemo(()=>{if(!y)return null;const a=new Map;return T.forEach(u=>{const h=u.subsections.map(b=>{const w=b.items.filter(v=>v.label.toLowerCase().includes(y));return w.length?{...b,items:w}:null}).filter(Boolean);(h.length||u.title.toLowerCase().includes(y))&&a.set(u.slug,h.length?h:u.subsections)}),a},[y]),M=f.useRef(null),[ie,re]=f.useState(!1),[oe,se]=f.useState(0);f.useEffect(()=>{const a=M.current;if(!a)return;const u=()=>{re(a.scrollTop>600);const h=a.scrollHeight-a.clientHeight,b=h>0?a.scrollTop/h*100:0;se(Math.max(0,Math.min(100,b)))};return a.addEventListener("scroll",u,{passive:!0}),u(),()=>a.removeEventListener("scroll",u)},[]);const le=()=>{var a;(a=M.current)==null||a.scrollTo({top:0,behavior:"smooth"})},z=a=>{var h;const u=(h=M.current)==null?void 0:h.querySelector(`#tut-${a}`);u&&M.current&&M.current.scrollTo({top:u.offsetTop-20,behavior:"smooth"}),k(b=>{const w=new Set(b);return w.delete(a),w})},I=g.totalProblems?Math.round(g.solved/g.totalProblems*100):0;return e.jsxs("div",{className:"tut-container",ref:M,children:[e.jsx("div",{className:"tut-reading-progress","aria-hidden":"true",style:{width:`${oe}%`}}),e.jsxs("header",{className:"tut-header",children:[e.jsxs("div",{className:"tut-hero",children:[e.jsxs("div",{className:"tut-hero-left",children:[e.jsx("h1",{className:"tut-title",children:"DSA Tutorial"}),e.jsx("p",{className:"tut-sub",children:"Every data structure and algorithm, top to bottom — theory plus the problems that drill it."})]}),e.jsxs("div",{className:"tut-hero-right",children:[e.jsxs("div",{className:"tut-stat",children:[e.jsx("span",{className:"tut-stat-num",children:g.totalAll}),e.jsx("span",{className:"tut-stat-label",children:"items"})]}),e.jsx("div",{className:"tut-stat-divider","aria-hidden":"true"}),e.jsxs("div",{className:"tut-stat",children:[e.jsx("span",{className:"tut-stat-num",children:T.length}),e.jsx("span",{className:"tut-stat-label",children:"topics"})]}),t&&e.jsxs(e.Fragment,{children:[e.jsx("div",{className:"tut-stat-divider","aria-hidden":"true"}),e.jsxs("div",{className:"tut-stat tut-stat-progress",children:[e.jsxs("span",{className:"tut-stat-num tut-stat-num-accent",children:[I,"%"]}),e.jsxs("span",{className:"tut-stat-label",children:[g.solved,"/",g.totalProblems," solved"]})]})]})]})]}),t&&e.jsx("div",{className:"tut-progress-bar","aria-label":`${I}% complete`,children:e.jsx("div",{className:"tut-progress-fill",style:{width:`${I}%`}})})]}),e.jsxs("div",{className:"tut-toolbar",children:[e.jsxs("div",{className:"tut-search",children:[e.jsx(pe,{size:14,className:"tut-search-icon"}),e.jsx("input",{value:p,onChange:a=>m(a.target.value),placeholder:"Search the entire syllabus"}),p&&e.jsx("button",{className:"tut-search-clear",onClick:()=>m(""),"aria-label":"Clear",children:e.jsx(K,{size:14})})]}),e.jsx("div",{className:"tut-filters",role:"tablist","aria-label":"Filter items",children:[{id:"all",label:"All"},{id:"theory",label:"Theory"},{id:"problems",label:"Problems"},{id:"unsolved",label:t?"Unsolved":"Available"}].map(a=>e.jsx("button",{type:"button",role:"tab","aria-selected":F===a.id,className:`tut-filter-chip ${F===a.id?"active":""}`,onClick:()=>ee(a.id),children:a.label},a.id))}),e.jsxs("div",{className:"tut-actions",children:[e.jsx("button",{type:"button",onClick:ae,children:"Expand all"}),e.jsx("span",{className:"tut-actions-sep","aria-hidden":"true",children:"·"}),e.jsx("button",{type:"button",onClick:te,children:"Collapse all"})]})]}),e.jsxs("div",{className:`tut-body ${A?"with-toc":""}`,children:[A&&e.jsx("button",{className:"tut-toc-overlay","aria-label":"Close contents",onClick:()=>B(!1)}),e.jsxs("aside",{className:`tut-toc ${A?"open":""}`,"aria-label":"Topics",children:[e.jsxs("div",{className:"tut-toc-head",children:[e.jsx("h3",{className:"tut-toc-title",children:"Topics"}),e.jsx("button",{className:"tut-toc-close",onClick:()=>B(!1),"aria-label":"Close",children:e.jsx(K,{size:12})})]}),e.jsx("nav",{className:"tut-toc-nav",children:Ne.map(a=>{const u=T.map((h,b)=>({section:h,idx:b})).filter(({section:h})=>a.slugs.includes(h.slug));return u.length?e.jsxs("div",{className:"tut-toc-group",children:[e.jsx("div",{className:"tut-toc-group-label",children:a.label}),e.jsx("ol",{className:"tut-toc-list",children:u.map(({section:h,idx:b})=>{const w=d[b],v=!x.has(h.slug),O=w&&w.total>0&&w.solved===w.total,S=w&&w.solved>0&&w.solved<w.total,N=O?"done":S?"partial":"";return e.jsx("li",{children:e.jsxs("button",{type:"button",className:`tut-toc-row ${v?"is-active":""}`,onClick:()=>{z(h.slug),B(!1)},title:h.title,"aria-current":v?"true":void 0,children:[e.jsx("span",{className:"tut-toc-num",children:String(b+1).padStart(2,"0")}),e.jsx("span",{className:"tut-toc-name",children:h.title}),e.jsx("span",{className:`tut-toc-dot ${N}`,"aria-hidden":"true"})]})},h.slug)})})]},a.id):null})})]}),e.jsx("main",{className:"tut-main",children:T.map((a,u)=>{const h=L?L.get(a.slug):a.subsections;if(!h)return null;const b=x.has(a.slug)&&!L,w=Z(a),v=d[u],O=v&&v.solved>0;return e.jsxs("section",{id:`tut-${a.slug}`,className:`tut-section ${b?"":"is-expanded"}`,children:[e.jsxs("button",{type:"button",className:"tut-section-head",onClick:()=>ne(a.slug),"aria-expanded":!b,"aria-controls":`tut-${a.slug}-body`,children:[e.jsx("span",{className:"tut-section-num",children:String(u+1).padStart(2,"0")}),e.jsx("span",{className:"tut-section-chev","aria-hidden":"true",children:b?e.jsx(R,{size:14}):e.jsx(E,{size:14})}),e.jsx("h2",{className:"tut-section-title",children:a.title}),e.jsx("span",{className:`tut-section-count ${O?"has-solved":""}`,children:v&&v.total>0?`${v.solved}/${v.total}`:w})]}),!b&&e.jsxs("div",{id:`tut-${a.slug}-body`,className:"tut-section-body",children:[a.note&&e.jsx("p",{className:"tut-section-note",children:a.note}),h.map(S=>{const N=S.items.filter(_=>Pe(_,F,c,l));return N.length===0?null:e.jsxs("div",{className:"tut-subsection",children:[e.jsx("h3",{className:"tut-subsection-title",children:S.label}),e.jsx("ul",{className:"tut-item-list",children:N.map((_,de)=>e.jsx(De,{item:_,problemByName:c,conceptByName:i,byId:l,highlight:y},`${S.id}-${de}`))})]},S.id)})]})]},a.slug)})})]}),ie&&e.jsx("button",{className:"tut-back-top",onClick:le,"aria-label":"Back to top",children:e.jsx(me,{size:14})}),e.jsx("nav",{className:"tut-rail","aria-label":"Sections quick access",children:d.map((a,u)=>{const h=2*Math.PI*13,b=h*a.pct;return e.jsxs("button",{className:`tut-rail-dot ${a.pct===1?"done":""}`,onClick:()=>z(a.slug),title:`${a.title} — ${a.solved}/${a.total} solved`,"aria-label":`Jump to ${a.title}, ${a.solved} of ${a.total} solved`,children:[e.jsxs("svg",{viewBox:"0 0 32 32",className:"tut-rail-svg",children:[e.jsx("circle",{cx:"16",cy:"16",r:"13",className:"tut-rail-bg"}),e.jsx("circle",{cx:"16",cy:"16",r:"13",className:"tut-rail-fg",strokeDasharray:`${b} ${h-b}`,transform:"rotate(-90 16 16)"})]}),e.jsx("span",{className:"tut-rail-num",children:u+1})]},a.slug)})})]})}function Pe(n,t,r,o){var s;if(t==="all")return!0;if(t==="theory")return n.kind==="theory"||n.kind==="topic";if(t==="problems")return n.kind==="problem";if(t==="unsolved"){if(n.kind!=="problem")return!1;const l=i=>(i||"").toLowerCase().replace(/[^a-z0-9]+/g,""),c=r.get(l(n.label));return c?!((s=o[c.id])!=null&&s.is_completed):!1}return!0}function j(n,t){if(!t)return n;const o=n.toLowerCase().indexOf(t);return o===-1?n:e.jsxs(e.Fragment,{children:[n.slice(0,o),e.jsx("mark",{children:n.slice(o,o+t.length)}),n.slice(o+t.length)]})}function De({item:n,problemByName:t,conceptByName:r,byId:o,highlight:s}){var g;const[l,c]=f.useState(!1);if(n.kind==="topic"){const d=n.conceptSlug?null:r.get(q(n.label)),p=n.conceptSlug||(d==null?void 0:d.slug);let m=d==null?void 0:d.module_slug;if(p&&!m){const k=[...r.values()].find(F=>F.slug===p);k&&(m=k.module_slug)}const y=!!n.body,x=!!(p&&m);return e.jsxs("li",{className:`tut-item-theory-wrap tut-item-topic-wrap ${l?"expanded":""}`,children:[e.jsxs("button",{type:"button",className:"tut-item tut-item-topic tut-item-theory-button",onClick:()=>c(k=>!k),"aria-expanded":l,children:[e.jsx("span",{className:"tut-item-icon",children:l?e.jsx(E,{size:11}):e.jsx(R,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,s)}),e.jsx("span",{className:"tut-item-kind",children:"topic"})]}),l&&e.jsxs("div",{className:"tut-theory-body",children:[y&&e.jsx(X,{body:n.body}),x&&e.jsxs(D,{to:`/learn/${m}/${p}`,className:"tut-theory-readmore",children:[e.jsx(P,{size:11})," Open full concept page",e.jsx(U,{size:10})]}),!y&&!x&&e.jsxs("p",{className:"tut-theory-placeholder",children:[e.jsx(ge,{size:10,className:"tut-inline-icon"}),"See the related concepts above for in-depth coverage."]})]})]})}if(n.kind==="theory"){const d=n.conceptSlug?null:r.get(q(n.label)),p=n.conceptSlug||(d==null?void 0:d.slug);let m=d==null?void 0:d.module_slug;if(p&&!m){const k=[...r.values()].find(F=>F.slug===p);k&&(m=k.module_slug)}const y=!!n.body,x=!!(p&&m);return y?e.jsxs("li",{className:`tut-item-theory-wrap ${l?"expanded":""}`,children:[e.jsxs("button",{type:"button",className:"tut-item tut-item-theory tut-item-theory-button",onClick:()=>c(k=>!k),"aria-expanded":l,children:[e.jsx("span",{className:"tut-item-icon",children:l?e.jsx(E,{size:11}):e.jsx(R,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,s)}),e.jsx("span",{className:"tut-item-kind",children:"theory"})]}),l&&e.jsxs("div",{className:"tut-theory-body",children:[e.jsx(X,{body:n.body}),x&&e.jsxs(D,{to:`/learn/${m}/${p}`,className:"tut-theory-readmore",children:[e.jsx(P,{size:11})," Open full concept page",e.jsx(U,{size:10})]})]})]}):x?e.jsx("li",{className:"tut-item tut-item-theory",children:e.jsxs(D,{to:`/learn/${m}/${p}`,className:"tut-item-link",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(P,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,s)}),e.jsx("span",{className:"tut-item-kind",children:"theory"})]})}):e.jsxs("li",{className:"tut-item tut-item-theory-soft",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(P,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,s)}),e.jsx("span",{className:"tut-item-kind",children:"theory · soon"})]})}const i=n.id?null:t.get(q(n.label));if(i){const d=o[i.id],p=d==null?void 0:d.is_completed,m=!p&&((d==null?void 0:d.status)==="attempted"||(d==null?void 0:d.is_starred)),y=p?"solved":m?"attempted":"todo";return e.jsx("li",{className:`tut-item ${p?"solved":""}`,children:e.jsxs(D,{to:`/category/${encodeURIComponent(i.topic_id)}/${encodeURIComponent(i.id)}`,className:"tut-item-link",children:[e.jsx("span",{className:`tut-bubble tut-bubble-${y}`,"aria-label":p?"solved":m?"attempted":"not started",children:p&&e.jsx(be,{size:9})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,s)}),e.jsx("span",{className:`tut-item-diff tut-diff-${(g=i.difficulty)==null?void 0:g.toLowerCase()}`,children:i.difficulty})]})})}return e.jsxs("li",{className:"tut-item tut-item-soon",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(fe,{size:10})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,s)}),e.jsx("span",{className:"tut-item-kind",children:"soon"})]})}function C(n,t=""){if(n==null)return null;const r=String(n),o=/(`[^`]+`)|(\[[^\]]+\]\([^)]+\))|(\*\*[^*]+\*\*)|(\*[^*]+\*)/g,s=[];let l=0,c=0,i;for(;(i=o.exec(r))!==null;){if(i.index>l&&s.push(e.jsx(V.Fragment,{children:r.slice(l,i.index)},`${t}-t-${c++}`)),i[1])s.push(e.jsx("code",{className:"tut-theory-code",children:i[1].slice(1,-1)},`${t}-c-${c++}`));else if(i[2]){const g=/\[([^\]]+)\]\(([^)]+)\)/.exec(i[2]);s.push(e.jsx("a",{href:g[2],target:"_blank",rel:"noopener noreferrer",children:g[1]},`${t}-a-${c++}`))}else i[3]?s.push(e.jsx("strong",{children:i[3].slice(2,-2)},`${t}-b-${c++}`)):i[4]&&s.push(e.jsx("em",{children:i[4].slice(1,-1)},`${t}-i-${c++}`));l=i.index+i[0].length}return l<r.length&&s.push(e.jsx(V.Fragment,{children:r.slice(l)},`${t}-t-${c++}`)),s.length===0?r:s}function Re(n){const t=/^>\s*(Note|Warning|Tip|Insight|Caution):\s*(.*)$/i.exec(n.trim());return t?{kind:t[1].toLowerCase(),text:t[2]}:null}function Ae({kind:n,children:t}){const r=n==="warning"||n==="caution"?e.jsx(Y,{size:14}):n==="tip"||n==="insight"?e.jsx(qe,{size:14}):e.jsx(Me,{size:14}),o=n==="warning"||n==="caution"?"Warning":n==="tip"?"Tip":n==="insight"?"Insight":"Note";return e.jsxs("aside",{className:`tut-callout tut-callout-${n}`,children:[e.jsx("span",{className:"tut-callout-icon",children:r}),e.jsxs("div",{className:"tut-callout-body",children:[e.jsx("span",{className:"tut-callout-label",children:o}),e.jsx("span",{className:"tut-callout-text",children:t})]})]})}const Be={py:"Python",python:"Python",js:"JavaScript",javascript:"JavaScript",ts:"TypeScript",typescript:"TypeScript",cpp:"C++",c:"C",java:"Java",go:"Go",rs:"Rust",rust:"Rust",sh:"Shell",bash:"Shell",shell:"Shell",sql:"SQL",json:"JSON",ascii:"Diagram",diagram:"Diagram",txt:"Diagram",plain:"Code","":"Code"};function Le(n){if(!n)return!1;const t=n.slice(0,800),r=(t.match(/[│─┌┐└┘├┤┬┴┼╭╮╯╰═║╔╗╚╝→←↑↓⇒⇐|+\-*=>]/g)||[]).length,o=(t.match(/[a-zA-Z]/g)||[]).length;return r<6?!1:r>o*.25}function G({lang:n,code:t}){const r=(n||"").toLowerCase(),o=r==="ascii"||r==="diagram"||r==="txt"||!r&&Le(t),s=o?"Diagram":Be[r]||(r?r.toUpperCase():"Code"),[l,c]=f.useState(!1),i=f.useCallback(()=>{navigator.clipboard&&navigator.clipboard.writeText(t).then(()=>{c(!0),window.setTimeout(()=>c(!1),1400)}).catch(()=>{})},[t]);return e.jsxs("div",{className:`tut-codeblock ${o?"tut-codeblock-ascii":""}`,children:[e.jsxs("div",{className:"tut-codeblock-bar",children:[e.jsx("span",{className:"tut-codeblock-lang",children:s}),e.jsxs("button",{type:"button",className:"tut-codeblock-copy",onClick:i,"aria-label":l?"Copied":"Copy code",children:[l?e.jsx(Q,{size:12}):e.jsx(Fe,{size:12}),e.jsx("span",{children:l?"Copied":"Copy"})]})]}),e.jsx("pre",{className:`tut-theory-pre tut-theory-pre-${r||"plain"}`,children:e.jsx("code",{children:t})})]})}function J(n,t){if(!n)return null;const r=n.split(`
`),o=[];let s=[],l=!1,c="",i=[];const g=()=>{if(!s.length)return;const d=s.join(`
`).trim();if(!d){s=[];return}d.split(/\n{2,}/).forEach((p,m)=>{const y=p.trim();if(!y)return;const x=Re(y);if(x){o.push(e.jsx(Ae,{kind:x.kind,children:C(x.text,`${t}-cl-${o.length}`)},`${t}-cl-${o.length}-${m}`));return}o.push(e.jsx("p",{children:C(y,`${t}-p-${o.length}`)},`${t}-p-${o.length}-${m}`))}),s=[]};for(let d=0;d<r.length;d++){const p=r[d],m=/^```(\w*)\s*$/.exec(p);if(m){l?(o.push(e.jsx(G,{lang:c,code:i.join(`
`)},`${t}-pre-${o.length}`)),l=!1,c="",i=[]):(g(),l=!0,c=m[1]||"");continue}if(l){i.push(p);continue}s.push(p)}return l&&o.push(e.jsx(G,{lang:c,code:i.join(`
`)},`${t}-pre-${o.length}`)),g(),o}function Ie({complexity:n}){if(!n)return null;if(typeof n=="string")return e.jsxs("div",{className:"tut-theory-complexity",children:[e.jsxs("span",{className:"tut-theory-cx-label",children:[e.jsx($,{size:11})," Complexity"]}),e.jsx("span",{className:"tut-theory-cx-value",children:C(n,"cx-str")})]});const t=[];return n.time&&t.push(["Time",n.time]),n.build&&t.push(["Build",n.build]),n.operation&&t.push(["Operation",n.operation]),n.best&&t.push(["Best",n.best]),n.average&&t.push(["Average",n.average]),n.worst&&t.push(["Worst",n.worst]),n.space&&t.push(["Space",n.space]),n.notes&&t.push(["Notes",n.notes]),e.jsxs("div",{className:"tut-theory-cx-card",children:[e.jsxs("div",{className:"tut-theory-cx-head",children:[e.jsx($,{size:13}),e.jsx("span",{children:"Complexity"})]}),e.jsxs("table",{className:"tut-theory-cx-table",children:[e.jsx("thead",{children:e.jsxs("tr",{children:[e.jsx("th",{className:"tut-cx-col-op",children:"Operation"}),e.jsx("th",{className:"tut-cx-col-cost",children:"Cost"})]})}),e.jsx("tbody",{children:t.map(([r,o],s)=>e.jsxs("tr",{className:s%2?"tut-cx-row-alt":"",children:[e.jsx("td",{className:"tut-cx-op",children:r}),e.jsx("td",{className:"tut-cx-cost",children:C(o,`cxr-${s}`)})]},s))})]})]})}function _e(n){if(!n)return null;const t=String(n).trim(),r=/^\*\*([^*]+?)\*\*\.?\s*/.exec(t);let o="",s=t;r&&(o=r[1].replace(/\.$/,"").trim(),s=t.slice(r[0].length));const l=s.search(/\b(?:Fix|Remedy|Solution)\s*:/i);let c=s,i="";return l>=0&&(c=s.slice(0,l).trim(),i=s.slice(l).replace(/^\s*(?:Fix|Remedy|Solution)\s*:\s*/i,"").trim()),c=c.replace(/\.\s*$/,"").trim(),{name:o,desc:c,fix:i}}function Ee({items:n}){return!n||!n.length?null:e.jsxs("div",{className:"tut-theory-pitfalls",children:[e.jsxs("div",{className:"tut-theory-pitfalls-head",children:[e.jsx(Se,{size:13}),e.jsx("span",{children:"Pitfalls"}),e.jsx("span",{className:"tut-pitfalls-count",children:n.length})]}),e.jsx("div",{className:"tut-pitfall-grid",children:n.map((t,r)=>{const o=_e(t);if(!o)return null;const{name:s,desc:l,fix:c}=o;return e.jsxs("div",{className:"tut-pitfall-card",children:[e.jsxs("div",{className:"tut-pitfall-head",children:[e.jsx(Y,{size:13}),e.jsx("span",{children:s||`Pitfall ${r+1}`})]}),l&&e.jsx("p",{className:"tut-pitfall-desc",children:C(l,`pfd-${r}`)}),c&&e.jsxs("p",{className:"tut-pitfall-fix",children:[e.jsxs("span",{className:"tut-pitfall-fix-label",children:[e.jsx(Te,{size:11})," Fix"]}),e.jsx("span",{className:"tut-pitfall-fix-text",children:C(c,`pff-${r}`)})]})]},r)})})]})}function ze(n){const t=(n||"").toLowerCase();return t.includes("mental")||t.includes("intuition")?ye:t.includes("canonical")||t.includes("operation")||t.includes("mechanic")?we:t.includes("when to")||t.includes("use case")||t.includes("reach")?ve:t.includes("variant")||t.includes("flavor")?ke:t.includes("problem")||t.includes("interview")||t.includes("example")?xe:R}function H(n){return String(n||"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function We({entries:n,parentId:t}){var c;const[r,o]=f.useState(((c=n[0])==null?void 0:c.id)||null),s=f.useRef(null);if(f.useEffect(()=>{if(!n.length)return;const i=n.map(d=>document.getElementById(`${t}-${d.id}`)).filter(Boolean);if(!i.length)return;const g=new IntersectionObserver(d=>{const p=d.filter(m=>m.isIntersecting).sort((m,y)=>m.boundingClientRect.top-y.boundingClientRect.top);if(p[0]){const m=p[0].target.id.replace(`${t}-`,"");o(m)}},{rootMargin:"-15% 0px -70% 0px",threshold:[0,1]});return i.forEach(d=>g.observe(d)),()=>g.disconnect()},[n,t]),!n.length)return null;const l=i=>g=>{g.preventDefault();const d=document.getElementById(`${t}-${i}`);d&&d.scrollIntoView({behavior:"smooth",block:"start"}),o(i)};return e.jsxs("aside",{className:"tut-section-toc",ref:s,"aria-label":"On this page",children:[e.jsxs("div",{className:"tut-section-toc-head",children:[e.jsx(Ce,{size:11}),e.jsx("span",{children:"On this page"})]}),e.jsx("ol",{className:"tut-section-toc-list",children:n.map(i=>e.jsx("li",{className:r===i.id?"is-active":"",children:e.jsxs("a",{href:`#${t}-${i.id}`,onClick:l(i.id),children:[e.jsx("span",{className:"tut-section-toc-dot"}),e.jsx("span",{className:"tut-section-toc-label",children:i.label})]})},i.id))})]})}function Ve({targetId:n,label:t}){const[r,o]=f.useState(!1),s=l=>{l.preventDefault();const c=`${window.location.origin}${window.location.pathname}${window.location.hash.split("#")[0]||""}#${n}`;navigator.clipboard&&navigator.clipboard.writeText(c).then(()=>{o(!0),window.setTimeout(()=>o(!1),1200)}).catch(()=>{});const i=document.getElementById(n);i&&i.scrollIntoView({behavior:"smooth",block:"start"})};return e.jsx("button",{type:"button",className:"tut-anchor-link",onClick:s,"aria-label":`Copy link to ${t||"section"}`,title:r?"Link copied":"Copy link to section",children:r?e.jsx(Q,{size:12}):e.jsx(Oe,{size:12})})}function X({body:n}){var s,l,c;const r=`tb${f.useId().replace(/:/g,"")}`;if(!n)return null;if(typeof n=="string")return e.jsx("div",{className:"tut-theory-content",children:J(n,"str")});const o=[];return Array.isArray(n.sections)&&n.sections.forEach((i,g)=>{o.push({id:`sec-${g}-${H(i.heading)}`,label:i.heading})}),n.complexity&&o.push({id:"complexity",label:"Complexity"}),(s=n.pitfalls)!=null&&s.length&&o.push({id:"pitfalls",label:"Pitfalls"}),e.jsxs("div",{className:"tut-theory-layout",children:[e.jsxs("div",{className:"tut-theory-content",children:[n.summary&&e.jsx("p",{className:"tut-theory-summary",children:C(n.summary,"sum")}),(l=n.sections)==null?void 0:l.map((i,g)=>{const d=ze(i.heading),p=`${r}-sec-${g}-${H(i.heading)}`;return e.jsxs("section",{id:p,className:"tut-theory-section tut-theory-card",children:[e.jsxs("div",{className:"tut-theory-card-head",children:[e.jsx("span",{className:"tut-theory-card-icon",children:e.jsx(d,{size:13})}),e.jsx("h4",{className:"tut-theory-heading",children:i.heading}),e.jsx(Ve,{targetId:p,label:i.heading})]}),e.jsx("div",{className:"tut-theory-card-body",children:Array.isArray(i.body)?e.jsx("ul",{className:"tut-theory-list",children:i.body.map((m,y)=>e.jsx("li",{children:C(m,`sec-${g}-li-${y}`)},y))}):J(i.body,`sec-${g}`)})]},g)}),n.complexity&&e.jsx("div",{id:`${r}-complexity`,children:e.jsx(Ie,{complexity:n.complexity})}),((c=n.pitfalls)==null?void 0:c.length)>0&&e.jsx("div",{id:`${r}-pitfalls`,children:e.jsx(Ee,{items:n.pitfalls})})]}),e.jsx(We,{entries:o,parentId:r})]})}export{He as default};
