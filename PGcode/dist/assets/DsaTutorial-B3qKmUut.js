...
> Tip: Always allocate \`4·n\` slots for the recursive form — the worst-case tree height plus the off-by-one slack adds up.`},{heading:"When to reach for it",body:"Reach for a segment tree the moment a Fenwick tree (BIT) can't carry the problem. Fenwick handles **prefix sum / xor** under point updates beautifully in 8 lines — use it. Move to segment tree when you need **min / max / gcd** (no inverse, so Fenwick doesn't work), **range updates + range queries** (lazy propagation), arbitrary **associative merges** (matrix product, polynomial composition), or **walking the tree** to answer \"the first index `i` such that `a[i] >= x`\" in `O(log n)`. If the operation is offline and a simple **sort + sweep + Fenwick** would do, prefer that — segment tree is the heavier tool."},{heading:"Variants",body:'**Lazy propagation** — store pending range updates at each node; push down to children only on access. Enables range-add + range-sum (or range-assign + range-min) in `O(log n)`.\n\n**Persistent segment tree** — every update produces a new `O(log n)` path while sharing the rest of the tree. Powers the classic `k`-th smallest in a range via persistent counts on values.\n\n**Merge sort tree** — each node stores its range\'s sorted copy. `O(n log n)` space; answers "how many elements in `[l, r]` are `> x`" in `O(log^2 n)` via binary search per visited node.\n\n**2-D segment tree** — segment tree of segment trees. `O(log^2 n)` per op, `O(n^2)` space; usually overkill.\n\n**Segment tree beats (Chtholly tree)** — handles `chmin` / `chmax` range ops by tracking max / second-max per node.\n\n**Implicit / dynamic segment tree** — allocates children only when touched. Use for coordinate ranges up to `10^9` without coordinate compression.'},{heading:"Common interview problems",body:["Range Sum Query — Mutable (LeetCode 307) — the textbook intro.","Count of Smaller Numbers After Self (LeetCode 315) — merge-sort tree or BIT on compressed values.","My Calendar III (LeetCode 732) — range-add, max query, lazy propagation.","Skyline Problem (LeetCode 218) — segment tree on coordinate-compressed x with lazy max.","Range Module (LeetCode 715) — interval add / remove / query, frequently solved with segment tree.","Falling Squares (LeetCode 699) — range-assign max with lazy propagation.","Number of Longest Increasing Subsequence — segment tree keyed by value."]}],complexity:{build:"`O(n)` bottom-up — every node touched once",operation:"`O(log n)` per point update or range query; `O(log n)` per range update with lazy propagation",time:"Point query / update `O(log n)`; range query / range update `O(log n)`",space:"`O(4n)` for a recursive tree; `O(2n)` for the iterative variant on power-of-two sizes"},pitfalls:["**Off-by-one on the range convention.** Mixing `[l, r]` inclusive with `[l, r)` half-open is the #1 source of bugs. **Fix:** pick the half-open form, document it in a comment at the class top, and stick to it across query, update, and recursion.","**Forgetting to push the lazy tag down before reading children.** Each `query` / `update` that descends into children must first apply the parent's pending update and clear the tag. **Fix:** write a `push(p)` helper and call it as the first line of every recursive method.",'**Reaching for segment tree when Fenwick suffices.** For pure prefix sum or xor with point updates, BIT is 8 lines, twice as fast, and bug-free. **Fix:** ask "do I need min / max / range update / something non-invertible?" If no, write a Fenwick.',"**Under-allocating the tree array.** Sizing at `2n` for the recursive form crashes on non-power-of-two `n`. **Fix:** allocate `4n` for recursive, or round `n` up to the next power of two for iterative.",'**Combining non-associative aggregates.** Segment tree silently breaks if `f` isn\'t associative (e.g., subtraction). **Fix:** verify `f(f(a,b),c) == f(a,f(b,c))` on paper before coding; for "average," store `(sum, count)` and divide at the end.']}}]},{id:"basic-seg",label:"Basic Segment Tree",items:[{kind:"problem",label:"Segment Tree"},{kind:"problem",label:"Min-Max Range Queries"},{kind:"problem",label:"Range LCM Queries"},{kind:"problem",label:"XOR of a given range"}]},{id:"lazy",label:"Segment Tree with Updates",items:[{kind:"problem",label:"Lazy Propagation"},{kind:"problem",label:"Flipping Sign"}]},{id:"app",label:"Segment Tree Applications",items:[{kind:"problem",label:"Build a segment tree for N-ary rooted tree"},{kind:"problem",label:"Maximum of all subarrays of size K using Segment Tree"}]},{id:"fenwick",label:"Fenwick Tree",items:[{kind:"problem",label:"Fenwick Tree"},{kind:"problem",label:"Sum and Update Queries on 3D Matrix"}]}]}];function I(n){return n.subsections.reduce((t,l)=>t+l.items.length,0)}function xe(){return O.reduce((n,t)=>n+I(t),0)}function C(n){return(n||"").toLowerCase().replace(/[^a-z0-9]+/g,"")}function Le({session:n}){var z;const t=(z=n==null?void 0:n.user)==null?void 0:z.id,{data:l=[]}=re(),{data:s=[]}=oe(),{data:r}=se(t),d=f.useMemo(()=>(r==null?void 0:r.byId)||{},[r]),c=f.useMemo(()=>{const a=new Map;return l.forEach(h=>a.set(C(h.name),h)),a},[l]),i=f.useMemo(()=>{const a=new Map;return s.forEach(h=>a.set(C(h.title),h)),a},[s]),m=f.useMemo(()=>{let a=0,h=0,g=0;return O.forEach(y=>{y.subsections.forEach(v=>{v.items.forEach(k=>{var S;if(k.kind==="problem"){a++;const M=c.get(C(k.label));M&&(g++,(S=d[M.id])!=null&&S.is_completed&&h++)}})})}),{totalProblems:a,solved:h,matched:g,totalAll:xe()}},[c,d]),o=f.useMemo(()=>O.map(a=>{let h=0,g=0;return a.subsections.forEach(y=>{y.items.forEach(v=>{var S;if(v.kind!=="problem")return;h+=1;const k=c.get(C(v.label));k&&((S=d[k.id])!=null&&S.is_completed)&&(g+=1)})}),{slug:a.slug,title:a.title,solved:g,total:h,pct:h?g/h:0}}),[c,d]),[u,p]=f.useState(""),b=u.trim().toLowerCase(),[x,w]=f.useState(()=>new Set(O.map(a=>a.slug))),[F,Q]=f.useState("all"),[R,A]=f.useState(!1),Y=a=>w(h=>{const g=new Set(h);return g.has(a)?g.delete(a):g.add(a),g}),Z=()=>w(new Set(O.map(a=>a.slug))),ee=()=>w(new Set),B=f.useMemo(()=>{if(!b)return null;const a=new Map;return O.forEach(h=>{const g=h.subsections.map(y=>{const v=y.items.filter(k=>k.label.toLowerCase().includes(b));return v.length?{...y,items:v}:null}).filter(Boolean);(g.length||h.title.toLowerCase().includes(b))&&a.set(h.slug,g.length?g:h.subsections)}),a},[b]),q=f.useRef(null),[ne,te]=f.useState(!1);f.useEffect(()=>{const a=q.current;if(!a)return;const h=()=>te(a.scrollTop>600);return a.addEventListener("scroll",h),()=>a.removeEventListener("scroll",h)},[]);const ae=()=>{var a;(a=q.current)==null||a.scrollTo({top:0,behavior:"smooth"})},_=a=>{var g;const h=(g=q.current)==null?void 0:g.querySelector(`#tut-${a}`);h&&q.current&&q.current.scrollTo({top:h.offsetTop-20,behavior:"smooth"}),w(y=>{const v=new Set(y);return v.delete(a),v})},E=m.totalProblems?Math.round(m.solved/m.totalProblems*100):0;return e.jsxs("div",{className:"tut-container",ref:q,children:[e.jsxs("header",{className:"tut-header",children:[e.jsxs("div",{className:"tut-title-row",children:[e.jsxs("h1",{className:"tut-title",children:[e.jsx(P,{size:20,className:"tut-title-icon"})," DSA Tutorial"]}),e.jsx("div",{className:"tut-summary",children:t?e.jsxs("span",{children:[m.totalAll," items · ",O.length," topics"]}):e.jsxs("span",{children:[m.totalAll," items · ",O.length," topics"]})})]}),t&&e.jsxs("div",{className:"tut-progress",children:[e.jsxs("div",{className:"tut-progress-meta",children:[e.jsx("span",{className:"tut-progress-label",children:"Your progress"}),e.jsxs("span",{className:"tut-progress-value",children:[e.jsx("strong",{children:m.solved})," / ",m.totalProblems," solved",e.jsxs("span",{className:"tut-progress-pct",children:["· ",E,"%"]})]})]}),e.jsx("div",{className:"tut-progress-bar",children:e.jsx("div",{className:"tut-progress-fill",style:{width:`${E}%`}})})]}),e.jsxs("p",{className:"tut-sub",children:["Data structures and algorithms, top to bottom. Theory rows open the concept page; problem rows open the solver. Items marked ",e.jsx(J,{size:11,className:"tut-inline-icon"})," are coming soon."]})]}),e.jsxs("div",{className:"tut-controls",children:[e.jsxs("div",{className:"tut-search",children:[e.jsx(le,{size:13,className:"tut-search-icon"}),e.jsx("input",{value:u,onChange:a=>p(a.target.value),placeholder:"Search across the entire syllabus..."}),u&&e.jsx("button",{className:"tut-search-clear",onClick:()=>p(""),"aria-label":"Clear",children:e.jsx(V,{size:12})})]}),e.jsx("div",{className:"tut-filters",children:[{id:"all",label:"All"},{id:"theory",label:"Theory"},{id:"problems",label:"Problems"},{id:"unsolved",label:t?"Unsolved":"Available"}].map(a=>e.jsx("button",{type:"button",className:`tut-filter-chip ${F===a.id?"active":""}`,onClick:()=>Q(a.id),children:a.label},a.id))}),e.jsxs("div",{className:"tut-actions",children:[e.jsx("button",{onClick:ee,children:"Expand all"}),e.jsx("button",{onClick:Z,children:"Collapse all"})]})]}),e.jsxs("div",{className:`tut-body ${R?"with-toc":""}`,children:[R&&e.jsx("button",{className:"tut-toc-overlay","aria-label":"Close contents",onClick:()=>A(!1)}),e.jsxs("aside",{className:`tut-toc ${R?"open":""}`,children:[e.jsxs("div",{className:"tut-toc-head",children:[e.jsx("h3",{className:"tut-toc-title",children:"Contents"}),e.jsx("button",{className:"tut-toc-close",onClick:()=>A(!1),"aria-label":"Close",children:e.jsx(V,{size:12})})]}),e.jsx("ol",{className:"tut-toc-list",children:O.map((a,h)=>{const g=I(a);return e.jsx("li",{children:e.jsxs("button",{onClick:()=>{_(a.slug),A(!1)},children:[e.jsx("span",{className:"tut-toc-num",children:String(h+1).padStart(2,"0")}),e.jsx("span",{className:"tut-toc-name",children:a.title}),e.jsx("span",{className:"tut-toc-count",children:g})]})},a.slug)})})]}),e.jsx("main",{className:"tut-main",children:O.map((a,h)=>{const g=B?B.get(a.slug):a.subsections;if(!g)return null;const y=x.has(a.slug)&&!B,v=I(a);return e.jsxs("section",{id:`tut-${a.slug}`,className:"tut-section",children:[e.jsxs("header",{className:"tut-section-head",onClick:()=>Y(a.slug),children:[e.jsx("span",{className:"tut-section-num",children:String(h+1).padStart(2,"0")}),y?e.jsx(D,{size:14}):e.jsx(L,{size:14}),e.jsx("h2",{className:"tut-section-title",children:a.title}),e.jsx("span",{className:"tut-section-count",children:v})]}),a.note&&!y&&e.jsx("p",{className:"tut-section-note",children:a.note}),!y&&g.map(k=>{const S=k.items.filter(M=>Oe(M,F,c,d));return S.length===0?null:e.jsxs("div",{className:"tut-subsection",children:[e.jsx("h3",{className:"tut-subsection-title",children:k.label}),e.jsx("ul",{className:"tut-item-list",children:S.map((M,ie)=>e.jsx(Se,{item:M,problemByName:c,conceptByName:i,byId:d,highlight:b},`${k.id}-${ie}`))})]},k.id)})]},a.slug)})})]}),ne&&e.jsx("button",{className:"tut-back-top",onClick:ae,"aria-label":"Back to top",children:e.jsx(de,{size:14})}),e.jsx("nav",{className:"tut-rail","aria-label":"Sections quick access",children:o.map((a,h)=>{const g=2*Math.PI*13,y=g*a.pct;return e.jsxs("button",{className:`tut-rail-dot ${a.pct===1?"done":""}`,onClick:()=>_(a.slug),title:`${a.title} — ${a.solved}/${a.total} solved`,"aria-label":`Jump to ${a.title}, ${a.solved} of ${a.total} solved`,children:[e.jsxs("svg",{viewBox:"0 0 32 32",className:"tut-rail-svg",children:[e.jsx("circle",{cx:"16",cy:"16",r:"13",className:"tut-rail-bg"}),e.jsx("circle",{cx:"16",cy:"16",r:"13",className:"tut-rail-fg",strokeDasharray:`${y} ${g-y}`,transform:"rotate(-90 16 16)"})]}),e.jsx("span",{className:"tut-rail-num",children:h+1})]},a.slug)})})]})}function Oe(n,t,l,s){var r;if(t==="all")return!0;if(t==="theory")return n.kind==="theory"||n.kind==="topic";if(t==="problems")return n.kind==="problem";if(t==="unsolved"){if(n.kind!=="problem")return!1;const d=i=>(i||"").toLowerCase().replace(/[^a-z0-9]+/g,""),c=l.get(d(n.label));return c?!((r=s[c.id])!=null&&r.is_completed):!1}return!0}function j(n,t){if(!t)return n;const s=n.toLowerCase().indexOf(t);return s===-1?n:e.jsxs(e.Fragment,{children:[n.slice(0,s),e.jsx("mark",{children:n.slice(s,s+t.length)}),n.slice(s+t.length)]})}function Se({item:n,problemByName:t,conceptByName:l,byId:s,highlight:r}){var m;const[d,c]=f.useState(!1);if(n.kind==="topic"){const o=n.conceptSlug?null:l.get(C(n.label)),u=n.conceptSlug||(o==null?void 0:o.slug);let p=o==null?void 0:o.module_slug;if(u&&!p){const w=[...l.values()].find(F=>F.slug===u);w&&(p=w.module_slug)}const b=!!n.body,x=!!(u&&p);return e.jsxs("li",{className:`tut-item-theory-wrap tut-item-topic-wrap ${d?"expanded":""}`,children:[e.jsxs("button",{type:"button",className:"tut-item tut-item-topic tut-item-theory-button",onClick:()=>c(w=>!w),"aria-expanded":d,children:[e.jsx("span",{className:"tut-item-icon",children:d?e.jsx(L,{size:11}):e.jsx(D,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,r)}),e.jsx("span",{className:"tut-item-kind",children:"topic"})]}),d&&e.jsxs("div",{className:"tut-theory-body",children:[b&&e.jsx(H,{body:n.body}),x&&e.jsxs(N,{to:`/learn/${p}/${u}`,className:"tut-theory-readmore",children:[e.jsx(P,{size:11})," Open full concept page",e.jsx(K,{size:10})]}),!b&&!x&&e.jsxs("p",{className:"tut-theory-placeholder",children:[e.jsx(ce,{size:10,className:"tut-inline-icon"}),"See the related concepts above for in-depth coverage."]})]})]})}if(n.kind==="theory"){const o=n.conceptSlug?null:l.get(C(n.label)),u=n.conceptSlug||(o==null?void 0:o.slug);let p=o==null?void 0:o.module_slug;if(u&&!p){const w=[...l.values()].find(F=>F.slug===u);w&&(p=w.module_slug)}const b=!!n.body,x=!!(u&&p);return b?e.jsxs("li",{className:`tut-item-theory-wrap ${d?"expanded":""}`,children:[e.jsxs("button",{type:"button",className:"tut-item tut-item-theory tut-item-theory-button",onClick:()=>c(w=>!w),"aria-expanded":d,children:[e.jsx("span",{className:"tut-item-icon",children:d?e.jsx(L,{size:11}):e.jsx(D,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,r)}),e.jsx("span",{className:"tut-item-kind",children:"theory"})]}),d&&e.jsxs("div",{className:"tut-theory-body",children:[e.jsx(H,{body:n.body}),x&&e.jsxs(N,{to:`/learn/${p}/${u}`,className:"tut-theory-readmore",children:[e.jsx(P,{size:11})," Open full concept page",e.jsx(K,{size:10})]})]})]}):x?e.jsx("li",{className:"tut-item tut-item-theory",children:e.jsxs(N,{to:`/learn/${p}/${u}`,className:"tut-item-link",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(P,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,r)}),e.jsx("span",{className:"tut-item-kind",children:"theory"})]})}):e.jsxs("li",{className:"tut-item tut-item-theory-soft",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(P,{size:11})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,r)}),e.jsx("span",{className:"tut-item-kind",children:"theory · soon"})]})}const i=n.id?null:t.get(C(n.label));if(i){const o=s[i.id],u=o==null?void 0:o.is_completed,p=!u&&((o==null?void 0:o.status)==="attempted"||(o==null?void 0:o.is_starred)),b=u?"solved":p?"attempted":"todo";return e.jsx("li",{className:`tut-item ${u?"solved":""}`,children:e.jsxs(N,{to:`/category/${encodeURIComponent(i.topic_id)}/${encodeURIComponent(i.id)}`,className:"tut-item-link",children:[e.jsx("span",{className:`tut-bubble tut-bubble-${b}`,"aria-label":u?"solved":p?"attempted":"not started",children:u&&e.jsx(he,{size:9})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,r)}),e.jsx("span",{className:`tut-item-diff tut-diff-${(m=i.difficulty)==null?void 0:m.toLowerCase()}`,children:i.difficulty})]})})}return e.jsxs("li",{className:"tut-item tut-item-soon",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(J,{size:10})}),e.jsx("span",{className:"tut-item-label",children:j(n.label,r)}),e.jsx("span",{className:"tut-item-kind",children:"soon"})]})}function T(n,t=""){if(n==null)return null;const l=String(n),s=/(`[^`]+`)|(\[[^\]]+\]\([^)]+\))|(\*\*[^*]+\*\*)|(\*[^*]+\*)/g,r=[];let d=0,c=0,i;for(;(i=s.exec(l))!==null;){if(i.index>d&&r.push(e.jsx(W.Fragment,{children:l.slice(d,i.index)},`${t}-t-${c++}`)),i[1])r.push(e.jsx("code",{className:"tut-theory-code",children:i[1].slice(1,-1)},`${t}-c-${c++}`));else if(i[2]){const m=/\[([^\]]+)\]\(([^)]+)\)/.exec(i[2]);r.push(e.jsx("a",{href:m[2],target:"_blank",rel:"noopener noreferrer",children:m[1]},`${t}-a-${c++}`))}else i[3]?r.push(e.jsx("strong",{children:i[3].slice(2,-2)},`${t}-b-${c++}`)):i[4]&&r.push(e.jsx("em",{children:i[4].slice(1,-1)},`${t}-i-${c++}`));d=i.index+i[0].length}return d<l.length&&r.push(e.jsx(W.Fragment,{children:l.slice(d)},`${t}-t-${c++}`)),r.length===0?l:r}function Te(n){const t=/^>\s*(Note|Warning|Tip|Insight|Caution):\s*(.*)$/i.exec(n.trim());return t?{kind:t[1].toLowerCase(),text:t[2]}:null}function Ce({kind:n,children:t}){const l=n==="warning"||n==="caution"?e.jsx(X,{size:13}):n==="tip"||n==="insight"?e.jsx(ve,{size:13}):e.jsx(ke,{size:13});return e.jsxs("aside",{className:`tut-callout tut-callout-${n}`,children:[e.jsx("span",{className:"tut-callout-icon",children:l}),e.jsx("span",{className:"tut-callout-body",children:t})]})}function $(n,t){if(!n)return null;const l=n.split(`
`),s=[];let r=[],d=!1,c="",i=[];const m=()=>{if(!r.length)return;const o=r.join(`
`).trim();if(!o){r=[];return}o.split(/\n{2,}/).forEach((u,p)=>{const b=u.trim();if(!b)return;const x=Te(b);if(x){s.push(e.jsx(Ce,{kind:x.kind,children:T(x.text,`${t}-cl-${s.length}`)},`${t}-cl-${s.length}-${p}`));return}s.push(e.jsx("p",{children:T(b,`${t}-p-${s.length}`)},`${t}-p-${s.length}-${p}`))}),r=[]};for(let o=0;o<l.length;o++){const u=l[o],p=/^```(\w*)\s*$/.exec(u);if(p){d?(s.push(e.jsx("pre",{className:`tut-theory-pre tut-theory-pre-${c||"plain"}`,children:e.jsx("code",{children:i.join(`
`)})},`${t}-pre-${s.length}`)),d=!1,c="",i=[]):(m(),d=!0,c=p[1]||"");continue}if(d){i.push(u);continue}r.push(u)}return d&&s.push(e.jsx("pre",{className:`tut-theory-pre tut-theory-pre-${c||"plain"}`,children:e.jsx("code",{children:i.join(`
`)})},`${t}-pre-${s.length}`)),m(),s}function Fe({complexity:n}){if(!n)return null;if(typeof n=="string")return e.jsxs("div",{className:"tut-theory-complexity",children:[e.jsxs("span",{className:"tut-theory-cx-label",children:[e.jsx(U,{size:11})," Complexity"]}),e.jsx("span",{className:"tut-theory-cx-value",children:T(n,"cx-str")})]});const t=[];return n.time&&t.push(["Time",n.time]),n.build&&t.push(["Build",n.build]),n.operation&&t.push(["Operation",n.operation]),n.best&&t.push(["Best",n.best]),n.average&&t.push(["Average",n.average]),n.worst&&t.push(["Worst",n.worst]),n.space&&t.push(["Space",n.space]),n.notes&&t.push(["Notes",n.notes]),e.jsxs("div",{className:"tut-theory-cx-card",children:[e.jsxs("div",{className:"tut-theory-cx-head",children:[e.jsx(U,{size:13}),e.jsx("span",{children:"Complexity"})]}),e.jsxs("table",{className:"tut-theory-cx-table",children:[e.jsx("thead",{children:e.jsxs("tr",{children:[e.jsx("th",{className:"tut-cx-col-op",children:"Operation"}),e.jsx("th",{className:"tut-cx-col-cost",children:"Cost"})]})}),e.jsx("tbody",{children:t.map(([l,s],r)=>e.jsxs("tr",{className:r%2?"tut-cx-row-alt":"",children:[e.jsx("td",{className:"tut-cx-op",children:l}),e.jsx("td",{className:"tut-cx-cost",children:T(s,`cxr-${r}`)})]},r))})]})]})}function qe(n){if(!n)return null;const t=String(n).trim(),l=/^\*\*([^*]+?)\*\*\.?\s*/.exec(t);let s="",r=t;l&&(s=l[1].replace(/\.$/,"").trim(),r=t.slice(l[0].length));const d=r.search(/\b(?:Fix|Remedy|Solution)\s*:/i);let c=r,i="";return d>=0&&(c=r.slice(0,d).trim(),i=r.slice(d).replace(/^\s*(?:Fix|Remedy|Solution)\s*:\s*/i,"").trim()),c=c.replace(/\.\s*$/,"").trim(),{name:s,desc:c,fix:i}}function Me({items:n}){return!n||!n.length?null:e.jsxs("div",{className:"tut-theory-pitfalls",children:[e.jsxs("div",{className:"tut-theory-pitfalls-head",children:[e.jsx(fe,{size:13}),e.jsx("span",{children:"Pitfalls"}),e.jsx("span",{className:"tut-pitfalls-count",children:n.length})]}),e.jsx("div",{className:"tut-pitfall-grid",children:n.map((t,l)=>{const s=qe(t);if(!s)return null;const{name:r,desc:d,fix:c}=s;return e.jsxs("div",{className:"tut-pitfall-card",children:[e.jsxs("div",{className:"tut-pitfall-head",children:[e.jsx(X,{size:13}),e.jsx("span",{children:r||`Pitfall ${l+1}`})]}),d&&e.jsx("p",{className:"tut-pitfall-desc",children:T(d,`pfd-${l}`)}),c&&e.jsxs("p",{className:"tut-pitfall-fix",children:[e.jsxs("span",{className:"tut-pitfall-fix-label",children:[e.jsx(ye,{size:11})," Fix"]}),e.jsx("span",{className:"tut-pitfall-fix-text",children:T(c,`pff-${l}`)})]})]},l)})})]})}function je(n){const t=(n||"").toLowerCase();return t.includes("mental")||t.includes("intuition")?ue:t.includes("canonical")||t.includes("operation")||t.includes("mechanic")?pe:t.includes("when to")||t.includes("use case")||t.includes("reach")?me:t.includes("variant")||t.includes("flavor")?ge:t.includes("problem")||t.includes("interview")||t.includes("example")?be:D}function G(n){return String(n||"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function Pe({entries:n,parentId:t}){var c;const[l,s]=f.useState(((c=n[0])==null?void 0:c.id)||null),r=f.useRef(null);if(f.useEffect(()=>{if(!n.length)return;const i=n.map(o=>document.getElementById(`${t}-${o.id}`)).filter(Boolean);if(!i.length)return;const m=new IntersectionObserver(o=>{const u=o.filter(p=>p.isIntersecting).sort((p,b)=>p.boundingClientRect.top-b.boundingClientRect.top);if(u[0]){const p=u[0].target.id.replace(`${t}-`,"");s(p)}},{rootMargin:"-15% 0px -70% 0px",threshold:[0,1]});return i.forEach(o=>m.observe(o)),()=>m.disconnect()},[n,t]),!n.length)return null;const d=i=>m=>{m.preventDefault();const o=document.getElementById(`${t}-${i}`);o&&o.scrollIntoView({behavior:"smooth",block:"start"}),s(i)};return e.jsxs("aside",{className:"tut-section-toc",ref:r,"aria-label":"On this page",children:[e.jsxs("div",{className:"tut-section-toc-head",children:[e.jsx(we,{size:11}),e.jsx("span",{children:"On this page"})]}),e.jsx("ol",{className:"tut-section-toc-list",children:n.map(i=>e.jsx("li",{className:l===i.id?"is-active":"",children:e.jsxs("a",{href:`#${t}-${i.id}`,onClick:d(i.id),children:[e.jsx("span",{className:"tut-section-toc-dot"}),e.jsx("span",{className:"tut-section-toc-label",children:i.label})]})},i.id))})]})}function H({body:n}){var r,d,c;const l=`tb${f.useId().replace(/:/g,"")}`;if(!n)return null;if(typeof n=="string")return e.jsx("div",{className:"tut-theory-content",children:$(n,"str")});const s=[];return Array.isArray(n.sections)&&n.sections.forEach((i,m)=>{s.push({id:`sec-${m}-${G(i.heading)}`,label:i.heading})}),n.complexity&&s.push({id:"complexity",label:"Complexity"}),(r=n.pitfalls)!=null&&r.length&&s.push({id:"pitfalls",label:"Pitfalls"}),e.jsxs("div",{className:"tut-theory-layout",children:[e.jsxs("div",{className:"tut-theory-content",children:[n.summary&&e.jsx("p",{className:"tut-theory-summary",children:T(n.summary,"sum")}),(d=n.sections)==null?void 0:d.map((i,m)=>{const o=je(i.heading),u=`${l}-sec-${m}-${G(i.heading)}`;return e.jsxs("section",{id:u,className:"tut-theory-section tut-theory-card",children:[e.jsxs("div",{className:"tut-theory-card-head",children:[e.jsx("span",{className:"tut-theory-card-icon",children:e.jsx(o,{size:13})}),e.jsx("h4",{className:"tut-theory-heading",children:i.heading})]}),e.jsx("div",{className:"tut-theory-card-body",children:Array.isArray(i.body)?e.jsx("ul",{className:"tut-theory-list",children:i.body.map((p,b)=>e.jsx("li",{children:T(p,`sec-${m}-li-${b}`)},b))}):$(i.body,`sec-${m}`)})]},m)}),n.complexity&&e.jsx("div",{id:`${l}-complexity`,children:e.jsx(Fe,{complexity:n.complexity})}),((c=n.pitfalls)==null?void 0:c.length)>0&&e.jsx("div",{id:`${l}-pitfalls`,children:e.jsx(Me,{items:n.pitfalls})})]}),e.jsx(Pe,{entries:s,parentId:l})]})}export{Le as default};
