...
> Tip: Always allocate \`4·n\` slots for the recursive form — the worst-case tree height plus the off-by-one slack adds up.`},{heading:"When to reach for it",body:"Reach for a segment tree the moment a Fenwick tree (BIT) can't carry the problem. Fenwick handles **prefix sum / xor** under point updates beautifully in 8 lines — use it. Move to segment tree when you need **min / max / gcd** (no inverse, so Fenwick doesn't work), **range updates + range queries** (lazy propagation), arbitrary **associative merges** (matrix product, polynomial composition), or **walking the tree** to answer \"the first index `i` such that `a[i] >= x`\" in `O(log n)`. If the operation is offline and a simple **sort + sweep + Fenwick** would do, prefer that — segment tree is the heavier tool."},{heading:"Variants",body:'**Lazy propagation** — store pending range updates at each node; push down to children only on access. Enables range-add + range-sum (or range-assign + range-min) in `O(log n)`.\n\n**Persistent segment tree** — every update produces a new `O(log n)` path while sharing the rest of the tree. Powers the classic `k`-th smallest in a range via persistent counts on values.\n\n**Merge sort tree** — each node stores its range\'s sorted copy. `O(n log n)` space; answers "how many elements in `[l, r]` are `> x`" in `O(log^2 n)` via binary search per visited node.\n\n**2-D segment tree** — segment tree of segment trees. `O(log^2 n)` per op, `O(n^2)` space; usually overkill.\n\n**Segment tree beats (Chtholly tree)** — handles `chmin` / `chmax` range ops by tracking max / second-max per node.\n\n**Implicit / dynamic segment tree** — allocates children only when touched. Use for coordinate ranges up to `10^9` without coordinate compression.'},{heading:"Common interview problems",body:["Range Sum Query — Mutable (LeetCode 307) — the textbook intro.","Count of Smaller Numbers After Self (LeetCode 315) — merge-sort tree or BIT on compressed values.","My Calendar III (LeetCode 732) — range-add, max query, lazy propagation.","Skyline Problem (LeetCode 218) — segment tree on coordinate-compressed x with lazy max.","Range Module (LeetCode 715) — interval add / remove / query, frequently solved with segment tree.","Falling Squares (LeetCode 699) — range-assign max with lazy propagation.","Number of Longest Increasing Subsequence — segment tree keyed by value."]}],complexity:{build:"`O(n)` bottom-up — every node touched once",operation:"`O(log n)` per point update or range query; `O(log n)` per range update with lazy propagation",time:"Point query / update `O(log n)`; range query / range update `O(log n)`",space:"`O(4n)` for a recursive tree; `O(2n)` for the iterative variant on power-of-two sizes"},pitfalls:["**Off-by-one on the range convention.** Mixing `[l, r]` inclusive with `[l, r)` half-open is the #1 source of bugs. **Fix:** pick the half-open form, document it in a comment at the class top, and stick to it across query, update, and recursion.","**Forgetting to push the lazy tag down before reading children.** Each `query` / `update` that descends into children must first apply the parent's pending update and clear the tag. **Fix:** write a `push(p)` helper and call it as the first line of every recursive method.",'**Reaching for segment tree when Fenwick suffices.** For pure prefix sum or xor with point updates, BIT is 8 lines, twice as fast, and bug-free. **Fix:** ask "do I need min / max / range update / something non-invertible?" If no, write a Fenwick.',"**Under-allocating the tree array.** Sizing at `2n` for the recursive form crashes on non-power-of-two `n`. **Fix:** allocate `4n` for recursive, or round `n` up to the next power of two for iterative.",'**Combining non-associative aggregates.** Segment tree silently breaks if `f` isn\'t associative (e.g., subtraction). **Fix:** verify `f(f(a,b),c) == f(a,f(b,c))` on paper before coding; for "average," store `(sum, count)` and divide at the end.']}}]},{id:"basic-seg",label:"Basic Segment Tree",items:[{kind:"problem",label:"Segment Tree"},{kind:"problem",label:"Min-Max Range Queries"},{kind:"problem",label:"Range LCM Queries"},{kind:"problem",label:"XOR of a given range"}]},{id:"lazy",label:"Segment Tree with Updates",items:[{kind:"problem",label:"Lazy Propagation"},{kind:"problem",label:"Flipping Sign"}]},{id:"app",label:"Segment Tree Applications",items:[{kind:"problem",label:"Build a segment tree for N-ary rooted tree"},{kind:"problem",label:"Maximum of all subarrays of size K using Segment Tree"}]},{id:"fenwick",label:"Fenwick Tree",items:[{kind:"problem",label:"Fenwick Tree"},{kind:"problem",label:"Sum and Update Queries on 3D Matrix"}]}]}];function I(n){return n.subsections.reduce((a,r)=>a+r.items.length,0)}function me(){return O.reduce((n,a)=>n+I(a),0)}function T(n){return(n||"").toLowerCase().replace(/[^a-z0-9]+/g,"")}function Ce({session:n}){var z;const a=(z=n==null?void 0:n.user)==null?void 0:z.id,{data:r=[]}=ae(),{data:o=[]}=ie(),{data:i}=re(a),s=w.useMemo(()=>(i==null?void 0:i.byId)||{},[i]),p=w.useMemo(()=>{const t=new Map;return r.forEach(d=>t.set(T(d.name),d)),t},[r]),h=w.useMemo(()=>{const t=new Map;return o.forEach(d=>t.set(T(d.title),d)),t},[o]),g=w.useMemo(()=>{let t=0,d=0,u=0;return O.forEach(f=>{f.subsections.forEach(v=>{v.items.forEach(k=>{var S;if(k.kind==="problem"){t++;const M=p.get(T(k.label));M&&(u++,(S=s[M.id])!=null&&S.is_completed&&d++)}})})}),{totalProblems:t,solved:d,matched:u,totalAll:me()}},[p,s]),l=w.useMemo(()=>O.map(t=>{let d=0,u=0;return t.subsections.forEach(f=>{f.items.forEach(v=>{var S;if(v.kind!=="problem")return;d+=1;const k=p.get(T(v.label));k&&((S=s[k.id])!=null&&S.is_completed)&&(u+=1)})}),{slug:t.slug,title:t.title,solved:u,total:d,pct:d?u/d:0}}),[p,s]),[c,m]=w.useState(""),b=c.trim().toLowerCase(),[x,y]=w.useState(()=>new Set(O.map(t=>t.slug))),[F,X]=w.useState("all"),[N,R]=w.useState(!1),$=t=>y(d=>{const u=new Set(d);return u.has(t)?u.delete(t):u.add(t),u}),Q=()=>y(new Set(O.map(t=>t.slug))),Y=()=>y(new Set),A=w.useMemo(()=>{if(!b)return null;const t=new Map;return O.forEach(d=>{const u=d.subsections.map(f=>{const v=f.items.filter(k=>k.label.toLowerCase().includes(b));return v.length?{...f,items:v}:null}).filter(Boolean);(u.length||d.title.toLowerCase().includes(b))&&t.set(d.slug,u.length?u:d.subsections)}),t},[b]),q=w.useRef(null),[Z,ee]=w.useState(!1);w.useEffect(()=>{const t=q.current;if(!t)return;const d=()=>ee(t.scrollTop>600);return t.addEventListener("scroll",d),()=>t.removeEventListener("scroll",d)},[]);const ne=()=>{var t;(t=q.current)==null||t.scrollTo({top:0,behavior:"smooth"})},_=t=>{var u;const d=(u=q.current)==null?void 0:u.querySelector(`#tut-${t}`);d&&q.current&&q.current.scrollTo({top:d.offsetTop-20,behavior:"smooth"}),y(f=>{const v=new Set(f);return v.delete(t),v})},E=g.totalProblems?Math.round(g.solved/g.totalProblems*100):0;return e.jsxs("div",{className:"tut-container",ref:q,children:[e.jsxs("header",{className:"tut-header",children:[e.jsxs("div",{className:"tut-title-row",children:[e.jsxs("h1",{className:"tut-title",children:[e.jsx(j,{size:20,className:"tut-title-icon"})," DSA Tutorial"]}),e.jsx("div",{className:"tut-summary",children:a?e.jsxs("span",{children:[g.totalAll," items · ",O.length," topics"]}):e.jsxs("span",{children:[g.totalAll," items · ",O.length," topics"]})})]}),a&&e.jsxs("div",{className:"tut-progress",children:[e.jsxs("div",{className:"tut-progress-meta",children:[e.jsx("span",{className:"tut-progress-label",children:"Your progress"}),e.jsxs("span",{className:"tut-progress-value",children:[e.jsx("strong",{children:g.solved})," / ",g.totalProblems," solved",e.jsxs("span",{className:"tut-progress-pct",children:["· ",E,"%"]})]})]}),e.jsx("div",{className:"tut-progress-bar",children:e.jsx("div",{className:"tut-progress-fill",style:{width:`${E}%`}})})]}),e.jsxs("p",{className:"tut-sub",children:["Data structures and algorithms, top to bottom. Theory rows open the concept page; problem rows open the solver. Items marked ",e.jsx(H,{size:11,className:"tut-inline-icon"})," are coming soon."]})]}),e.jsxs("div",{className:"tut-controls",children:[e.jsxs("div",{className:"tut-search",children:[e.jsx(oe,{size:13,className:"tut-search-icon"}),e.jsx("input",{value:c,onChange:t=>m(t.target.value),placeholder:"Search across the entire syllabus..."}),c&&e.jsx("button",{className:"tut-search-clear",onClick:()=>m(""),"aria-label":"Clear",children:e.jsx(V,{size:12})})]}),e.jsx("div",{className:"tut-filters",children:[{id:"all",label:"All"},{id:"theory",label:"Theory"},{id:"problems",label:"Problems"},{id:"unsolved",label:a?"Unsolved":"Available"}].map(t=>e.jsx("button",{type:"button",className:`tut-filter-chip ${F===t.id?"active":""}`,onClick:()=>X(t.id),children:t.label},t.id))}),e.jsxs("div",{className:"tut-actions",children:[e.jsx("button",{onClick:Y,children:"Expand all"}),e.jsx("button",{onClick:Q,children:"Collapse all"})]})]}),e.jsxs("div",{className:`tut-body ${N?"with-toc":""}`,children:[N&&e.jsx("button",{className:"tut-toc-overlay","aria-label":"Close contents",onClick:()=>R(!1)}),e.jsxs("aside",{className:`tut-toc ${N?"open":""}`,children:[e.jsxs("div",{className:"tut-toc-head",children:[e.jsx("h3",{className:"tut-toc-title",children:"Contents"}),e.jsx("button",{className:"tut-toc-close",onClick:()=>R(!1),"aria-label":"Close",children:e.jsx(V,{size:12})})]}),e.jsx("ol",{className:"tut-toc-list",children:O.map((t,d)=>{const u=I(t);return e.jsx("li",{children:e.jsxs("button",{onClick:()=>{_(t.slug),R(!1)},children:[e.jsx("span",{className:"tut-toc-num",children:String(d+1).padStart(2,"0")}),e.jsx("span",{className:"tut-toc-name",children:t.title}),e.jsx("span",{className:"tut-toc-count",children:u})]})},t.slug)})})]}),e.jsx("main",{className:"tut-main",children:O.map((t,d)=>{const u=A?A.get(t.slug):t.subsections;if(!u)return null;const f=x.has(t.slug)&&!A,v=I(t);return e.jsxs("section",{id:`tut-${t.slug}`,className:"tut-section",children:[e.jsxs("header",{className:"tut-section-head",onClick:()=>$(t.slug),children:[e.jsx("span",{className:"tut-section-num",children:String(d+1).padStart(2,"0")}),f?e.jsx(B,{size:14}):e.jsx(L,{size:14}),e.jsx("h2",{className:"tut-section-title",children:t.title}),e.jsx("span",{className:"tut-section-count",children:v})]}),t.note&&!f&&e.jsx("p",{className:"tut-section-note",children:t.note}),!f&&u.map(k=>{const S=k.items.filter(M=>ge(M,F,p,s));return S.length===0?null:e.jsxs("div",{className:"tut-subsection",children:[e.jsx("h3",{className:"tut-subsection-title",children:k.label}),e.jsx("ul",{className:"tut-item-list",children:S.map((M,te)=>e.jsx(be,{item:M,problemByName:p,conceptByName:h,byId:s,highlight:b},`${k.id}-${te}`))})]},k.id)})]},t.slug)})})]}),Z&&e.jsx("button",{className:"tut-back-top",onClick:ne,"aria-label":"Back to top",children:e.jsx(se,{size:14})}),e.jsx("nav",{className:"tut-rail","aria-label":"Sections quick access",children:l.map((t,d)=>{const u=2*Math.PI*13,f=u*t.pct;return e.jsxs("button",{className:`tut-rail-dot ${t.pct===1?"done":""}`,onClick:()=>_(t.slug),title:`${t.title} — ${t.solved}/${t.total} solved`,"aria-label":`Jump to ${t.title}, ${t.solved} of ${t.total} solved`,children:[e.jsxs("svg",{viewBox:"0 0 32 32",className:"tut-rail-svg",children:[e.jsx("circle",{cx:"16",cy:"16",r:"13",className:"tut-rail-bg"}),e.jsx("circle",{cx:"16",cy:"16",r:"13",className:"tut-rail-fg",strokeDasharray:`${f} ${u-f}`,transform:"rotate(-90 16 16)"})]}),e.jsx("span",{className:"tut-rail-num",children:d+1})]},t.slug)})})]})}function ge(n,a,r,o){var i;if(a==="all")return!0;if(a==="theory")return n.kind==="theory"||n.kind==="topic";if(a==="problems")return n.kind==="problem";if(a==="unsolved"){if(n.kind!=="problem")return!1;const s=h=>(h||"").toLowerCase().replace(/[^a-z0-9]+/g,""),p=r.get(s(n.label));return p?!((i=o[p.id])!=null&&i.is_completed):!1}return!0}function P(n,a){if(!a)return n;const o=n.toLowerCase().indexOf(a);return o===-1?n:e.jsxs(e.Fragment,{children:[n.slice(0,o),e.jsx("mark",{children:n.slice(o,o+a.length)}),n.slice(o+a.length)]})}function be({item:n,problemByName:a,conceptByName:r,byId:o,highlight:i}){var g;const[s,p]=w.useState(!1);if(n.kind==="topic"){const l=n.conceptSlug?null:r.get(T(n.label)),c=n.conceptSlug||(l==null?void 0:l.slug);let m=l==null?void 0:l.module_slug;if(c&&!m){const y=[...r.values()].find(F=>F.slug===c);y&&(m=y.module_slug)}const b=!!n.body,x=!!(c&&m);return e.jsxs("li",{className:`tut-item-theory-wrap tut-item-topic-wrap ${s?"expanded":""}`,children:[e.jsxs("button",{type:"button",className:"tut-item tut-item-topic tut-item-theory-button",onClick:()=>p(y=>!y),"aria-expanded":s,children:[e.jsx("span",{className:"tut-item-icon",children:s?e.jsx(L,{size:11}):e.jsx(B,{size:11})}),e.jsx("span",{className:"tut-item-label",children:P(n.label,i)}),e.jsx("span",{className:"tut-item-kind",children:"topic"})]}),s&&e.jsxs("div",{className:"tut-theory-body",children:[b&&e.jsx(G,{body:n.body}),x&&e.jsxs(D,{to:`/learn/${m}/${c}`,className:"tut-theory-readmore",children:[e.jsx(j,{size:11})," Open full concept page",e.jsx(K,{size:10})]}),!b&&!x&&e.jsxs("p",{className:"tut-theory-placeholder",children:[e.jsx(le,{size:10,className:"tut-inline-icon"}),"See the related concepts above for in-depth coverage."]})]})]})}if(n.kind==="theory"){const l=n.conceptSlug?null:r.get(T(n.label)),c=n.conceptSlug||(l==null?void 0:l.slug);let m=l==null?void 0:l.module_slug;if(c&&!m){const y=[...r.values()].find(F=>F.slug===c);y&&(m=y.module_slug)}const b=!!n.body,x=!!(c&&m);return b?e.jsxs("li",{className:`tut-item-theory-wrap ${s?"expanded":""}`,children:[e.jsxs("button",{type:"button",className:"tut-item tut-item-theory tut-item-theory-button",onClick:()=>p(y=>!y),"aria-expanded":s,children:[e.jsx("span",{className:"tut-item-icon",children:s?e.jsx(L,{size:11}):e.jsx(B,{size:11})}),e.jsx("span",{className:"tut-item-label",children:P(n.label,i)}),e.jsx("span",{className:"tut-item-kind",children:"theory"})]}),s&&e.jsxs("div",{className:"tut-theory-body",children:[e.jsx(G,{body:n.body}),x&&e.jsxs(D,{to:`/learn/${m}/${c}`,className:"tut-theory-readmore",children:[e.jsx(j,{size:11})," Open full concept page",e.jsx(K,{size:10})]})]})]}):x?e.jsx("li",{className:"tut-item tut-item-theory",children:e.jsxs(D,{to:`/learn/${m}/${c}`,className:"tut-item-link",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(j,{size:11})}),e.jsx("span",{className:"tut-item-label",children:P(n.label,i)}),e.jsx("span",{className:"tut-item-kind",children:"theory"})]})}):e.jsxs("li",{className:"tut-item tut-item-theory-soft",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(j,{size:11})}),e.jsx("span",{className:"tut-item-label",children:P(n.label,i)}),e.jsx("span",{className:"tut-item-kind",children:"theory · soon"})]})}const h=n.id?null:a.get(T(n.label));if(h){const l=o[h.id],c=l==null?void 0:l.is_completed,m=!c&&((l==null?void 0:l.status)==="attempted"||(l==null?void 0:l.is_starred)),b=c?"solved":m?"attempted":"todo";return e.jsx("li",{className:`tut-item ${c?"solved":""}`,children:e.jsxs(D,{to:`/category/${encodeURIComponent(h.topic_id)}/${encodeURIComponent(h.id)}`,className:"tut-item-link",children:[e.jsx("span",{className:`tut-bubble tut-bubble-${b}`,"aria-label":c?"solved":m?"attempted":"not started",children:c&&e.jsx(de,{size:9})}),e.jsx("span",{className:"tut-item-label",children:P(n.label,i)}),e.jsx("span",{className:`tut-item-diff tut-diff-${(g=h.difficulty)==null?void 0:g.toLowerCase()}`,children:h.difficulty})]})})}return e.jsxs("li",{className:"tut-item tut-item-soon",children:[e.jsx("span",{className:"tut-item-icon",children:e.jsx(H,{size:10})}),e.jsx("span",{className:"tut-item-label",children:P(n.label,i)}),e.jsx("span",{className:"tut-item-kind",children:"soon"})]})}function C(n,a=""){if(n==null)return null;const r=String(n),o=/(`[^`]+`)|(\[[^\]]+\]\([^)]+\))|(\*\*[^*]+\*\*)|(\*[^*]+\*)/g,i=[];let s=0,p=0,h;for(;(h=o.exec(r))!==null;){if(h.index>s&&i.push(e.jsx(W.Fragment,{children:r.slice(s,h.index)},`${a}-t-${p++}`)),h[1])i.push(e.jsx("code",{className:"tut-theory-code",children:h[1].slice(1,-1)},`${a}-c-${p++}`));else if(h[2]){const g=/\[([^\]]+)\]\(([^)]+)\)/.exec(h[2]);i.push(e.jsx("a",{href:g[2],target:"_blank",rel:"noopener noreferrer",children:g[1]},`${a}-a-${p++}`))}else h[3]?i.push(e.jsx("strong",{children:h[3].slice(2,-2)},`${a}-b-${p++}`)):h[4]&&i.push(e.jsx("em",{children:h[4].slice(1,-1)},`${a}-i-${p++}`));s=h.index+h[0].length}return s<r.length&&i.push(e.jsx(W.Fragment,{children:r.slice(s)},`${a}-t-${p++}`)),i.length===0?r:i}function fe(n){const a=/^>\s*(Note|Warning|Tip|Insight|Caution):\s*(.*)$/i.exec(n.trim());return a?{kind:a[1].toLowerCase(),text:a[2]}:null}function ye({kind:n,children:a}){const r=n==="warning"||n==="caution"?e.jsx(ce,{size:13}):n==="tip"||n==="insight"?e.jsx(ue,{size:13}):e.jsx(pe,{size:13});return e.jsxs("aside",{className:`tut-callout tut-callout-${n}`,children:[e.jsx("span",{className:"tut-callout-icon",children:r}),e.jsx("span",{className:"tut-callout-body",children:a})]})}function J(n,a){if(!n)return null;const r=n.split(`
`),o=[];let i=[],s=!1,p="",h=[];const g=()=>{if(!i.length)return;const l=i.join(`
`).trim();if(!l){i=[];return}l.split(/\n{2,}/).forEach((c,m)=>{const b=c.trim();if(!b)return;const x=fe(b);if(x){o.push(e.jsx(ye,{kind:x.kind,children:C(x.text,`${a}-cl-${o.length}`)},`${a}-cl-${o.length}-${m}`));return}o.push(e.jsx("p",{children:C(b,`${a}-p-${o.length}`)},`${a}-p-${o.length}-${m}`))}),i=[]};for(let l=0;l<r.length;l++){const c=r[l],m=/^```(\w*)\s*$/.exec(c);if(m){s?(o.push(e.jsx("pre",{className:`tut-theory-pre tut-theory-pre-${p||"plain"}`,children:e.jsx("code",{children:h.join(`
`)})},`${a}-pre-${o.length}`)),s=!1,p="",h=[]):(g(),s=!0,p=m[1]||"");continue}if(s){h.push(c);continue}i.push(c)}return s&&o.push(e.jsx("pre",{className:`tut-theory-pre tut-theory-pre-${p||"plain"}`,children:e.jsx("code",{children:h.join(`
`)})},`${a}-pre-${o.length}`)),g(),o}function we({complexity:n}){if(!n)return null;if(typeof n=="string")return e.jsxs("div",{className:"tut-theory-complexity",children:[e.jsxs("span",{className:"tut-theory-cx-label",children:[e.jsx(U,{size:11})," Complexity"]}),e.jsx("span",{className:"tut-theory-cx-value",children:C(n,"cx-str")})]});const a=[];return n.best&&a.push(["Best",n.best]),n.average&&a.push(["Average",n.average]),n.worst&&a.push(["Worst",n.worst]),n.space&&a.push(["Space",n.space]),n.notes&&a.push(["Notes",n.notes]),e.jsxs("div",{className:"tut-theory-cx-card",children:[e.jsxs("div",{className:"tut-theory-cx-head",children:[e.jsx(U,{size:12}),e.jsx("span",{children:"Complexity"})]}),e.jsx("table",{className:"tut-theory-cx-table",children:e.jsx("tbody",{children:a.map(([r,o],i)=>e.jsxs("tr",{children:[e.jsx("th",{children:r}),e.jsx("td",{children:C(o,`cxr-${i}`)})]},i))})})]})}function ve({items:n}){return!n||!n.length?null:e.jsxs("div",{className:"tut-theory-pitfalls",children:[e.jsxs("div",{className:"tut-theory-pitfalls-head",children:[e.jsx(he,{size:12}),e.jsx("span",{children:"Pitfalls"})]}),e.jsx("ul",{children:n.map((a,r)=>e.jsx("li",{children:C(a,`pf-${r}`)},r))})]})}function G({body:n}){var a;return n?typeof n=="string"?e.jsx("div",{className:"tut-theory-content",children:J(n,"str")}):e.jsxs("div",{className:"tut-theory-content",children:[n.summary&&e.jsx("p",{className:"tut-theory-summary",children:C(n.summary,"sum")}),(a=n.sections)==null?void 0:a.map((r,o)=>e.jsxs("div",{className:"tut-theory-section",children:[e.jsx("h4",{className:"tut-theory-heading",children:r.heading}),Array.isArray(r.body)?e.jsx("ul",{className:"tut-theory-list",children:r.body.map((i,s)=>e.jsx("li",{children:C(i,`sec-${o}-li-${s}`)},s))}):J(r.body,`sec-${o}`)]},o)),e.jsx(we,{complexity:n.complexity}),e.jsx(ve,{items:n.pitfalls})]}):null}export{Ce as default};
