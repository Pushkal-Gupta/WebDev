...
The other special case is the **forget-gate bias of an LSTM**, traditionally initialised to \\(+1\\) so the network remembers by default at the start of training. The general principle: when a sigmoid-gated unit has a "do nothing / pass through" mode at one end of its range, bias the unit toward that mode at init so the gradient signal has a chance to teach the gate the cases where it should do something else.`},{kind:"code",language:"python",heading:"Kaiming init across a real model in PyTorch",body:`import torch
import torch.nn as nn
...
# this is 100x faster to debug than "loss is nan at step 47k" with no provenance.`},{kind:"prose",heading:'\\(\\log(1+x)\\), \\(e^x - 1\\), and other "tiny argument" helpers',body:"The log-sum-exp trick handles big positive arguments. The *other* end of the range — arguments very close to zero — needs its own family of fixes. Consider \\(\\log(1 + x)\\) for \\(x = 10^{-9}\\). In real arithmetic the answer is \\(\\approx 10^{-9}\\). In fp32 the answer is *zero*: the addition \\(1 + 10^{-9}\\) rounds back to \\(1\\) because \\(10^{-9}\\) is well below \\(\\varepsilon_{\\text{fp32}} \\approx 1.19 \\times 10^{-7}\\), so the entire \\(x\\) input was discarded before `log` ever ran.\n\nThe fix is `math.log1p(x)` (or `np.log1p`, `torch.log1p`), which computes \\(\\log(1 + x)\\) **as one fused operation** that uses a Taylor series for small \\(x\\) and never goes through the lossy \\(1 + x\\) addition. Symmetric helpers: `expm1(x)` computes \\(e^x - 1\\) accurately for small \\(x\\) (the naive `exp(x) - 1` cancels catastrophically near zero), `hypot(a, b)` computes \\(\\sqrt{a^2 + b^2}\\) without overflowing \\(a^2\\), and `logaddexp(a, b)` computes \\(\\log(e^a + e^b)\\) with the same max-subtraction trick as logsumexp.\n\nML places where these matter: KL divergences between very close distributions, Bernoulli log-likelihoods near \\(p = 0\\) or \\(p = 1\\), score-function gradients with tiny weights, log-probabilities of long sequences (summed log-probs of correctly-predicted high-confidence tokens are all just-below-zero values). Whenever your formula contains \\(\\log(\\text{thing close to 1})\\) or \\(\\exp(\\text{thing close to 0}) - 1\\), reach for \\(\\text{log1p}\\) / \\(\\text{expm1}\\) and the bug never happens."},{kind:"prose",heading:"\\(\\exp(x)\\) overflow — clip, scale, or use the stable variant",body:"\\(e^x\\) overflows in fp32 around \\(x = 88.7\\) and in fp16 around \\(x = 11.1\\). Attention logits scale linearly with the dot-product dimension before any \\(1/\\sqrt{d_k}\\) divisor; on a 4096-dim head an unscaled inner product easily lands in the hundreds. Boltzmann distributions with low temperature, importance weights, and Gaussian likelihoods with tight variance all produce arguments that look small on paper and overflow in float.\n\nThree patterns to know:\n\n1. **Use the fused stable kernel.** Most cases are softmax (or sigmoid, or cross-entropy), and the framework ships a stable version: `F.log_softmax`, `F.softmax` (internally max-subtracted), `F.cross_entropy`, `F.binary_cross_entropy_with_logits`. Reach for these *first* and the overflow never reaches your code.\n2. **Clip the input to the safe range.** When you genuinely need a raw `exp`, clip with `x.clamp(min=-50, max=50)` (in fp32) or `min=-9, max=9` (in fp16) before the call. You will lose information in the tails — but the tails are dominated by saturation anyway, so the practical accuracy cost is tiny.\n3. **Work in log-space throughout.** If your downstream consumer takes `log(exp(x) + ...)`, fuse it with `torch.logaddexp` or `torch.logsumexp` and never exponentiate. The classic example: NLL loss on per-token log-probabilities should *sum log-probs*, not *multiply probs then log*. Log-space addition is the log-sum-exp identity; log-space multiplication is just ordinary addition. Almost all probability arithmetic in deep learning lives in log-space for exactly this reason.\n\nWatch especially for `exp(-x)` of *large positive* \\(x\\): this underflows to zero in fp16 above \\(x = 11\\) and contaminates anything you divide by it (Bahdanau attention's exponential decay, RBF kernels with small bandwidth, importance sampling with small probability ratios)."},{kind:"ascii",heading:"Mixed-precision tradeoffs at a glance",caption:"fp32 vs bf16 vs fp16 vs fp8 — the bits, the range, the precision, the engineering cost.",body:`   format     sign  exp  mant   max value     min normal      eps (~)     dec. digits   notes
   ...
   for training in 2024+:  bf16 wins on H100/B200, A100, TPU.   fp16 only if your card lacks bf16.`},{kind:"prose",heading:"fp16 vs bf16 vs fp32 vs fp8 — the range-vs-precision tradeoff",body:`Every 16-bit format steals bits from somewhere. fp16 keeps 10 mantissa bits (good precision, only ~3 decimal digits) but uses just 5 exponent bits (tiny dynamic range — overflows at 65504). bf16 keeps the full fp32 exponent (8 bits, same range as fp32) and gives up the mantissa down to 7 bits (~2 decimal digits of precision). fp8 in either E4M3 or E5M2 layout barely reaches one digit of precision and only exists because the matmul tensor cores on H100+ are *fast* at it.
