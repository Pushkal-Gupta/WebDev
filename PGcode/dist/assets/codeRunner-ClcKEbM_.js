import{s as D}from"./index-CPOMntAX.js";const b={int:{jsdoc:"number",java:"int",cpp:"int",go:"int",c:"int",swift:"Int",kotlin:"Int",rust:"i64",typescript:"number"},float:{jsdoc:"number",java:"double",cpp:"double",go:"float64",c:"double",swift:"Double",kotlin:"Double",rust:"f64",typescript:"number"},str:{jsdoc:"string",java:"String",cpp:"string",go:"string",c:"char*",swift:"String",kotlin:"String",rust:"String",typescript:"string"},bool:{jsdoc:"boolean",java:"boolean",cpp:"bool",go:"bool",c:"bool",swift:"Bool",kotlin:"Boolean",rust:"bool",typescript:"boolean"},"List[int]":{jsdoc:"number[]",java:"int[]",cpp:"vector<int>",go:"[]int",c:"int*",swift:"[Int]",kotlin:"IntArray",rust:"Vec<i64>",typescript:"number[]"},"List[str]":{jsdoc:"string[]",java:"String[]",cpp:"vector<string>",go:"[]string",c:"char**",swift:"[String]",kotlin:"Array<String>",rust:"Vec<String>",typescript:"string[]"},"List[List[int]]":{jsdoc:"number[][]",java:"int[][]",cpp:"vector<vector<int>>",go:"[][]int",c:"int**",swift:"[[Int]]",kotlin:"Array<IntArray>",rust:"Vec<Vec<i64>>",typescript:"number[][]"},"List[List[str]]":{jsdoc:"string[][]",java:"String[][]",cpp:"vector<vector<string>>",go:"[][]string",c:"char***",swift:"[[String]]",kotlin:"Array<Array<String>>",rust:"Vec<Vec<String>>",typescript:"string[][]"},"List[bool]":{jsdoc:"boolean[]",java:"boolean[]",cpp:"vector<bool>",go:"[]bool",c:"bool*",swift:"[Bool]",kotlin:"BooleanArray",rust:"Vec<bool>",typescript:"boolean[]"},ListNode:{jsdoc:"ListNode",java:"ListNode",cpp:"ListNode*",go:"*ListNode",c:"struct ListNode*",swift:"ListNode?",kotlin:"ListNode?",rust:"Option<Box<ListNode>>",typescript:"ListNode | null"},"Optional[ListNode]":{jsdoc:"ListNode",java:"ListNode",cpp:"ListNode*",go:"*ListNode",c:"struct ListNode*",swift:"ListNode?",kotlin:"ListNode?",rust:"Option<Box<ListNode>>",typescript:"ListNode | null"},TreeNode:{jsdoc:"TreeNode",java:"TreeNode",cpp:"TreeNode*",go:"*TreeNode",c:"struct TreeNode*",swift:"TreeNode?",kotlin:"TreeNode?",rust:"Option<Rc<RefCell<TreeNode>>>",typescript:"TreeNode | null"},"Optional[TreeNode]":{jsdoc:"TreeNode",java:"TreeNode",cpp:"TreeNode*",go:"*TreeNode",c:"struct TreeNode*",swift:"TreeNode?",kotlin:"TreeNode?",rust:"Option<Rc<RefCell<TreeNode>>>",typescript:"TreeNode | null"}},q=r=>{var s;return((s=b[r])==null?void 0:s.java)||r},x=r=>{var s;return((s=b[r])==null?void 0:s.jsdoc)||r},I=r=>{var s;return((s=b[r])==null?void 0:s.cpp)||r},O=r=>{var s;return((s=b[r])==null?void 0:s.go)||r},z=r=>{var s;return((s=b[r])==null?void 0:s.c)||r},E=r=>{var s;return((s=b[r])==null?void 0:s.swift)||r},B=r=>{var s;return((s=b[r])==null?void 0:s.kotlin)||r},C=r=>{var s;return((s=b[r])==null?void 0:s.rust)||r},R=r=>{var s;return((s=b[r])==null?void 0:s.typescript)||r},L=r=>r==="ListNode"||r==="Optional[ListNode]",S=r=>r==="TreeNode"||r==="Optional[TreeNode]",y=r=>r.replace(/[A-Z]/g,(s,a)=>a===0?s.toLowerCase():"_"+s.toLowerCase()),J=`# Definition for singly-linked list.
# class ListNode:
 ...
 * }
 */
`;function pt(r,s,a,i){var v,N,w,m;if(!s||!a)return null;a.length===2&&((v=a[0])==null?void 0:v.type)==="List[int]"&&((N=a[0])==null?void 0:N.name)==="values"&&((w=a[1])==null?void 0:w.type)==="int"&&((m=a[1])==null?void 0:m.name)==="pos"&&(a=[{name:"head",type:"Optional[ListNode]"}]);const h=[...a.map(o=>o.type),i||""],d=h.some(L),g=h.some(S);if(r==="python"){const o=a.map(c=>`${c.name}: ${c.type}`).join(", ");let l="";return d&&(l+=J),g&&(l+=U),`${l}class Solution:
    def ${s}(self, ${o}) -> ${i}:
        `}if(r==="javascript"){const o=a.map(f=>` * @param {${x(f.type)}} ${f.name}`).join(`
`),l=a.map(f=>f.name).join(", ");let c="";return d&&(c+=M),g&&(c+=F),`${c}/**
${o}
 * @return {${x(i)}}
 */
var ${s} = function(${l}) {
    
};`}if(r==="java"){const o=a.map(l=>`${q(l.type)} ${l.name}`).join(", ");return`class Solution {
    public ${q(i)} ${s}(${o}) {
        
    }
}`}if(r==="cpp"){const o=a.map(c=>{const f=I(c.type);return f.startsWith("vector<")||f==="string"?`${f}& ${c.name}`:`${f} ${c.name}`}).join(", ");let l="";return d&&(l+=G),g&&(l+=W),`${l}#include <bits/stdc++.h>
using namespace std;
...
};`}if(r==="c"){const o=[];a.forEach(_=>{if(_.type==="List[int]"||_.type==="List[bool]"){const $=_.type==="List[bool]"?"bool":"int";o.push(`${$}* ${_.name}`,`int ${_.name}Size`)}else _.type==="List[str]"?o.push(`char** ${_.name}`,`int ${_.name}Size`):_.type==="List[List[int]]"?o.push(`int** ${_.name}`,`int ${_.name}Size`,`int* ${_.name}ColSize`):_.type==="List[List[str]]"?o.push(`char*** ${_.name}`,`int ${_.name}Size`,`int* ${_.name}ColSize`):o.push(`${z(_.type)} ${_.name}`)});const l=i==="List[int]"||i==="List[bool]",c=i==="List[str]",f=i==="List[List[int]]",t=i==="List[List[str]]";(l||c)&&o.push("int* returnSize"),(f||t)&&o.push("int* returnSize","int** returnColumnSizes");let e;l?e=i==="List[bool]"?"bool*":"int*":c?e="char**":f?e="int**":t?e="char***":e=z(i);let p;e==="int"||e==="double"?p="    return 0;":e==="bool"?p="    return false;":l||c?p=`    *returnSize = 0;
    return NULL;`:f||t?p=`    *returnSize = 0;
    *returnColumnSizes = NULL;
    return NULL;`:e.endsWith("*")?p="    return NULL;":p="    ";let n="";return d&&(n+=H),g&&(n+=Y),`#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
...
${n}${e} ${s}(${o.join(", ")}) {
${p}
}`}if(r==="go"){const o=a.map(t=>`${t.name} ${O(t.type)}`).join(", ");let l="";d&&(l+=Q),g&&(l+=K);const c=O(i);let f;return c==="int"||c==="float64"?f="0":c==="bool"?f="false":c==="string"?f='""':c.startsWith("[]")?f=`${c}{}`:(c.startsWith("*"),f="nil"),`${l}func ${s}(${o}) ${c} {
    return ${f}
}`}if(r==="swift"){const o=a.map(l=>`_ ${l.name}: ${E(l.type)}`).join(", ");return`class Solution {
    func ${s}(${o}) -> ${E(i)} {
        
    }
}`}if(r==="kotlin"){const o=a.map(l=>`${l.name}: ${B(l.type)}`).join(", ");return`class Solution {
    fun ${s}(${o}): ${B(i)} {
        
    }
}`}if(r==="rust"){const o=y(s),l=a.map(c=>`${y(c.name)}: ${C(c.type)}`).join(", ");return`impl Solution {
    pub fn ${o}(${l}) -> ${C(i)} {
        
    }
}`}if(r==="typescript"){const o=a.map(l=>`${l.name}: ${R(l.type)}`).join(", ");return`function ${s}(${o}): ${R(i)} {
...
`;function lt(r){var s,a,i,u;return Array.isArray(r)&&r.length===2&&((s=r[0])==null?void 0:s.type)==="List[int]"&&((a=r[0])==null?void 0:a.name)==="values"&&((i=r[1])==null?void 0:i.type)==="int"&&((u=r[1])==null?void 0:u.name)==="pos"}function ft(r,s,a,i,u,h={}){if(!a||!i)return r;const d=lt(i),g=d?"head":i.map(m=>m.name).join(", "),v=L(u),N=S(u),w=Math.max(1,Number(h.multiCaseCount)||1);if(s==="python"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return["import sys, json","from typing import List, Optional, Dict, Tuple, Set",P,r,"","_ops = json.loads(sys.stdin.read().strip())","_results = []","_instance = None","for _op in _ops:","    _name = _op[0]","    _args = _op[1:]","    if _instance is None:","        _cls = globals()[_name]","        _instance = _cls(*_args)","        _results.append(None)","    else:","        _ret = getattr(_instance, _name)(*_args)","        _results.append(_ret)","print(json.dumps(_results))"].join(`
`);const o=d?["_values = json.loads(_lines[0])","_pos = int(_lines[1].strip())","head = _to_list_cycle(_values, _pos)"].join(`
`):i.map((c,f)=>L(c.type)?`${c.name} = _to_list(json.loads(_lines[${f}]))`:S(c.type)?`${c.name} = _to_tree(json.loads(_lines[${f}]))`:`${c.name} = json.loads(_lines[${f}])`).join(`
`);let l;return v?l="print(json.dumps(_from_list(_result)))":N?l="print(json.dumps(_from_tree(_result)))":l=["if isinstance(_result, bool):","    print(str(_result).lower())","elif _result is None:",'    print("null")',"elif isinstance(_result, str):","    print(_result)","else:","    print(json.dumps(_result))"].join(`
`),["from __future__ import annotations","import sys, json","from typing import List, Optional, Dict, Tuple, Set",P,r,"",'_lines = sys.stdin.read().strip().split("\\n")',o,"_sol = Solution()",`_result = _sol.${a}(${g})`,l].join(`
`)}if(s==="javascript"){const m=d?["const _values = JSON.parse(_lines[0]);","const _pos = parseInt(_lines[1].trim(), 10);","const head = _toListCycle(_values, _pos);"].join(`
`):i.map((l,c)=>L(l.type)?`const ${l.name} = _toList(JSON.parse(_lines[${c}]));`:S(l.type)?`const ${l.name} = _toTree(JSON.parse(_lines[${c}]));`:`const ${l.name} = JSON.parse(_lines[${c}]);`).join(`
`);let o;return v?o="console.log(JSON.stringify(_fromList(_result)));":N?o="console.log(JSON.stringify(_fromTree(_result)));":o="console.log(JSON.stringify(_result));",[X,r,"","const _lines = require('fs').readFileSync('/dev/stdin', 'utf8').trim().split('\\n');",m,`const _result = ${a}(${g});`,o].join(`
`)}if(s==="java"){const m=d?["String _raw_values = br.readLine().trim();","        String _raw_pos = br.readLine().trim();","        int[] _values = _parseIntArr(_raw_values);","        int _pos = Integer.parseInt(_raw_pos);","        ListNode head = _toListCycle(_values, _pos);"].join(`
`):i.map(t=>{const e=`String _raw_${t.name} = br.readLine().trim();`;return t.type==="int"?`${e}
        ...
        `):L(t.type)?[e,`        ListNode ${t.name} = _toList(_parseIntArr(_raw_${t.name}));`].join(`
        `):S(t.type)?[e,`        TreeNode ${t.name} = _toTree(_parseIntegerArr(_raw_${t.name}));`].join(`
        `):`${e}
        // TODO: parse ${t.type}`}).join(`
        `),o=v?[`        ListNode _result = sol.${a}(${g});`,"        System.out.println(_jsonList(_result));"].join(`
`):N?[`        TreeNode _result = sol.${a}(${g});`,"        System.out.println(_jsonTree(_result));"].join(`
`):[`        Object _result = (Object) sol.${a}(${g});`,"        System.out.println(_jsonify(_result));"].join(`
`),l=v?[`                ListNode _result = sol.${a}(${g});`,"                _out.append(_jsonList(_result));"].join(`
`):N?[`                TreeNode _result = sol.${a}(${g});`,"                _out.append(_jsonTree(_result));"].join(`
`):[`                Object _result = (Object) sol.${a}(${g});`,"                _out.append(_jsonify(_result));"].join(`
`),c=w>1?["        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));","        Solution sol = new Solution();",`        int _N = ${w};`,"        StringBuilder _out = new StringBuilder();","        for (int _c = 0; _c < _N; _c++) {","            try {",`                ${m}`,l,"            } catch (Throwable _t) {",`                _out.append("${k}").append(_t.getClass().getSimpleName()).append(": ").append(String.valueOf(_t.getMessage()));`,"            }",`            _out.append('\\n').append("${j}").append('\\n');`,"            if (_c < _N - 1) {","                // Consume the case separator line from stdin","                String _sep = br.readLine();","                // tolerate missing separator silently","            }","        }","        System.out.print(_out.toString());"].join(`
`):["        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));",`        ${m}`,"        Solution sol = new Solution();",o].join(`
...
`)}if(s==="cpp"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=(t,e)=>{const p=`_raw_${t}`;return e==="int"?`long long ${t}_ll = stoll(_pgc_trim(${p})); int ${t} = (int)${t}_ll;`:e==="float"?`double ${t} = stod(_pgc_trim(${p}));`:e==="bool"?`bool ${t} = (_pgc_trim(${p}) == "true");`:e==="str"?`string ${t} = _pgc_strip_quotes(${p});`:e==="List[int]"?`vector<int> ${t} = _pgc_parse_vi(${p});`:e==="List[str]"?`vector<string> ${t} = _pgc_parse_vs(${p});`:e==="List[bool]"?`vector<bool> ${t} = _pgc_parse_vb(${p});`:e==="List[List[int]]"?`vector<vector<int>> ${t} = _pgc_parse_vvi(${p});`:e==="List[List[str]]"?`vector<vector<string>> ${t} = _pgc_parse_vvs(${p});`:L(e)?`ListNode* ${t} = _pgc_to_list(_pgc_parse_vi(${p}));`:S(e)?`TreeNode* ${t} = _pgc_to_tree(_pgc_parse_tree_tokens(${p}));`:`string ${t} = ${p}; // TODO: unsupported type ${e}`};let l;u==="int"?l="_pgc_ser_int((long long)_result)":u==="float"?l="_pgc_ser_double(_result)":u==="bool"?l="_pgc_ser_bool(_result)":u==="str"?l="_pgc_ser_str(_result)":u==="List[int]"?l="_pgc_ser_vi(_result)":u==="List[str]"?l="_pgc_ser_vs(_result)":u==="List[bool]"?l="_pgc_ser_vb(_result)":u==="List[List[int]]"?l="_pgc_ser_vvi(_result)":u==="List[List[str]]"?l="_pgc_ser_vvs(_result)":v?l="_pgc_ser_vi(_pgc_from_list(_result))":N?l="_pgc_from_tree(_result)":l="([&]{ ostringstream _o; _o << _result; return _o.str(); })()";const c=d?["    string _raw_values; getline(cin, _raw_values);","    string _raw_pos; getline(cin, _raw_pos);","    vector<int> _values = _pgc_parse_vi(_raw_values);","    int _pos = stoi(_pgc_trim(_raw_pos));","    ListNode* head = _pgc_to_list_cycle(_values, _pos);",`    auto _result = sol.${a}(head);`,`    _out << ${l};`].join(`
`):[...i.map(t=>`    string _raw_${t.name}; getline(cin, _raw_${t.name});`),...i.map(t=>`    ${o(t.name,t.type)}`),`    auto _result = sol.${a}(${i.map(t=>t.name).join(", ")});`,`    _out << ${l};`].join(`
`),f=w>1?["    Solution sol;","    ostringstream _out;",`    int _N = ${w};`,"    for (int _c = 0; _c < _N; _c++) {","        try {",c.split(`
`).map(t=>"        "+t).join(`
`),"        } catch (const std::exception& _e) {",`            _out << "${k}" << _e.what();`,"        } catch (...) {",`            _out << "${k}" << "unknown";`,"        }",`        _out << '\\n' << "${j}" << '\\n';`,"        if (_c < _N - 1) {","            string _sep; getline(cin, _sep); // consume case separator","        }","    }","    cout << _out.str();"].join(`
`):["    Solution sol;","    ostringstream _out;",c,"    _out << '\\n';","    cout << _out.str();"].join(`
`);return[tt,r,"","int main() {","    ios_base::sync_with_stdio(false);","    cin.tie(nullptr);",f,"    return 0;","}"].join(`
`)}if(s==="c"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=u==="List[int]",l=u==="List[bool]",c=u==="List[str]",f=u==="List[List[int]]",t=[],e=[];d?(t.push("    char* _raw_values = _pgc_read_line();"),t.push("    char* _raw_pos = _pgc_read_line();"),t.push("    int _pos = atoi(_pgc_trim(_raw_pos));"),t.push("    struct ListNode* head = _pgc_to_list_cycle(_raw_values, _pos);"),e.push("head")):i.forEach(n=>{const _=`_raw_${n.name}`;t.push(`    char* ${_} = _pgc_read_line();`),n.type==="int"?(t.push(`    int ${n.name} = atoi(_pgc_trim(${_}));`),e.push(n.name)):n.type==="float"?(t.push(`    double ${n.name} = atof(_pgc_trim(${_}));`),e.push(n.name)):n.type==="bool"?(t.push(`    bool ${n.name} = strcmp(_pgc_trim(${_}), "true") == 0;`),e.push(n.name)):n.type==="str"?(t.push(`    char* ${n.name} = strdup(_pgc_strip_quotes(${_}));`),e.push(n.name)):n.type==="List[int]"?(t.push(`    int ${n.name}Size; int* ${n.name} = _pgc_parse_int_arr(${_}, &${n.name}Size);`),e.push(n.name,`${n.name}Size`)):n.type==="List[bool]"?(t.push(`    int ${n.name}Size; bool* ${n.name} = _pgc_parse_bool_arr(${_}, &${n.name}Size);`),e.push(n.name,`${n.name}Size`)):n.type==="List[str]"?(t.push(`    int ${n.name}Size; char** ${n.name} = _pgc_parse_str_arr(${_}, &${n.name}Size);`),e.push(n.name,`${n.name}Size`)):n.type==="List[List[int]]"?(t.push(`    int ${n.name}Size; int* ${n.name}ColSize; int** ${n.name} = _pgc_parse_int_2d(${_}, &${n.name}Size, &${n.name}ColSize);`),e.push(n.name,`${n.name}Size`,`${n.name}ColSize`)):L(n.type)?(t.push(`    struct ListNode* ${n.name} = _pgc_to_list(${_});`),e.push(n.name)):S(n.type)?(t.push(`    struct TreeNode* ${n.name} = _pgc_to_tree(${_});`),e.push(n.name)):(t.push(`    char* ${n.name} = ${_}; // TODO: unsupported type ${n.type}`),e.push(n.name))});let p;if(o||l||c){const n=c?"char**":l?"bool*":"int*",_=c?"_pgc_print_str_arr":l?"_pgc_print_bool_arr":"_pgc_print_int_arr";p=["    int _retSize;",`    ${n} _result = ${a}(${e.join(", ")}, &_retSize);`,`    ${_}(_result, _retSize);`,"    putchar('\\n');"].join(`
`)}else f?p=["    int _retSize; int* _retColSizes;",`    int** _result = ${a}(${e.join(", ")}, &_retSize, &_retColSizes);`,"    _pgc_print_int_2d(_result, _retSize, _retColSizes);","    putchar('\\n');"].join(`
`):L(u)?p=[`    struct ListNode* _result = ${a}(${e.join(", ")});`,"    _pgc_print_list(_result);","    putchar('\\n');"].join(`
`):S(u)?p=[`    struct TreeNode* _result = ${a}(${e.join(", ")});`,"    _pgc_print_tree(_result);","    putchar('\\n');"].join(`
`):u==="int"?p=[`    int _result = ${a}(${e.join(", ")});`,"    _pgc_print_int(_result);","    putchar('\\n');"].join(`
`):u==="float"?p=[`    double _result = ${a}(${e.join(", ")});`,"    _pgc_print_double(_result);","    putchar('\\n');"].join(`
`):u==="bool"?p=[`    bool _result = ${a}(${e.join(", ")});`,"    _pgc_print_bool(_result);","    putchar('\\n');"].join(`
`):u==="str"?p=[`    char* _result = ${a}(${e.join(", ")});`,"    _pgc_print_str(_result);","    putchar('\\n');"].join(`
`):p=[`    ${a}(${e.join(", ")});`,"    putchar('\\n');"].join(`
`);return[et,r,"","int main(void) {",...t,p,"    return 0;","}"].join(`
`)}if(s==="go"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=[],l=d?["head"]:i.map(e=>e.name);d?(o.push("    var _values []int"),o.push("    json.Unmarshal([]byte(_lines[0]), &_values)"),o.push("    _pos, _ := strconv.Atoi(strings.TrimSpace(_lines[1]))"),o.push("    head := _pgc_arr_to_list_cycle(_values, _pos)")):i.forEach((e,p)=>{const n=`_lines[${p}]`;e.type==="int"?(o.push(`    var ${e.name} int`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="float"?(o.push(`    var ${e.name} float64`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="bool"?(o.push(`    var ${e.name} bool`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="str"?(o.push(`    var ${e.name} string`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="List[int]"?(o.push(`    var ${e.name} []int`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="List[bool]"?(o.push(`    var ${e.name} []bool`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="List[str]"?(o.push(`    var ${e.name} []string`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="List[List[int]]"?(o.push(`    var ${e.name} [][]int`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):e.type==="List[List[str]]"?(o.push(`    var ${e.name} [][]string`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`)):L(e.type)?(o.push(`    var ${e.name}Arr []int`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name}Arr)`),o.push(`    ${e.name} := _pgc_arr_to_list(${e.name}Arr)`)):S(e.type)?o.push(`    ${e.name} := _pgc_parse_tree_line(${n})`):(o.push(`    var ${e.name} interface{}`),o.push(`    json.Unmarshal([]byte(${n}), &${e.name})`))});let c;L(u)?c=["    _arr := _pgc_list_to_arr(_result)","    fmt.Println(_pgc_jsonify(_arr))"].join(`
`):S(u)?c="    fmt.Println(_pgc_tree_to_json(_result))":c="    fmt.Println(_pgc_jsonify(_result))";const f=["func main() {","    scanner := bufio.NewScanner(os.Stdin)","    scanner.Buffer(make([]byte, 1024*1024), 1024*1024*16)","    var _lines []string","    for scanner.Scan() {","        _lines = append(_lines, scanner.Text())","    }","    _ = _lines",...o,`    _result := ${a}(${l.join(", ")})`,c,"}"].join(`
`);let t=r.replace(/^[\t ]*package\s+main[\t ]*\r?\n/m,"").replace(/^[\t ]*import\s*\([\s\S]*?\)\r?\n/m,"").replace(/^[\t ]*import\s+"[^"]+"\r?\n/gm,"");return[rt,t,"",f].join(`
`)}if(s==="swift"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=(t,e,p)=>{const n=`_v${p}`,_=`        let ${n}: Any? = _pgcReadJSON(_lines[${p}])`;return e==="int"?`${_}
        let ${t}: Int = _pgcAsInt(${n})`:e==="float"?`${_}
        let ${t}: Double = _pgcAsDouble(${n})`:e==="bool"?`${_}
        let ${t}: Bool = _pgcAsBool(${n})`:e==="str"?`${_}
        let ${t}: String = _pgcAsString(${n})`:e==="List[int]"?`${_}
        let ${t}: [Int] = _pgcIntArr(${n})`:e==="List[bool]"?`${_}
        let ${t}: [Bool] = _pgcBoolArr(${n})`:e==="List[str]"?`${_}
        let ${t}: [String] = _pgcStrArr(${n})`:e==="List[List[int]]"?`${_}
        let ${t}: [[Int]] = _pgcIntIntArr(${n})`:e==="List[List[str]]"?`${_}
        let ${t}: [[String]] = _pgcStrStrArr(${n})`:L(e)?`${_}
        let ${t}: ListNode? = _pgcArrToList(_pgcIntArr(${n}))`:S(e)?`        let ${t}: TreeNode? = _pgcParseTreeLine(_lines[${p}])`:`${_}
        let ${t}: Any? = ${n}`},l=d?["        let _values: [Int] = _pgcIntArr(_pgcReadJSON(_lines[0]))","        let _pos: Int = Int(_lines[1].trimmingCharacters(in: .whitespacesAndNewlines)) ?? -1","        let head: ListNode? = _pgcArrToListCycle(_values, _pos)"].join(`
`):i.map((t,e)=>o(t.name,t.type,e)).join(`
`),c=d?"head":i.map(t=>t.name).join(", ");let f;return v?f=`        let _arr = _pgcListToArr(_result)
        print(_pgcJsonify(_arr))`:N?f="        print(_pgcTreeToArr(_result))":u==="bool"?f='        print(_result ? "true" : "false")':u==="int"?f="        print(_result)":f="        print(_pgcJsonify(_result))",[nt,r,"","let _input = FileHandle.standardInput.availableData",'let _raw = String(data: _input, encoding: .utf8) ?? ""','let _lines = _raw.split(separator: "\\n", omittingEmptySubsequences: false).map { String($0) }',"","do {",l,`    let _result = Solution().${a}(${c})`,f,"}"].join(`
`)}if(s==="kotlin"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=(t,e,p)=>{const n=`_lines[${p}]`;return e==="int"?`        val ${t}: Int = ${n}.trim().toInt()`:e==="float"?`        val ${t}: Double = ${n}.trim().toDouble()`:e==="bool"?`        val ${t}: Boolean = ${n}.trim() == "true"`:e==="str"?`        val ${t}: String = PGC.stripQuotes(${n})`:e==="List[int]"?`        val ${t}: IntArray = PGC.parseIntArr(${n})`:e==="List[bool]"?`        val ${t}: BooleanArray = PGC.parseBoolArr(${n})`:e==="List[str]"?`        val ${t}: Array<String> = PGC.parseStrArr(${n})`:e==="List[List[int]]"?`        val ${t}: Array<IntArray> = PGC.parseIntIntArr(${n})`:e==="List[List[str]]"?`        val ${t}: Array<Array<String>> = PGC.parseStrStrArr(${n})`:L(e)?`        val ${t}: ListNode? = PGC.arrToList(PGC.parseIntArr(${n}))`:S(e)?`        val ${t}: TreeNode? = PGC.arrToTree(PGC.parseTreeTokens(${n}))`:`        val ${t}: String = ${n}`},l=d?["        val _values: IntArray = PGC.parseIntArr(_lines[0])","        val _pos: Int = _lines[1].trim().toInt()","        val head: ListNode? = PGC.arrToListCycle(_values, _pos)"].join(`
`):i.map((t,e)=>o(t.name,t.type,e)).join(`
`),c=d?"head":i.map(t=>t.name).join(", ");let f;return v?f="    println(PGC.listToStr(_result))":N?f="    println(PGC.treeToStr(_result))":f="    println(PGC.jsonify(_result))",[it,r,"","fun main() {","    val _lines = System.`in`.bufferedReader().readLines()",l,`    val _result = Solution().${a}(${c})`,f,"}"].join(`
`)}if(s==="rust"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=y(a),l=(e,p,n)=>{const _=y(e),$=`&_lines[${n}]`;return p==="int"?`    let ${_}: i64 = _pgc_parse_i64(${$});`:p==="float"?`    let ${_}: f64 = _pgc_parse_f64(${$});`:p==="bool"?`    let ${_}: bool = _pgc_parse_bool(${$});`:p==="str"?`    let ${_}: String = _pgc_parse_str(${$});`:p==="List[int]"?`    let ${_}: Vec<i64> = _pgc_parse_vi(${$});`:p==="List[bool]"?`    let ${_}: Vec<bool> = _pgc_parse_vb(${$});`:p==="List[str]"?`    let ${_}: Vec<String> = _pgc_parse_vs(${$});`:p==="List[List[int]]"?`    let ${_}: Vec<Vec<i64>> = _pgc_parse_vvi(${$});`:p==="List[List[str]]"?`    let ${_}: Vec<Vec<String>> = _pgc_parse_vvs(${$});`:L(p)?`    let ${_}: Option<Box<ListNode>> = _pgc_arr_to_list(&_pgc_parse_vi(${$}));`:S(p)?`    let ${_}: Option<Rc<RefCell<TreeNode>>> = _pgc_arr_to_tree(&_pgc_parse_tree_tokens(${$}));`:`    let ${_}: String = _lines[${n}].clone();`},c=d?["    let _values: Vec<i64> = _pgc_parse_vi(&_lines[0]);","    let _pos: i64 = _pgc_parse_i64(&_lines[1]);","    let head: Option<Box<ListNode>> = _pgc_arr_to_list_cycle(&_values, _pos);"].join(`
`):i.map((e,p)=>l(e.name,e.type,p)).join(`
`),f=d?"head":i.map(e=>y(e.name)).join(", ");let t;return v?t=["    let _arr = _pgc_list_to_vec(_result);",'    println!("{}", _pgc_ser_vi(&_arr));'].join(`
`):N?t='    println!("{}", _pgc_tree_to_str(_result));':u==="int"?t='    println!("{}", _pgc_ser_i64(_result));':u==="float"?t='    println!("{}", _pgc_ser_f64(_result));':u==="bool"?t='    println!("{}", _pgc_ser_bool(_result));':u==="str"?t='    println!("{}", _pgc_ser_str(&_result));':u==="List[int]"?t='    println!("{}", _pgc_ser_vi(&_result));':u==="List[bool]"?t='    println!("{}", _pgc_ser_vb(&_result));':u==="List[str]"?t='    println!("{}", _pgc_ser_vs(&_result));':u==="List[List[int]]"?t='    println!("{}", _pgc_ser_vvi(&_result));':u==="List[List[str]]"?t='    println!("{}", _pgc_ser_vvs(&_result));':t='    println!("{:?}", _result);',[st,r,"","fn main() {","    let mut _buf = String::new();","    io::stdin().read_to_string(&mut _buf).ok();","    let _lines: Vec<String> = _buf.lines().map(|s| s.to_string()).collect();",c,`    let _result = Solution::${o}(${f});`,t,"}"].join(`
`)}if(s==="typescript"){if(i.length===1&&i[0].name==="operations"&&i[0].type&&i[0].type.startsWith("List[List"))return r;const o=(t,e,p)=>{const n=`_lines[${p}]`;return L(e)?`const ${t}: ListNode | null = _pgcToList(JSON.parse(${n}));`:S(e)?`const ${t}: TreeNode | null = _pgcToTree(JSON.parse(${n}));`:`const ${t}: any = JSON.parse(${n});`},l=d?["const _values: any = JSON.parse(_lines[0]);","const _pos: number = parseInt(_lines[1].trim(), 10);","const head: ListNode | null = _pgcToListCycle(_values, _pos);"].join(`
`):i.map((t,e)=>o(t.name,t.type,e)).join(`
`),c=d?"head":i.map(t=>t.name).join(", ");let f;return v?f="console.log(JSON.stringify(_pgcFromList(_result as ListNode | null)));":N?f="console.log(JSON.stringify(_pgcFromTree(_result as TreeNode | null)));":f="console.log(JSON.stringify(_result));",[ot,r,"","const _raw = require('fs').readFileSync('/dev/stdin', 'utf8');","const _lines = _raw.split('\\n');",l,`const _result: any = ${a}(${c} as any);`,f].join(`
`)}return r}function dt(r){return(r||[]).join(`
`)}function A(r,s){if(r===s)return!0;if(r===null||s===null)return r===s;if(typeof r!=typeof s)return!1;if(typeof r!="object")return r===s;if(Array.isArray(r)!==Array.isArray(s))return!1;if(Array.isArray(r)){if(r.length!==s.length)return!1;for(let u=0;u<r.length;u++)if(!A(r[u],s[u]))return!1;return!0}const a=Object.keys(r),i=Object.keys(s);if(a.length!==i.length)return!1;for(const u of a)if(!Object.prototype.hasOwnProperty.call(s,u)||!A(r[u],s[u]))return!1;return!0}function at(r,s){const a=(s??"").toString().trim(),i=(r??"").toString().trim();if(a===i)return!0;let u,h,d=!1,g=!1;try{u=JSON.parse(a),d=!0}catch{}try{h=JSON.parse(i),g=!0}catch{}if(d&&g)return A(u,h);const v=a.replace(/\s+/g,"").toLowerCase(),N=i.replace(/\s+/g,"").toLowerCase();return v===N}function gt(r,s){return at(s,r)}const ut="https://ce.judge0.com/submissions?base64_encoded=false&wait=true",T={python:{id:71,name:"Python 3",monaco:"python",harness:!0},javascript:{id:63,name:"JavaScript",monaco:"javascript",harness:!0},java:{id:62,name:"Java",monaco:"java",harness:!0},cpp:{id:54,name:"C++",monaco:"cpp",harness:!0},c:{id:50,name:"C",monaco:"c",harness:!0},go:{id:60,name:"Go",monaco:"go",harness:!0},rust:{id:73,name:"Rust",monaco:"rust",harness:!0},typescript:{id:74,name:"TypeScript",monaco:"typescript",harness:!0},csharp:{id:51,name:"C#",monaco:"csharp",harness:!1},ruby:{id:72,name:"Ruby",monaco:"ruby",harness:!1},kotlin:{id:78,name:"Kotlin",monaco:"kotlin",harness:!0},swift:{id:83,name:"Swift",monaco:"swift",harness:!0},php:{id:68,name:"PHP",monaco:"php",harness:!1},bash:{id:46,name:"Bash",monaco:"shell",harness:!1}},ht=Object.entries(T).map(([r,s])=>({value:r,label:s.name,monaco:s.monaco}));Object.entries(T).filter(([,r])=>r.harness).map(([r,s])=>({value:r,label:s.name,monaco:s.monaco}));async function V(r,s,a){if(!T[s])throw new Error(`Unsupported language: ${s}`);if(!Array.isArray(a)||a.length===0)return[];const{data:i,error:u}=await D.functions.invoke("run-code",{body:{code:r,language:s,stdins:a}});if(u){console.warn("run-code edge function failed, falling back to direct Judge0:",u.message);const h=[];for(const d of a)h.push(await ct(r,s,d));return h}if(i!=null&&i.error)throw new Error(i.error);return i.results}async function vt(r,s,a){if(!Array.isArray(a)||a.length===0)return[];const i=a.join(`
${Z}
`),[u]=await V(r,s,[i]);if(u.status!=="success")return a.map(()=>({status:u.status,output:u.output}));const h=(u.output||"").split(`${j}
`).map(g=>g.replace(/\n$/,"")),d=[];for(let g=0;g<a.length;g++){const v=h[g];if(v===void 0){d.push({status:"runtime_error",output:"Execution aborted before this case ran"});continue}if(v.startsWith(k)){d.push({status:"runtime_error",output:v.slice(k.length)});continue}d.push({status:"success",output:v})}return d}async function mt(r,s,a=""){return(await V(r,s,[a]))[0]}async function ct(r,s,a=""){var g;const i=T[s],u=await fetch(ut,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({language_id:i.id,source_code:r,stdin:a})});if(!u.ok)throw new Error(`Execution service returned ${u.status}`);const h=await u.json(),d=(g=h.status)==null?void 0:g.id;return d===6?{status:"compile_error",output:h.compile_output||"Compilation failed"}:d===5?{status:"time_limit",output:"Time Limit Exceeded (5s)"}:d===3?{status:"success",output:h.stdout||"(No output)"}:{status:"runtime_error",output:h.stderr||h.compile_output||h.message||"Runtime error"}}export{ht as P,vt as a,dt as b,V as c,gt as d,pt as g,mt as r,ft as w};
