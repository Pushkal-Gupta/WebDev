...
function normalizeComplexity(input) {
  ...
  if (s.includes('o(logn)') || s.includes('o(log_2n)')) return 'O(log n)';
  if (s.includes('o(nlogn)') || s.includes('o(n·logn)')) return 'O(n log n)';
  if (s.includes('o(n^2)') || s.includes('o(n²)') || s.includes('o(n*n)')) return 'O(n²)';
  if (s.includes('o(2^n)') || s.includes('o(2ⁿ)')) return 'O(2ⁿ)';
  if (s.includes('o(n)') || s.includes('o(v+e)') || s.includes('o(v·e)')) return 'O(n)';
  return null;
}
  ...
  return (
    <div className="ccx" role="img" aria-label="Growth-rate comparison chart">
      ...
      <p className="ccx-caption">
        ...
        {highlight ? <> Your <code>{highlight}</code> bar is the one highlighted.</> : null}
